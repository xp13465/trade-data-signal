...
- **ai长线模式(G/H/I)仓位管理（2026-08-14 #49 新增，默认关，叠加现有判断不改 A-F）**：`策略实验室`凯利回测区新增「ai长线模式(G/H/I)仓位管理」独立开关（长线族群总入口）——对 G/H/I 三中长线模式套「**持仓≤20万（20倍单次本金，单次本金=1万）+ FIFO 强制平最久持仓**」硬控，目标=本金可控化（关态 G/H/I 需 45-148 倍单次本金不可操作）；架构=模式→策略映射（G/H/I v1 统一 `fifo20w`，后续某模式优化只改该模式策略映射不动按钮整体）；默认关，开启后 G/H/I 卡片行套硬控值（乐观 b1 口径、标「AI长线·开」角标）+「G/H/I 对比表」展示关/开(保守b0)/开(乐观b1)三态（数据来源 [`docs/kelly-ghiposition-manage-matrix.md`](docs/kelly-ghiposition-manage-matrix.md) §7.2 推荐 K=1 参考口径，前端 FIFO 仿真内核与报告逐位对齐 §21）；效果按模式区分（tooltip 分写说明）：G 最值得开（关 47.2%/+64.2万/136万 → 开乐观 200.5%/+40.1万/20万）、I 值得开（关 39.5%/+43.9万/111万 → 开乐观 153.4%/+30.7万/20万）、H 用收益换本金可控慎用（已纳入，后期研究为什么差）；诚实标注：强平日真实盈亏不可知（保守 b0=按 0 利 / 乐观 b1=按持有时间线性，真实值在区间，不把乐观当承诺）；开 cap 后 G/H/I 全为 K=1 最优（按钮旁提示建议切 K=1 收益更高，前端默认 K=3）；A-F 短线模式天然≤20 倍不受影响
