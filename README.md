...
- **首页走势图卡（「近期技术参考点」下方「汪汪队信号」前方）**：监控策略参数是否「历史拟合好·未来失灵」。两张曲线——①**准确率**（实盘实际 vs 回测预期，显示范围30/60/90交易日，统计口径固定60日滚动），准确率口径=信号后方向命中（实盘=信号日收盘→最新收盘方向，看多涨/看空跌，band_hold与未结算不计，与首页「近期技术参考点」汇总条 since_correct 同口径；回测=按卖出模式到期收益方向）②**综合过拟合风险分**（0-100），= 0.40×回测-实盘偏离 + 0.25×滚动样本外衰减 + 0.20×参数稳定 + 0.15×象限退化，**绿<30正常 / 黄30-60关注 / 红>60高风险**（参考线30/60，曲线按值实时变色——绿/黄/红分段）。**2026-08-16 三合一改造③：两图从 echarts 直绘改为首页轻量 SVG 引擎 `_lwSetup` 渲染**（与恐贪/A股情绪分同款外观：绿黄红分段走 `_lwColorFn`、参考线走 markLine、准确率双曲线走多 series），零 echarts.init 首屏提速、支持「走势图渲染」charts.lightweight 开关即时切换。**2026-08-16 二次迭代：SVG 3 色为基准**，echarts fallback 去固定色让 visualMap 分段生效（lite/echarts 两模式对齐绿黄红）。
- **维度切换（2026-08-15 新增，三组按钮两图联动；2026-08-16 窗口改「显示范围」）**：①**显示范围** 30/60/90（横轴截取最近 N 个交易日展示，**只影响显示截取、统计口径固定 60 日滚动**——选 30 或 90 都是 60 窗口算，只是图上看多长；2026-08-16 更名自「窗口」，优化 30/60/90 三套 rolling 只留 60 一套降体积）②**评级** 全部/高/中/低（按回测 rating 或实盘 signal_stats·10日score 分档）③**类型** 全部/主买/辅买/追买/备买/卖/止损卖（「追买」=买点类型 buy_special，与全站信号叫法统一）。一次点任一按钮 → 准确率 + 风险分两图同时重绘。**卖出类回测仅买入信号**故只显示实盘单曲线（标注「回测仅买入信号」）。⚠**样本去重**：回测同一笔交易按 16 象限×卖出模式重复计数，已按 (模式+信号日+标的+信号) 去重，n 为真实唯一成交数（全史等权胜率与去重前一致）；近窗口样本不足（n<20）档位不画误导曲线、显示空态提示。⚠诚实标注：实盘评级=「当前10日score快照」分档，回测评级=「生成时score」固化，两者时间轴不完全一致。**2026-08-16 标题❓hover 短 + click 弹详版使用指南**（复用 rule-modal，讲清图怎么看/按钮怎么用/口径，对齐技术信号 help 范式）。
...
- **📰 新闻面专门入口 + 独立弹窗（2026-08-16 新增）**：首页 AI 预测卡旁新增「📰 新闻」按钮（与「🤖 AI 预测」并列），点开独立新闻速递弹窗（不挤 AI 预测弹窗），4 项内容：①**当日全量新闻**三源（东财/财联社/金十）当天全部，时间倒序，每条标来源+重要度；②**重要/预告置顶分组**（重要新闻 + 📅 明日预告单独分组置顶）；③**按来源筛选**三个 chip（全部/东财/财联社/金十）只看某源；④**分时/涨幅对照**弹窗顶部迷你分时图（实时拉上证当日分时序列，重要新闻时点打标记，点新闻高亮该时点；休市/分时不可得时诚实降级为「当日大盘 涨/跌 x%」一句静态对照+说明）；当日无 news_digest 时空态提示。数据源 = `data/news_digest.json`（当日）+ `data/news_digest/<date>.json`（归档）
...
- **新闻采完即上线 + 首页外露行可点进（2026-08-16 新增）**：`fetch_news.py` 16:45 采完当日 `news_digest.json` + 归档后**直接同步上线**（copy 到 `static-site/data/` + R2 `data/` 前缀 upload-data-files + staticdata，不再等 20:40 `gen_daily_brief.py` 上传链，16:45→20:40 窗口前端不再读昨日旧闻致「明日关键事件」日期错位）；前端加日期守卫——`news_digest.date` 非当日则「今日要闻」置空不冒充当日、明日预告保留；首页 AI 预测卡下外露两行（今日要闻/明日关键事件）**整行可点进**打开新闻面弹窗（行尾「更多 →」+ hover 高亮），两行都无数据时**兜底渲染「📰 历史新闻入口」行**（点击打开新闻面弹窗，按日期回看归档）
