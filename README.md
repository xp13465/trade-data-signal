...
- **首页走势图卡（「近期技术参考点」下方「汪汪队信号」前方）**：监控策略参数是否「历史拟合好·未来失灵」。两张曲线——①**准确率**（实盘实际 vs 回测预期，近30/60/90交易日切换），准确率口径=信号后方向命中（实盘=信号日收盘→最新收盘方向，看多涨/看空跌，band_hold与未结算不计，与首页「近期技术参考点」汇总条 since_correct 同口径；回测=按卖出模式到期收益方向）②**综合过拟合风险分**（0-100），= 0.40×回测-实盘偏离 + 0.25×滚动样本外衰减 + 0.20×参数稳定 + 0.15×象限退化，**绿<30正常 / 黄30-60关注 / 红>60高风险**（参考线30/60，曲线按值实时变色——绿/黄/红分段）。**2026-08-16 三合一改造③：两图从 echarts 直绘改为首页轻量 SVG 引擎 `_lwSetup` 渲染**（与恐贪/A股情绪分同款外观：绿黄红分段走 `_lwColorFn`、参考线走 markLine、准确率双曲线走多 series），零 echarts.init 首屏提速、支持「走势图渲染」charts.lightweight 开关即时切换。
- **维度切换（2026-08-15 新增，三组按钮两图联动）**：①**窗口** 30/60/90（控总体风险分曲线）②**评级** 全部/高/中/低（按回测 rating 或实盘 signal_stats·10日score 分档）③**类型** 全部/主买/辅买/追买/备买/卖/止损卖（「追买」=买点类型 buy_special，与全站信号叫法统一）。一次点任一按钮 → 准确率 + 风险分两图同时重绘。**卖出类回测仅买入信号**故只显示实盘单曲线（标注「回测仅买入信号」）。⚠**样本去重**：回测同一笔交易按 16 象限×卖出模式重复计数，已按 (模式+信号日+标的+信号) 去重，n 为真实唯一成交数（全史等权胜率与去重前一致）；近窗口样本不足（n<20）档位不画误导曲线、显示空态提示。⚠诚实标注：实盘评级=「当前10日score快照」分档，回测评级=「生成时score」固化，两者时间轴不完全一致。
- **AI降亏过滤开关（2026-08-15 新增，2026-08-16 改默认开）**：勾选后监控**只统计「未被AI宏删线过滤」的信号**（未被 v1.1.0 8键降亏命中 且 已入样 _bt_in_universe），让监控数据同步反映实操过滤后的实际。**默认开启**（首次无本地记忆时开，用户手动切换后写 localStorage `tds_overfit_fade` 记住，手动关=记 0）。后端 `scripts/overfit_monitor.py` 打点时并行生成 **未过滤 / 已过滤（filtered）两套 accuracy+overfit 数据**（version v2），前端开关只切换读取、**不再前端重算**（§23.6 读标记不自算）。过滤判定与首页/凯利区同一 AI宏 5+3+1 口径（基础5+核心3=8键 + 未入样本整类），判词 `signal_ai_filtered` 复刻 `queries._ai_macro_hit_filters` + `_bt_in_universe`。**K 档切换 UI（2026-08-16 三合一改造②新增，当前数据源未支持故 disabled）**：按钮组 K=1/2/3/4 已就位，但后端 `overfit_monitor.json` **尚未生成按 K 档维度的 bank**，故按钮 disabled + tooltip 诚实标注「待数据支持」（§23.6 读标记不自算，不伪造前端 top-K；当前两图按 全信号/删线过滤 两 bank 读取，非 top-K 过滤）。真正的按 K 档监控 bank 属后续另一任务（扩 overfit_monitor.py）。
