...
- **走势图轻量渲染（2026-08-12）**：首页 ~39 个指数/KPI sparkline 从 ECharts 换**轻量 SVG**（外观逐项对等：平滑曲线 + 面积 + 涨跌色 + hover tooltip），消灭首屏 ~39 个 echarts.init 提速 200-500ms；皮肤弹窗「⚡ 走势图渲染」可切回 ECharts 完整版（localStorage 记忆），ETF 评分弹窗近30日走势同开关（P0 已接入）
