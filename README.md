...
- **仓位控制过滤 positionCap + 每日资金池等分（2026-08-12 新增，K 敏感性回测；2026-08-12 起默认开启 K=2 + 3 个降亏推荐 A45/A5/追关注×熊市——用户定"默认的最优组合要开启"，4 个默认推荐 toggle 以金色高亮+⭐ 默认推荐徽标+hover 推荐理由展示，其余 24 个不推荐 toggle 保持普通样式）**：凯利回测区 toggle 支持**每日资金池等分**（默认正确口径——当日 N 个信号每笔=每日金额/N、每日总投资额恒定，解决"一天 100 个信号不可能买 100 次"的不可实现杠杆，最大持仓 1218 万→171.7 万、收益率 32.27%→38.28%）与**仓位控制过滤**（按 signal_date 分组当日基笔信号，组内按 跟踪分→评级→信号类型→买入日 排序保留前 K 个，K 可配置 1-4 默认 2；K=1 收益率 48.88% 三项全优、K=2 更分散抗跌、K≥3 趋同无价值）。交易页当日信号列表联动：前 K 个标「建议执行」、其余「当日已满（仓位控制）」灰显。与降亏同开仅推荐 a45NovMidLateSpecial/a5NovMidSpecial/excludeSpecialBear（默认已开启），绝不同开 live4/COMBO4 全开/greedy 广谱（详见 §21 算法公示与 [`docs/kelly-position-cap-k-sensitivity.md`](docs/kelly-position-cap-k-sensitivity.md)）
