...
### 🌐 全球盘面跑马灯（2026-08-17 新增，纯客户端实时行情）
- **首页 AI 预测卡「今日要闻/明日关键事件」下方一行跑马灯**（金十式横向无缝滚动），9 全球品种：现货黄金 / 现货白银 / WTI原油 / 布伦特油 / 富时A50 / 纳指100 / 美元指数 / 离岸人民币 / 美元日元
- **纯客户端直连第三方、零服务器压力**：所有行情由浏览器端 fetch 实时拉取，不经本项目后端，无 API key、无数据产物、不占服务器资源
- **1主2备数据源**：主=东财 `push2delay` 单只 stock/get（现货全 9）；备1=腾讯 `qt.gtimg.cn` 批量 hf_/wh_/us_（7/9，黄金/白银为 COMEX 期货口径，纳指100 走 us_ 美股口径）；备2=gold-api（XAU/XAG 现货）+ open.er-api（USDCNH/USDJPY 日更）
- **诚实缺口**：富时A50 / 美元指数除东财外无 CORS 备源，东财挂时渲染「暂无」；纳指100 东财+腾讯双源、无备2；降级顺序=东财 → 腾讯 → gold-api/er-api → 全挂显示降级文案，同品种固定单源不混源
- **交互**：红涨绿跌（腾讯/gold-api 备源带 `*`/`†` 口径标记），齿轮 ⚙️ 自定品种显示顺序（存 localStorage），30s 轮询、页面隐藏暂停、请求失败静默降级不弹错
...
- **买卖点信号邮件/飞书带「回测宇宙+AI 过滤+AI 警示+AI 建议」标记（2026-08-17 #61 新增）**：盘中/收盘信号邮件与飞书 post 的每个信号带首页同款三层标记，明确告诉用户"这个信号回测认不认、AI 认不认"，提高可信度——①**未入回测宇宙**（`_bt_in_universe===false`，=首页「未入样本」，历史表现不背书）②**AI 过滤**（统称，两开关正交的删除线过滤层：命中降亏键→「AI降亏·建议回避」徽标现含命中键中文缘由（如「AI降亏·建议回避【追关注×熊市交叉】」，多键用「、」分隔，邮件/飞书两处同文案）；未入样本→同①标注）③**AI 警示**（独立类别，入样卖出信号 sell/sell_stop_loss 的离场保护警示，与 AI 过滤正交）④**AI 建议 top-K**（入样买入非降亏信号按首页 K=1 主推档排序，进 top1=「AI建议1」亮绿强化、其余=「当日已满」灰显）。数据源=读 overview.json 回测侧注入的 `_bt_in_universe`/`ai_macro`（§23.6 禁止自行重算宇宙），AI 建议 top-K 复刻首页 `_posCapSortedFn` 排序（overview etfs track_score + signal_stats score 同数据源），飞书 fade 明细补「建议操作」列与邮件内容对齐（§23.10）。改 `scripts/check_signals.py`（`load_overview_markers`/`build_ai_suggest_map`/`_signal_marker_badge_html` 等），A12 订阅推送同走标记。
...
- **凯利三键改造 + 历史四档轨迹（2026-08-17 #... v1.1.2 版本）**：把现网默认键 **excludeSpecialBear（追关注×熊市交叉）** 的内部判定从「MA60 熊」升级为 **四档市场状态 {牛市·主升 / 上升期 / 下降期 / 熊市·主跌} × buy_special × A股类**（用 hs300 指数价 vs MA200 + MA20/60/120 多空排列判定四档，命中{熊市·主跌,下降期}才剔除），仍是基础5成员、8键数量不变；另加 **2 个默认关备选键**：`legacyMa60Special`（老 MA60 熊×追买，保留原判定，A股类）与 `declinePhaseSpecial`（下降期×buy_special×全市场），备选键默认关、不持久化刷新重置、只影响展示不默认启用（§23.7 只增不改）。同版本新增**历史四档状态轨迹图**（纯展示，不影响过滤）：①指数走势图下叠四档状态色带 ②独立时间线面板 ③时间范围切换（近1年默认/近5年/全史2002起）。判定口径与回测完全同源（§23.6/§22 回测=线上）。**四档文案收窄为仅沪深300（2026-08-18 用户改主意定）**：四档=**沪深300 单一事实源**，色带/时间线面板/chip 仅叠加在 **hs300 指数走势图**上，不扩散到其他 A 股指数（用户原话「既然确定是沪深300算的，就只用沪深300上」）；全站展示文案「大盘四档/大盘状态」统一改「**沪深300四档**」（走势图 tooltip/series/独立面板标题「沪深300四档状态轨迹」/图例/首页 chip「沪深300 ·」），并加「**沪深300 单一指数口径·非综合多指数**」标注防误解；走势图四档色带高度 1/2→1/4（隐藏轴 max:1→max:2，独立时间线面板整条色带不动）。AI 过滤/凯利基准内部大盘档语义（hs300）冻结不改（§5.4/§23.7 只增不改）。
...
### 🤖 每日 AI 速递（deepseek）
- **历史反思校准（自成长，2026-08-17 新增，2026-08-18 严格口径+三档联动）**：每次 AI 预测生成前，系统按**严格口径近30日命中率**分档决定反思注入——**<50% 及格线以下=加强反思**（注入全部失败样本）；**50-75% 及格档=正常**（近3次）；**75-90% 良好=轻量参考**（仅最近一次）；**≥90% 优秀=参考借鉴**（不注入失败样本，改为提示保持近期成功推理模式）。注入样本严格时间隔离（只用本预测日之前已回填的失败样本，防未来函数），样本积累中则暂不注入。有样本时前端 AI 预测详情块显示「🔍 含历史反思校准」（按档位显示加强/正常/轻量/参考成功模式）及本次实际参考的失败样本明细（日期+类型+归因），无样本/兜底版优雅降级不显示。失败样本落盘 `data/daily_brief_reflections.json`，本次注入要点随 `daily_brief.json`/`daily_brief_history.json` 的 `meta.reflection`（含 tier/hit_rate）归档供透明展示（§22 单一数据源，前端所有 AI 预测展示位一致）
- 收盘后自动生成 **daily_brief 白话解读**（deepseek 生成，本地合规 gating），邮件直发
- **首页并排展示**：横幅「🤖 AI 预测」与「📜 历史收盘分析」并排，弹窗内历史列表分页反查（点开某日=预测内容+meta断言+次日实际涨跌+命中标记）+ 近30/90日命中率（meta机检次日回填）；2026-08-15 起为**三层全命中**：预测给出明确方向 + **中间层 7 个全押**（深证成指/创业板指/科创50/北证50/恒生指数/恒生科技涨跌幅% + **10年国债收益率变化基点**）+ 大盘上证与领涨/领跌板块次日涨跌幅区间（宽度≤0.5%、越窄越准），次日实测落进大盘区间 **且中间层 7 个全部命中 且** 所有预测板块全中=✅三层命中（大盘+中间层7+板块）；10年国债命中=（次日 cn10y − 当日 cn10y）×100 落在预测基点区间；改造前老条目无区间/无中间层不伪造，只保留旧"方向相等"判定（✅仅方向命中）；**2026-08-18 起严格口径：仅方向命中（无区间/幅度层可校准）= 未中**（计入分母不计中，同样进失败反思库），命中率按严格口径计：仅当含区间且三层全命中才算"中"（任一层数据缺失整体不硬判）
...
- **盘中沪深300四档「今日预估 vs 昨日实际」对比（2026-08-18 新增）**：首页沪深300四档 chip 盘中(hour<15 且今日=最近交易日)显示「今日:XX(预估) | 昨日:XX」对比格式——今日预估=盘中实时价(快照反哺进 index_daily 当日 close)×昨日均线(当日日线未走完均线不更新)判定，仅盘中参考、未收盘随价变动；昨日=最近已收盘交易日收盘实际档位；收盘后(约 17:00-17:50 update_all)重算为当日实际档位。后端 `_market_state_of` 增 `est_close` 参数输出 `market_state_est` 字段，`market_state`(昨日实际)保持不动(§22 一致/§23.7 只增不改)；前端 `_renderMarketStateChip` 有 `market_state_est` 时渲染对比格式、无则维持「沪深300 · XX」（口径=沪深300 单一指数，非综合多指数）
...
- **📄 金融数据在线速递独立页（2026-08-17 新增，UUMit 链接知识商品交付页）**：新增独立静态页 [`static-site/databrief.html`](static-site/databrief.html)，部署到 `https://ss.fx8.store/databrief.html`，供 UUMit 链接知识商品交付给买家——买家购买后获得 URL+提取码，输入码解锁看内容，页面每日动态 fetch `daily_brief.json`/`overview.json`（数据产物每日更新，实现"持续更新"卖点）。密码解锁（前端 JS 校验，防君子语义，提取码 `FX8-2026`，localStorage 7 天免重复输入）；分区展示：AI 今日预测卡（方向/区间/把握度）、四维解读（风险/资金/情绪/技术面）、风险点/亮点/关注指数、今日速览（恐贪/情绪/跨市场评分 + 大盘预警信号）、详细速递文案、免责声明；深色金融风（#0b1a2e→#132a45 渐变 + 青 #2a9d8f 强调），移动端优先；fetch 失败兜底「数据待更新,请交易日收盘后查看」不白屏。纯新增独立文件，不改任何现有功能（§23.7 冻结契约）；数据只读展示，不重算口径（§22）。
