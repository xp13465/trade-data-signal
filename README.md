...
- ⚠️ **回测 G 口径声明（诚实标注）**：本监控的最优/主展示/预警卖出模式取 **G = 信号驱动卖出全量口径**（A/F/G 三模式信号方向命中率比对），**不含 v1.0.0 基准的「13 万 P≤3d 资本管理」叠加**；G 全量口径峰值持仓 136 万 = 26×本金、不可实际操作（可操作口径=G 加 P≤3d 后峰值持仓≤20×本金，见「凯利」段）。方向上，此处衡量的是**信号方向命中率（可辩护指标，与资金/持仓无关）**，非可操作仓位的净盈亏；看仓位可操作性请以凯利「可操作 G 玩法」为准，勿把本监控的 G 全量数字当作可实际成交的仓位收益。
