...
- **首页 AI 开关 + 信号级降亏标记（2026-08-13 新增）**：首页信号列表标题下第一行新增「AI 降亏过滤总开关 + AI仓位建议 K 档按钮组」——①AI 降亏过滤总开关：与凯利区 AI宏 共享状态（localStorage `tds_kelly_filters`，7 个降亏 toggle 状态持久化，双页实时联动）；后端 overview.json 生成时（app/queries.py）给每条信号注入 `ai_macro: {hit, filters}`（7 个降亏谓词同源凯利回测：追关注×熊市交叉/J1/J2/n2/r7/辅关注×3/5月/Greedy-15，price_bin 依赖子条件信号级不可判定时诚实降级不误标）；总开关开 + 该信号命中降亏条件（且该条件在凯利区实际开启）→ 信号灰显+删除线+「AI降亏」标注（hover 显示命中的降亏条件，建议回避）；②AI仓位建议 K 档按钮组（3124 排序，K=3 默认主推）：直接切换 K 并联动「AI建议/当日已满」灰显（与凯利区/交易页共享 tds_poscap）
