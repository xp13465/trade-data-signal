...
### 🤖 每日 AI 速递（deepseek）
- **历史反思校准（自成长，2026-08-17 新增）**：每次 AI 预测生成前，系统先读历史失败预测样本（方向/区间误判 + 规则级归因）做反思注入，让模型参考既往失误谨慎校准本次判断——不是凭空判断；严格时间隔离（只用本预测日之前已回填的失败样本，防未来函数），样本积累中则暂不注入。有样本时前端 AI 预测详情块显示「🔍 含历史反思校准」及本次实际参考的失败样本明细（日期+类型+归因），无样本/兜底版优雅降级不显示。失败样本落盘 `data/daily_brief_reflections.json`，本次注入要点随 `daily_brief.json`/`daily_brief_history.json` 的 `meta.reflection` 归档供透明展示（§22 单一数据源，前端所有 AI 预测展示位一致）
...
- **理财专员使用指南** `/guide.html`（2026-08-18 新增）：将 [`docs/理财专员使用指南.md`](docs/理财专员使用指南.md) 渲染为静态站内页，面向理财专员讲解如何使用本网站（站点定位/6 大核心板块/每日操作流程/预期收益/风险提示/进阶用法），与「关于」页 `/about.html` 目录 + 首页 footer 互链入口；由 `scripts/gen_guide_html.py` 用 Python-Markdown 从 md 生成（表格/目录锚点），纯静态零 JS（除主题 + baidu 统计）
