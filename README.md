...
### 🎛️ 分析参考点AI监控（过拟合监控，2026-08-15 B档｜2026-08-15 改造）
- **首页走势图卡（「近期技术参考点」下方「汪汪队信号」前方）**：监控策略参数是否「历史拟合好·未来失灵」。两张曲线——①**准确率**（实盘实际 vs 回测预期，近30/60/90交易日切换），准确率口径=信号后方向命中（实盘=信号日收盘→最新收盘方向，看多涨/看空跌，band_hold与未结算不计，与首页「近期技术参考点」汇总条 since_correct 同口径；回测=按卖出模式到期收益方向）②**综合过拟合风险分**（0-100），= 0.40×回测-实盘偏离 + 0.25×滚动样本外衰减 + 0.20×参数稳定 + 0.15×象限退化，**绿<30正常 / 黄30-60关注 / 红>60高风险**（参考线30/60，曲线按值实时变色——绿/黄/红分段）。
...
- **AI降亏过滤开关（2026-08-15 新增，默认关）**：勾选后监控**只统计「未被AI宏删线过滤」的信号**（未被 v1.1.0 8键降亏命中 且 已入样 _bt_in_universe），让监控数据同步反映实操过滤后的实际。后端 `scripts/overfit_monitor.py` 打点时并行生成 **未过滤 / 已过滤（filtered）两套 accuracy+overfit 数据**（version v2），前端开关只切换读取、**不再前端重算**（§23.6 读标记不自算）。过滤判定与首页/凯利区同一 AI宏 5+3+1 口径（基础5+核心3=8键 + 未入样本整类），判词 `signal_ai_filtered` 复刻 `queries._ai_macro_hit_filters` + `_bt_in_universe`。K 档选择器（1最激进★/3最稳健/4最保守/2次稳健）本次后置预留，UI 位置已留。
