...
- **首页并排展示**：横幅「🤖 AI 预测」与「📜 历史收盘分析」并排，弹窗内历史列表分页反查（点开某日=预测内容+meta断言+次日实际涨跌+命中标记）+ 近30/90日命中率（meta机检次日回填）；方向判定容忍带 ±0.5%（|次日涨跌|≤0.5% 判 flat 震荡、>0.5% 判 up、<-0.5% 判 down，模型把握度低时倾向给 flat，2026-08-14 口径 0.1%→0.5% 重刷历史）
