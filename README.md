...
- **首页 AI 预测卡「今日要闻/明日关键事件」下方一行跑马灯**（金十式横向无缝滚动），8 全球品种：现货黄金 / 现货白银 / WTI原油 / 布伦特油 / 富时A50 / 美元指数 / 离岸人民币 / 美元日元
- **纯客户端直连第三方、零服务器压力**：所有行情由浏览器端 fetch 实时拉取，不经本项目后端，无 API key、无数据产物、不占服务器资源
- **1主2备数据源**：主=东财 `push2delay` 单只 stock/get（现货全 8）；备1=腾讯 `qt.gtimg.cn` 批量 hf_/wh_（6/8，黄金/白银为 COMEX 期货口径）；备2=gold-api（XAU/XAG 现货）+ open.er-api（USDCNH/USDJPY 日更）
- **诚实缺口**：富时A50 / 美元指数除东财外无 CORS 备源，东财挂时渲染「暂无」；降级顺序=东财 → 腾讯 → gold-api/er-api → 全挂显示降级文案，同品种固定单源不混源
