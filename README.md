...
- **K2C5 港股追涨 + K3 主关注×概念（2026-08-15 #86 新增实验键）**：凯利回测「降亏标志」区市场组新增两个默认关闭的 toggle（用户方案A，自开关看效果，不持久化、刷新重置默认关、不影响 1.0.0 默认组合稳定性）。K2C5=剔除 `buy_special/buy_backup`×港股（数据验证 948 笔，报告 §6.2 第一优先：剔除后 all A+7,384、y1 双正、能让港股卡 y1 翻正，但对可操作 G 玩法有害——G 的 P≤3d 里当强平缓冲垫，故默认关）；K3=剔除 `buy`×概念（数据验证 4440 笔，y1 提升最大但高波动+牺牲 2024/2025 大赚年，报告建议默认关+监控）。两键均未跑边际每日池回测，比值标「待实测」（§21 公示在 flag tip/advice 列出规则 + 来源报告，不编造数字）
