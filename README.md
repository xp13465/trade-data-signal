...
- **维度切换（2026-08-15 新增，三组按钮两图联动）**：①**窗口** 30/60/90（控总体风险分曲线）②**评级** 全部/高/中/低（按回测 rating 或实盘 signal_stats·10日score 分档）③**类型** 全部/主买/辅买/特买/备买/卖/止损卖。一次点任一按钮 → 准确率 + 风险分两图同时重绘。**卖出类回测仅买入信号**故只显示实盘单曲线（标注「回测仅买入信号」）。⚠**样本去重**：回测同一笔交易按 16 象限×卖出模式重复计数，已按 (模式+信号日+标的+信号) 去重，n 为真实唯一成交数（全史等权胜率与去重前一致）；近窗口样本不足（n<20）档位不画误导曲线、显示空态提示。⚠诚实标注：实盘评级=「当前10日score快照」分档，回测评级=「生成时score」固化，两者时间轴不完全一致。
