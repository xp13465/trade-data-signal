...
- **首页走势图卡（「近期技术参考点」下方「汪汪队信号」前方）**：监控策略参数是否「历史拟合好·未来失灵」。两张曲线——①**准确率**（实盘实际 vs 回测预期，近30/60/90交易日切换），准确率口径=信号后方向命中（实盘=信号日收盘→最新收盘方向，看多涨/看空跌，band_hold与未结算不计，与首页「近期技术参考点」汇总条 since_correct 同口径；回测=按卖出模式到期收益方向）②**综合过拟合风险分**（0-100），= 0.40×回测-实盘偏离 + 0.25×滚动样本外衰减 + 0.20×参数稳定 + 0.15×象限退化，**绿<30正常 / 黄30-60关注 / 红>60高风险**（参考线30/60）。
