...
最近更新：2026-07-07（per-index buy_aux 增强：sw_801140 轻工加 RSI上穿40（方案B，f -25%→-14% 未转正但胜率+5pp 退而求其次）+ sw_801030 基础化工加反弹力度2%（方案A 新 filter `close_above_bl_2pct`，f -21%→+20% 转正三 horizon 一致 n=19 警示）；电力设备维持现状）
...
**当前规则：买=RSI 事件化（C1 主）+ BB下轨回归辅买点（B1 buy_aux，支持 per-index 增强）+ sw_801110/sw_801140 叠加 RSI上穿40 确认（方案B）+ sw_801030 叠加反弹力度2% 确认（方案A）；卖=20日高回落5%（D1 high-based）∧ close>MA60（S1 多头过滤）∧ DIF<DEA（MACD 死叉确认，s.* 情绪分豁免）**（2026-07-05 方案 B 改，见 §7.4 / §7.7 / §9）
...
| 买·辅 per-index 增强（可选） | 配置化 `buy_aux_filter` | `config/indicators.yaml` 给单个指数加 `buy_aux_filter` 字段叠加增强过滤，未配置走基线 B1。当前 3 个品类配置：(1) **sw_801110 家电** = `rsi_cross_40`：`buy_aux &= (rsi_prev≤40)&(rsi>40)`（RSI 上穿40，与 C1 上穿30 对称，价格反弹+动量转升双维确认），f -38%→+16% 转正（见 §7.4）；(2) **sw_801140 轻工** = `rsi_cross_40`（同家电），f -25%→-14% 未转正但胜率 45%→50% 退而求其次（见 §7.4）；(3) **sw_801030 基础化工** = `close_above_bl_2pct`：`buy_aux &= (close > bl×1.02)`（反弹力度确认，过滤 barely-crossed 假信号），f -21%→+20% 转正，三 horizon 一致，n=19<30 样本警示（见 §7.4）。详见 `16-行业buy_aux批量回测-电力设备-轻工-基础化工.md` |
...
- 买点·辅（B1，2026-07-05 加）：`布林下轨回归(下轨{bl:.0f},close{c:.0f}), RSI={r:.0f}, cross={cross:.0f}[标签]`，例 `布林下轨回归(下轨3852,close3870), RSI=41, cross=47[偏冷]`
  - **per-index 增强时加确认段**（在 RSI 段后插）：
    - sw_801110 / sw_801140 配 `rsi_cross_40`：插 `RSI[上穿40]`，例 `布林下轨回归(下轨3852,close3870), RSI=41, RSI[上穿40], cross=47[偏冷]`
