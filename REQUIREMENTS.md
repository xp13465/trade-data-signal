...
最近更新：2026-07-08（per-index buy 主买点 RSI 阈值收紧：kc50 科创50/sw_801730 电力设备/sw_801760 传媒 加 buy_filter=rsi_cross_25（f 显著改善：kc50 15.92%→57.56%、电力设备 0%→29.55%、传媒 0%→41.74%）；其他 57 品类 buy 维持基线 RSI 上穿 30）。上一轮 2026-07-08：per-index buy_aux 增强：cyb 创业板指 + sw_801150 医药生物 加 RSI上穿40（方案B，cyb f -13%→+15% 三 horizon 一致转正 n=20 警示；医药 f -2%→+22% 三 horizon 一致转正 n=36≥30 稳健，方案A 亦转正 f+38% 但 n=16 选 B）；csi500/sz_div/sw_801050/sw_801120 维持现状）
...
**当前规则：买=RSI 事件化（C1 主，per-index 可选收紧 RSI 阈值）+ BB下轨回归辅买点（B1 buy_aux，支持 per-index 增强）+ sw_801110/sw_801140/csi1000/sw_801760/cyb/sw_801150 叠加 RSI上穿40 确认（方案B）+ sw_801030 叠加反弹力度2% 确认（方案A）；卖=20日高回落5%（D1 high-based）∧ close>MA60（S1 多头过滤）∧ DIF<DEA（MACD 死叉确认，s.* 情绪分豁免）**（2026-07-08 方案 B 改，见 §7.4 / §7.7 / §9）
...
| 买触发阈值（C1 主） | 30（上穿），per-index 可选收紧 | `rsi_prev ≤ 30 且 rsi > 30`，前一日在超卖区（含边界）、当日升回 30 之上。**per-index `buy_filter` 可收紧阈值**（如 `rsi_cross_25` = `rsi_prev ≤ 25 且 rsi > 25`，更早捕捉超卖反弹），当前 kc50 科创50 / sw_801730 电力设备 / sw_801760 传媒 配置（见 §7.4） |
| 买触发（B1 辅） | BB 下轨回归 | `bu,mid,bl = _bollinger(close,20,2.0)`；`buy_aux=(close_prev<bl_prev)&(close>bl)`。前一日跌破下轨、当日收回下轨之上 |
| 买·辅 per-index 增强（可选） | 配置化 `buy_aux_filter` | `config/indicators.yaml` 给单个指数加 `buy_aux_filter` 字段叠加增强过滤，未配置走基线 B1。当前 7 个品类配置：(1) **sw_801110 家电** = `rsi_cross_40`：`buy_aux &= (rsi_prev≤40)&(rsi>40)`（RSI 上穿40，与 C1 上穿30 对称，价格反弹+动量转升双维确认），f -38%→+16% 转正（见 §7.4）；(2) **sw_801140 轻工** = `rsi_cross_40`（同家电），f -25%→-14% 未转正但胜率 45%→50% 退而求其次（见 §7.4）；(3) **sw_801030 基础化工** = `close_above_bl_2pct`：`buy_aux &= (close > bl×1.02)`（反弹力度确认，过滤 barely-crossed 假信号），f -21%→+20% 转正，三 horizon 一致，n=19<30 样本警示（见 §7.4）；(4) **csi1000 中证1000** = `rsi_cross_40`（同家电/轻工），f -19.5%→+3.47% 转正，三 horizon 一致（5d/10d/20d 全 f>0），胜率 49.3%→47.4%，盈亏比 0.74→1.20，n 71→19（见 §7.4）；(5) **sw_801760 传媒** = `rsi_cross_40`（同家电/轻工/csi1000），f -14%→+28% 转正，三 horizon 一致（5d/10d/20d 全 f>0），胜率 55%→74%，盈亏比 0.65→0.57，n 69→19（见 §7.4）。(6) **cyb 创业板指** = `rsi_cross_40`（同上），f -13%→+15% 转正，三 horizon 一致（5d/10d/20d 全 f>0），胜率 51%→65%，盈亏比 0.77→0.70，n 92→20（n<30 警示，见 §7.4）。(7) **sw_801150 医药生物** = `rsi_cross_40`（同上），f -2%→+22% 转正，三 horizon 一致，n=36≥30 稳健转正，胜率 48%→50%，盈亏比 1.04→1.80（方案A 亦转正 f+38% 但 n=16<30 选 B，见 §7.4）。详见 `16-行业buy_aux批量回测-电力设备-轻工-基础化工.md` + `17-A股宽基buy_aux回测-sh-csi1000-hs300.md` + `18-行业buy_aux回测-传媒-综合-钢铁.md` + `20-行业buy_aux回测-中证500-创业板指-深证红利-有色金属-食品饮料-医药生物.md` |
...
- **买·主在「超卖结束、升回阈值之上」时点**（C1）：基线 RSI 从 ≤30 升回 >30 那天，标价格有望反弹的拐点；不是 RSI 仍处于超卖区（≤30）的每一天。**per-index `buy_filter` 可收紧阈值**（如 kc50/sw_801730/sw_801760 用 `rsi_cross_25` = RSI 从 ≤25 升回 >25，更早捕捉超卖反弹，详见 §7.4）。
...
- 买点·主（C1，基线 RSI 上穿 30）：`RSI上穿30({rsi_prev:.0f}->{rsi:.0f}),cross={cross:.0f}[标签]`，例 `RSI上穿30(29->34),cross=8[冰点]`
  - **per-index buy_filter 收紧时标注实际阈值**：kc50/sw_801730/sw_801760 配 `rsi_cross_25` → `RSI上穿25({rsi_prev:.0f}->{rsi:.0f}),cross={cross:.0f}[标签]`，例 `RSI上穿25(24->28),cross=44[偏冷]`
...
- **2026-07-08（per-index buy_aux 增强 · cyb 方案B + sw_801150 方案B，当前）**：依据 `20-行业buy_aux回测-中证500-创业板指-深证红利-有色金属-食品饮料-医药生物.md`（csi500/cyb/sz_div/sw_801050/sw_801120/sw_801150 六品类 buy_aux 全史 3 方案 A/B/C 回测，A 股宽基/风格/行业优先；csi500/sz_div/sw_801050/sw_801120 维持基线 B1 不动，cyb/sw_801150 选方案B）。新增 2 个品类 per-index 增强：(1) **cyb 创业板指** 配 `buy_aux_filter: rsi_cross_40`（同 sw_801110/csi1000/sw_801760，复用方案B）。凯利 f **-12.68%→+15.34% 转正**（10d 主），胜率 51.1%→65.0%（+13.9pp），盈亏比 0.77→0.70，n 92→20（-78.3%）；**三 horizon 一致转正**（5d +49.42% / 10d +15.34% / 20d +19.44%，全 f>0）；⚠️ n=20<30 样本警示（与 csi1000 n=19 / 传媒 n=19 同量级）。(2) **sw_801150 医药生物** 配 `buy_aux_filter: rsi_cross_40`（同上）。凯利 f **-2.12%→+22.16% 转正**（10d 主），胜率 48.0%→50.0%（+2.0pp），盈亏比 1.04→1.80（+0.76 大幅提升），n 123→36（-70.7%）；**三 horizon 一致转正**（5d +40.18% / 10d +22.16% / 20d +30.20%，全 f>0）；**n=36≥30 稳健转正**（17 品类里继家电 n=33 后第二个 n≥30 稳健转正）。方案 A（close_above_bl_2pct）亦三 horizon 一致转正（f=+37.70%）但 n=16<30 样本警示，故选 B（n=36≥30 更稳）。**signals.py 零改动**（rsi_cross_40 早已实现，仅 yaml 加 2 行配置）。reason：cyb/sw_801150 buy_aux 在 RSI 段后加 `RSI[上穿40]` 段。**只 cyb/sw_801150 生效**，其他 58 品类 buy_aux 不变（含已有 5 filter 不动）。重算后：buy 3861（不变）/ buy_aux 5314→5155（cyb 92→20 -72、sw_801150 123→36 -87，共 -159，其他不变）/ sell 3268（不变）；signal_daily 总 12443→12284。**csi500/sz_div/sw_801050/sw_801120 维持现状**：csi500 B 最接近 f=-6.51% 仍负；sz_div 全方案深负（A f=-93%）；有色金属 A 最接近 f=-3.51% 仍负；食品饮料 B 最接近 f=-0.95% 仍负。**诚实提醒**：cyb n=20 样本偏小需后续累积验证；医药 n=36≥30 稳健但 buy_aux 仍是辅买点置信度低于 C1 主买。
- **2026-07-08（per-index buy 主买点 RSI 阈值收紧，当前）**：依据 `22-buy收紧RSI回测-21个不建议.md`（21 个凯利不建议品类 C1 主买 RSI 收紧 6 方案回测，方案 B = RSI 上穿 25 替代基线 30）。新增 `buy_filter` 机制（`_load_buy_filters()` + `compute()` 查表 apply），配置化 per-index C1 主买 RSI 阈值收紧。**仅 3 个改善最显著的品类落地**：(1) **kc50 科创50** 配 `buy_filter: rsi_cross_25`（`rsi_prev≤25 & rsi>25`），凯利 f 15.92%→57.56%（+41.64pp），胜率 57.1%→84.6%，盈亏比 1.04→0.57，n 21→13，三 horizon 5d/10d/20d 全 f>0（59.30%/57.56%/45.17%）；(2) **sw_801730 电力设备** 配 `buy_filter: rsi_cross_25`（同上），凯利 f 0%→29.55%（转正），胜率 48.0%→60.9%，盈亏比 0.94→1.25，n 50→23，三 horizon 全转正（55.59%/29.55%/48.46%）；(3) **sw_801760 传媒** 配 `buy_filter: rsi_cross_25`（同上），凯利 f 0%→41.74%（转正），胜率 44.6%→60.9%，盈亏比 1.18→2.05，n 65→23，三 horizon 全转正（54.02%/41.74%/42.68%）。**signals.py 改动**：新增 `_load_buy_filters(cfg)` 函数（与 `_load_buy_aux_filters` 同模式），`compute()` 查表决定 RSI 阈值（25 vs 30），reason 格式 `RSI上穿25(...)` 标注实际阈值。`strategy_desc()` 同步更新（读 `buy_filter` 返回 `"RSI(14)上穿25"`）。**只这 3 品类生效**，其他 57 品类 buy 逻辑零改动（基线 RSI 上穿 30）。buy_aux 和 sell 全品类零改动。重算后：buy 3861→3673（kc50 21→13 -8、sw_801730 50→23 -27、sw_801760 65→23 -42，共 -77，偏差为历史数据随采集更新自然变动，其他品类不变）/ buy_aux 3928（不变）/ sell 3185（不变）。signal_stats.json diff 验证：差异 iids 仅 kc50/sw_801730/sw_801760（buy 变），其他 57 品类 buy + 全部 buy_aux/sell 零改动。**诚实提醒**：三品类 n 均<30（kc50 n=13 / 电力设备 n=23 / 传媒 n=23），样本偏小需后续累积验证；buy 仍是主买点置信度高于 buy_aux，但 RSI 25 比 30 更宽松、信号更敏感（更早入场但假信号风险略增）；RSI 25 并非"越低越好"——回测显示多数品类收紧后凯利反而恶化（即 RSI 30 已经是较好的平衡点），仅这 3 品类例外，不能外推。
