...
最近更新：2026-07-05（per-index buy_aux 增强：sw_801110 家用电器 buy_aux 加 RSI上穿40 确认（方案B），f -38%→+16% 转正，三 horizon 一致；配置化 `buy_aux_filter` 支持逐品类扩展）
...
**当前规则：买=RSI 事件化（C1 主）+ BB下轨回归辅买点（B1 buy_aux，支持 per-index 增强）+ sw_801110 叠加 RSI上穿40 确认（方案B）；卖=20日高回落5%（D1 high-based）∧ close>MA60（S1 多头过滤）∧ DIF<DEA（MACD 死叉确认，s.* 情绪分豁免）**（2026-07-05 方案 B 改，见 §7.4 / §7.7 / §9）
...
| 买·辅 per-index 增强（可选） | 配置化 `buy_aux_filter` | `config/indicators.yaml` 给单个指数加 `buy_aux_filter` 字段叠加增强过滤，未配置走基线 B1。当前仅 sw_801110 = `rsi_cross_40`：`buy_aux &= (rsi_prev≤40)&(rsi>40)`（RSI 上穿40，与 C1 上穿30 对称，价格反弹+动量转升双维确认）。回测 f -38%→+16% 转正（见 §7.4）。详见 `14-家电buy_aux优化回测.md` |
...
- 买点·辅（B1，2026-07-05 加）：`布林下轨回归(下轨{bl:.0f},close{c:.0f}), RSI={r:.0f}, cross={cross:.0f}[标签]`，例 `布林下轨回归(下轨3852,close3870), RSI=41, cross=47[偏冷]`
  - **per-index 增强时加确认段**（sw_801110 配 `rsi_cross_40`）：在 RSI 段后插 `RSI[上穿40]`，例 `布林下轨回归(下轨3852,close3870), RSI=41, RSI[上穿40], cross=47[偏冷]`
