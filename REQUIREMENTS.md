...
最近更新：2026-07-08（per-index buy_aux 增强：sw_801760 传媒 加 RSI上穿40（方案B，f -14%→+28% 三 horizon 一致转正，胜率 55%→74%，盈亏比 0.65→0.57，n 69→19）；综合 sw_801230 / 钢铁 sw_801040 维持现状（无方案 10d 转正））。上一轮 2026-07-07：csi1000 中证1000 加 RSI上穿40（方案B，f -19.5%→+3.47% 三 horizon 一致转正，胜率 49.3%→47.4%，盈亏比 0.74→1.20，n 71→19，第二个三 horizon 一致转正的宽基品类）；sw_801140 轻工加 RSI上穿40（方案B，f -25%→-14% 未转正但胜率+5pp 退而求其次）+ sw_801030 基础化工加反弹力度2%（方案A 新 filter `close_above_bl_2pct`，f -21%→+20% 转正三 horizon 一致 n=19 警示）；电力设备维持现状）
...
**当前规则：买=RSI 事件化（C1 主）+ BB下轨回归辅买点（B1 buy_aux，支持 per-index 增强）+ sw_801110/sw_801140/csi1000/sw_801760 叠加 RSI上穿40 确认（方案B）+ sw_801030 叠加反弹力度2% 确认（方案A）；卖=20日高回落5%（D1 high-based）∧ close>MA60（S1 多头过滤）∧ DIF<DEA（MACD 死叉确认，s.* 情绪分豁免）**（2026-07-08 方案 B 改，见 §7.4 / §7.7 / §9）
...
- **卖点**（D1 + S1 + MACD 死叉确认）= close 从近 20 日最高价（**high-based**，必须用 high 不用 close）**回落 5%**（前一日 close ≥ 阈 且当日 close < 阈，阈 = 近 20 日 high 之 max × 0.95）**且 close > MA60**（60 日多头趋势才放卖，砍下跌趋势假卖点）**且 DIF<DEA**（MACD 死叉态，动量转弱确认，过滤强趋势回调假摔）。signal='sell'。**s.\* 情绪分序列豁免 MACD 过滤**（a_sentiment 加 MACD 后 n=106→7 样本不足），保留原 D1+S1。
...
实现见 `app/compute/signals.py` 的 `compute()` + `_compute_value_signals()` + `_cross_tag()` + `_bollinger()` + `_macd()`，high 由 `app/compute/normalize.py::load_index_high()` 加载。指数（close）与全球指标/情绪分（g.*/s.* value）均应用 B1+S1；MACD 死叉确认仅指数 + g.* 应用（s.* 豁免）；a_sentiment 仍 skip_buy（RSI 结构性失效，buy 与 buy_aux 都跳过，仅算 sell）。
...
| 买·辅 per-index 增强（可选） | 配置化 `buy_aux_filter` | `config/indicators.yaml` 给单个指数加 `buy_aux_filter` 字段叠加增强过滤，未配置走基线 B1。当前 5 个品类配置：(1) **sw_801110 家电** = `rsi_cross_40`：`buy_aux &= (rsi_prev≤40)&(rsi>40)`（RSI 上穿40，与 C1 上穿30 对称，价格反弹+动量转升双维确认），f -38%→+16% 转正（见 §7.4）；(2) **sw_801140 轻工** = `rsi_cross_40`（同家电），f -25%→-14% 未转正但胜率 45%→50% 退而求其次（见 §7.4）；(3) **sw_801030 基础化工** = `close_above_bl_2pct`：`buy_aux &= (close > bl×1.02)`（反弹力度确认，过滤 barely-crossed 假信号），f -21%→+20% 转正，三 horizon 一致，n=19<30 样本警示（见 §7.4）；(4) **csi1000 中证1000** = `rsi_cross_40`（同家电/轻工），f -19.5%→+3.47% 转正，三 horizon 一致（5d/10d/20d 全 f>0），胜率 49.3%→47.4%，盈亏比 0.74→1.20，n 71→19（见 §7.4）；(5) **sw_801760 传媒** = `rsi_cross_40`（同家电/轻工/csi1000），f -14%→+28% 转正，三 horizon 一致（5d/10d/20d 全 f>0），胜率 55%→74%，盈亏比 0.65→0.57，n 69→19（见 §7.4）。详见 `16-行业buy_aux批量回测-电力设备-轻工-基础化工.md` + `17-A股宽基buy_aux回测-sh-csi1000-hs300.md` + `18-行业buy_aux回测-传媒-综合-钢铁.md` |
...
| 卖确认（MACD 死叉） | DIF < DEA | `dif, dea = _macd(close)`；`sell = sell & (dif < dea).fillna(False)`。MACD(12,26,9) 业界标准参数，DIF<DEA=死叉态（动量转弱），过滤强趋势回调假摔。**s.\* 情绪分序列豁免**（保留 D1+S1）。回测建议率 18.3%→43.3% |
...
- 买点·辅（B1，2026-07-05 加）：`布林下轨回归(下轨{bl:.0f},close{c:.0f}), RSI={r:.0f}, cross={cross:.0f}[标签]`，例 `布林下轨回归(下轨3852,close3870), RSI=41, cross=47[偏冷]`
  - **per-index 增强时加确认段**（在 RSI 段后插）：
    - sw_801110 / sw_801140 / csi1000 配 `rsi_cross_40`：插 `RSI[上穿40]`，例 `布林下轨回归(下轨3852,close3870), RSI=41, RSI[上穿40], cross=47[偏冷]`
    - sw_801030 配 `close_above_bl_2pct`：插 `反弹[2%]`，例 `布林下轨回归(下轨4553,close4674), RSI=39, 反弹[2%], cross=70[偏热]`
- 卖点（D1+S1+MACD）：`20日高回落5%(高{hh:.0f}->阈{thresh:.0f},close{c:.0f}), RSI={r:.0f}, cross={cross:.0f}[标签], MA60={m:.0f}[趋势过滤], MACD=DIF{d}/DEA{e}[死叉确认], vs前买{±pct:.2f}%[分类]`，例 `20日高回落5%(高4259->阈4046,close4028), RSI=40, cross=53[中性], MA60=4000[趋势过滤], MACD=DIF39/DEA62[死叉确认], vs前买+2.30%[止盈]`。**s.\* 情绪分序列豁免 MACD**，reason 无 MACD 段（保留 D1+S1 格式）。
...
- MACD 段（DIF/DEA）在 s.* 情绪分序列省略（豁免）；其余品类（指数 + g.*）sell 信号恒有值（DIF<DEA 才放卖）。
...
- **2026-07-05（方案 B MACD 死叉确认，当前）**：依据 `13-卖点逻辑优化回测.md`（60 品类 sell 全量回测，3 方案 A/B/C 对比）。D1+S1 卖点本性接近随机（指数向上漂移，10 日走弱概率≈50.6%），47 个凯利不建议。**方案 B**：D1+S1 基础上加 MACD 死叉确认——`dif, dea = _macd(close)`（MACD(12,26,9) 业界标准：DIF=EMA(close,12)-EMA(close,26)，DEA=EMA(DIF,9)，EMA 用 `ewm(span=N, adjust=False)`）；`sell = sell & (dif < dea).fillna(False)`（DIF<DEA=死叉态，动量转弱确认）。金融依据：D1 单维（仅价格回撤）易被回调假突破触发，MACD 死叉确认动量已转弱 = 价格弱 + 动量弱双维确认，经典双重过滤，过滤「强趋势中回调假摔」型假信号。MACD 参数（12/26/9）业界标准非调参，DIF<DEA 标准死叉定义，过拟合风险低。新增 `_macd()` 辅助函数（与 `a-stock-data/backtest_sell_optimize.py::macd()` 一致）。**s.\* 情绪分序列豁免 MACD 过滤**（a_sentiment 加 MACD 后 n=106→7 样本不足，cross_market 同理），保留原 D1+S1；指数 + g.* 指标应用 MACD。reason：非 s.* 的 sell 加 `MACD=DIF{d}/DEA{e}[死叉确认]` 段（s.* 无此段）。买点 C1 + buy_aux B1 + B 标注（vs前买）逻辑零改动。重算后：buy 3861（不变）/ buy_aux 5782（不变）/ sell 3268（vs 4700，MACD 砍 30%）；sell 凯利建议 11→26（建议率 18.3%→43.3%），不建议 47→29，样本不足 2→5（g.a_qvix_300/1000 + sw_801970 被 MACD 砍到 n<10，s.\* 豁免保住 a_sentiment n=106 强建议 f=52.88%）。
- **2026-07-05（per-index buy_aux 增强 · sw_801110 RSI上穿40 方案B，当前）**：依据 `14-家电buy_aux优化回测.md`（sw_801110 家用电器 buy_aux 全史 3 方案 A/B/C 回测）。**配置化 per-index 增强**：`config/indicators.yaml` 给单个指数加 `buy_aux_filter` 字段即可叠加增强过滤（未配置走基线 B1，支持后续逐品类扩展——32 个 buy_aux 品类会逐个验证后加各自方案）。当前仅 **sw_801110 家用电器**配置 `buy_aux_filter: rsi_cross_40` = BB下轨回归 ∧ RSI(14) 上穿40（`rsi_prev≤40 & rsi>40`，与 C1 上穿30 对称，价格反弹+动量转升双维确认，与 sell MACD 死叉确认正交对称）。新增 `_load_buy_aux_filters(cfg)` 读配置返回 `{index_id: filter_name}`，`compute()` 与 `_compute_value_signals()` 均查表 apply（g.*/s.* 预留扩展位，当前无配置）。reason：sw_801110 的 buy_aux 在 RSI 段后加 `RSI[上穿40]` 段。**只 sw_801110 生效**，其他 59 品类 buy_aux 逻辑零改动。回测 sw_801110 buy_aux（10d 主）：凯利 f **-38.5%→+16.2% 转正**，胜率 44.8%→54.5%（+9.7pp），盈亏比 0.66→1.19，均值 -1.23%→+0.54%，n 134→33（-75.6%）；三 horizon 一致转正（5d +19.1% / 10d +16.2% / 20d +17.1%），稳健非偶发。重算后：buy 3861（不变）/ buy_aux 5782→5680（sw_801110 135→33，-102，其他品类不变）/ sell 3268（不变）。signal_stats.json diff 验证：仅 sw_801110 buy_aux 改变，其他 59 品类 buy_aux + 全部 buy/sell 零改动。**诚实提醒**：f>0 = 正期望（建议仓位约 16%，仍属轻仓），buy_aux 仍是辅买点置信度低于 C1 主买，单品类打样结论不能直接外推其他品类，方案 B 是否为 buy_aux 通用优化方向需多品类验证后确认。
- **2026-07-07（per-index buy_aux 增强 · sw_801140 方案B + sw_801030 方案A）**：依据 `16-行业buy_aux批量回测-电力设备-轻工-基础化工.md`（sw_801730/sw_801140/sw_801030 三品类 buy_aux 全史 3 方案 A/B/C 回测，电力设备维持现状不动）。新增 2 个品类 per-index 增强：(1) **sw_801140 轻工制造** 配 `buy_aux_filter: rsi_cross_40`（同 sw_801110 家电，复用方案B）——退而求其次，凯利 f **-24.96%→-14.12% 未转正**（仍负，用户接受「提高胜率」为先），胜率 45.0%→50.0%（+5.0pp），盈亏比 0.79→0.78，均值 -1.04%→-0.56%，n 160→38（-76.4%）；三 horizon 不一致（5d +0.20% / 10d -14.12% / 20d -0.52%，5d/20d 接近 0 但 10d 仍偏负）。(2) **sw_801030 基础化工** 配 `buy_aux_filter: close_above_bl_2pct` = BB下轨回归 ∧ close > bl×1.02（反弹力度确认，过滤 barely-crossed 假信号，dead cat bounce 前半段）——**新增 filter 类型**，`signals.py::compute()` 与 `_compute_value_signals()` 加 `close_above_bl_2pct` 分支实现（`buy_aux &= (close > bl_*1.02)`，B1 基线已含 close>bl 故叠加 close>bl×1.02 即可）。凯利 f **-20.87%→+20.43% 转正**，胜率 42.5%→57.9%（+15.4pp），盈亏比 0.91→1.12，均值 -0.81%→+1.26%，n 160→19（-88.1%）；**三 horizon 一致转正**（5d +23.77% / 10d +20.43% / 20d +26.94%，全 f>0），稳健非偶发；⚠️ **n=19<30 样本不足警示**，结论仅供参考可能过拟合，但三 horizon 一致 + 与电力设备（f=+20.85% n=10）横向印证方案A 在周期/制造类有共性，是最稳健选择。reason：sw_801140 buy_aux 加 `RSI[上穿40]`（同家电）；sw_801030 buy_aux 加 `反弹[2%]`。**只这 2 品类生效**，其他 58 品类 buy_aux 不变（含 sw_801110 已有 rsi_cross_40 不动）。重算后：buy 3861（不变）/ buy_aux 5680→5416（sw_801140 161→38 -123、sw_801030 160→19 -141，共 -264，其他品类不变）/ sell 3268（不变）。signal_stats.json diff 验证：差异 iids 仅 sw_801140 / sw_801030（sw_801110 未受影响 f=+16.2% 维持，其他 57 品类 buy_aux + 全部 buy/sell 零改动）。**诚实提醒**：轻工 f 未转正是预期（用户接受退而求其次换胜率+5pp），buy_aux 仍是辅买点置信度低于 C1 主买；基础化工 n=19 样本偏小，f>0 = 正期望（建议仓位约 20%，仍属轻仓），需后续样本累积验证非过拟合。
- **2026-07-07（per-index buy_aux 增强 · csi1000 方案B，当前）**：依据 `17-A股宽基buy_aux回测-sh-csi1000-hs300.md`（sh/csi1000/hs300 三宽基 buy_aux 全史 3 方案 A/B/C 回测，sh/hs300 维持基线 B1 不动，仅 csi1000 选方案B）。新增 1 个品类 per-index 增强：**csi1000 中证1000** 配 `buy_aux_filter: rsi_cross_40`（同 sw_801110 家电 / sw_801140 轻工，复用方案B）= BB下轨回归 ∧ RSI(14) 上穿40（`rsi_prev≤40 & rsi>40`）。**signals.py 零改动**（rsi_cross_40 分支早在 sw_801110 时已实现，仅 yaml 加一行配置）。凯利 f **-19.5%→+3.47% 转正**（10d 主），胜率 49.3%→47.4%（-1.9pp 微降但盈亏比补），盈亏比 0.74→1.20（+0.46 大幅提升），均值 -0.77%→+0.15%，n 71→19（-73.2%）；**三 horizon 一致转正**（5d f=+20.91% / 10d f=+3.47% / 20d f=+21.98%，全 f>0），8 品类里继家电后第二个三 horizon 一致转正的稳健品类（轻工/基础化工之外的新增）。reason：csi1000 buy_aux 在 RSI 段后加 `RSI[上穿40]` 段（同家电/轻工）。**只 csi1000 生效**，其他 59 品类 buy_aux 不变（含 sw_801110/sw_801140/sw_801030 已有 filter 不动）。重算后：buy 3861（不变）/ buy_aux 5416→5364（csi1000 71→19 -52，其他品类不变）/ sell 3268（不变）；signal_daily 总 12545→12493。signal_stats.json diff 验证：差异 iids 仅 csi1000（csi1000 buy/sell 未变只 buy_aux 变；sw_801110/sw_801140/sw_801030 未受影响 f 维持，其他 56 品类 buy_aux + 全部 buy/sell 零改动）。**诚实提醒**：f>0 = 正期望（建议仓位约 3.5%，仍属轻仓），buy_aux 仍是辅买点置信度低于 C1 主买；n=19 样本偏小（与基础化工同），三 horizon 一致转正增强稳健性信心但需后续样本累积验证非过拟合；10d f=+3.47% 量级低于 5d/20d（+20% 量级），horizon 间量级波动提示信号对持有期敏感。


- **2026-07-08（per-index buy_aux 增强 · sw_801760 方案B，当前）**：依据 `18-行业buy_aux回测-传媒-综合-钢铁.md`（sw_801760 传媒 / sw_801230 综合 / sw_801040 钢铁 三品类 buy_aux 全史 3 方案 A/B/C 回测，综合/钢铁维持基线 B1 不动，仅 sw_801760 选方案B）。新增 1 个品类 per-index 增强：**sw_801760 传媒** 配 `buy_aux_filter: rsi_cross_40`（同 sw_801110 家电 / sw_801140 轻工 / csi1000，复用方案B）= BB下轨回归 ∧ RSI(14) 上穿40（`rsi_prev≤40 & rsi>40`）。**signals.py 零改动**（rsi_cross_40 分支早在 sw_801110 时已实现，仅 yaml 加一行配置）。凯利 f **-14.03%→+27.66% 转正**（10d 主），胜率 55.1%→73.7%（+18.6pp），盈亏比 0.65→0.57，均值 -0.42%→+0.59%，n 69→19（-72.5%）；**三 horizon 一致转正**（5d f=+33.97% / 10d f=+27.66% / 20d f=+2.78%，全 f>0）。方案 A（close_above_bl_2pct）亦三 horizon 一致转正（f=+74.20%）但 n=8<10 样本不足，故选 B（n=19≥10 更稳，与 csi1000/基础化工 同一 n=19 量级）。reason：sw_801760 buy_aux 在 RSI 段后加 `RSI[上穿40]` 段（同家电/轻工/csi1000）。**只 sw_801760 生效**，其他 59 品类 buy_aux 不变（含 sw_801110/sw_801140/sw_801030/csi1000 已有 filter 不动）。重算后：buy 3861（不变）/ buy_aux 5364→5314（sw_801760 69→19 -50，其他品类不变）/ sell 3268（不变）；signal_daily 总 12493→12443。signal_stats.json diff 验证：差异 iids 仅 sw_801760（sw_801760 buy/sell 未变只 buy_aux 变；其他 59 品类 buy_aux + 全部 buy/sell 零改动）。**综合 sw_801230 / 钢铁 sw_801040 维持现状**：综合无方案 10d 转正（B 最接近 f=-2.89% 但仍负且三 horizon 不一致）；钢铁仅 C（放量1.2倍）10d 转正（f=+10.99%）但三 horizon 不一致（20d f=-12.75%）+ C 非 signals.py 已实现 filter + n=17<30，不达落地标准。**诚实提醒**：f>0 = 正期望（建议仓位约 28%，但 n=19 样本偏小实际应保守），buy_aux 仍是辅买点置信度低于 C1 主买；20d f=+2.78% 量级偏低（胜率 31.6% 靠盈亏比 2.38 撑正），horizon 间量级波动提示信号对持有期敏感，需后续样本累积验证非过拟合。
264	### 7.5 与上一版（C1）差异对比
...
- **2026-07-05（per-index buy_aux 增强 · sw_801110 RSI上穿40 方案B）**：依据 `14-家电buy_aux优化回测.md`（sw_801110 家用电器 buy_aux 全史 3 方案 A/B/C 回测，用户选方案 B）。**配置化 per-index 增强**：`config/indicators.yaml` 给单个指数加 `buy_aux_filter` 字段即可叠加增强过滤（未配置走基线 B1，支持后续逐品类扩展——32 个 buy_aux 品类会逐个验证后加各自方案）。`app/compute/signals.py` 新增 `_load_buy_aux_filters(cfg)` 读配置返回 `{index_id: filter_name}`，`compute()` 与 `_compute_value_signals()` 均查表 apply（g.*/s.* 预留扩展位）。当前仅 **sw_801110 家用电器**配置 `buy_aux_filter: rsi_cross_40` = BB下轨回归 ∧ RSI(14) 上穿40（`rsi_prev≤40 & rsi>40`，与 C1 上穿30 对称，价格反弹+动量转升双维确认，与 sell MACD 死叉确认正交对称）。reason：sw_801110 的 buy_aux 在 RSI 段后加 `RSI[上穿40]` 段。**只 sw_801110 生效**，其他 59 品类 buy_aux 逻辑零改动。回测 sw_801110 buy_aux（10d 主）：凯利 f **-38.5%→+16.2% 转正**，胜率 44.8%→54.5%（+9.7pp），盈亏比 0.66→1.19，均值 -1.23%→+0.54%，n 134→33（-75.6%）；三 horizon 一致转正（5d +19.1% / 10d +16.2% / 20d +17.1%），稳健非偶发。重算后：buy 3861（不变）/ buy_aux 5782→5680（sw_801110 135→33，-102，其他品类不变）/ sell 3268（不变）。signal_stats.json diff 验证：仅 sw_801110 buy_aux 改变，其他 59 品类 buy_aux + 全部 buy/sell 零改动。REQUIREMENTS §7 更新（§7 概述 / §7.1 参数表加 per-index 增强 / §7.3 reason 加 RSI[上穿40] / §7.4 变更历史）。验收：py_compile + node --check 通过。**诚实提醒**：f>0 = 正期望（建议仓位约 16%，仍属轻仓），buy_aux 仍是辅买点置信度低于 C1 主买，单品类打样结论不能直接外推其他品类，方案 B 是否为 buy_aux 通用优化方向需多品类验证后确认。
- **2026-07-07（per-index buy_aux 增强 · sw_801140 方案B + sw_801030 方案A）**：依据 `16-行业buy_aux批量回测-电力设备-轻工-基础化工.md`（sw_801730/sw_801140/sw_801030 三品类 buy_aux 全史 3 方案 A/B/C 回测，电力设备维持现状不动）。新增 2 个品类 per-index 增强：(1) **sw_801140 轻工制造** 配 `buy_aux_filter: rsi_cross_40`（同 sw_801110 家电，复用方案B）——退而求其次，凯利 f **-24.96%→-14.12% 未转正**（仍负，用户接受「提高胜率」为先），胜率 45.0%→50.0%（+5.0pp），盈亏比 0.79→0.78，均值 -1.04%→-0.56%，n 160→38（-76.4%）；三 horizon 不一致（5d +0.20% / 10d -14.12% / 20d -0.52%）。(2) **sw_801030 基础化工** 配 `buy_aux_filter: close_above_bl_2pct` = BB下轨回归 ∧ close > bl×1.02（反弹力度确认，过滤 barely-crossed 假信号）——**新增 filter 类型**，`signals.py::compute()` 与 `_compute_value_signals()` 加 `close_above_bl_2pct` 分支实现（`buy_aux &= (close > bl_*1.02)`）。凯利 f **-20.87%→+20.43% 转正**，胜率 42.5%→57.9%（+15.4pp），盈亏比 0.91→1.12，均值 -0.81%→+1.26%，n 160→19（-88.1%）；**三 horizon 一致转正**（5d +23.77% / 10d +20.43% / 20d +26.94%），稳健非偶发；⚠️ n=19<30 样本不足警示，但三 horizon 一致 + 与电力设备方案A（f=+20.85% n=10）横向印证，是最稳健选择。reason：sw_801140 buy_aux 加 `RSI[上穿40]`（同家电）；sw_801030 buy_aux 加 `反弹[2%]`。**只这 2 品类生效**，其他 58 品类 buy_aux 不变（含 sw_801110 已有 rsi_cross_40 不动）。重算后：buy 3861（不变）/ buy_aux 5680→5416（sw_801140 161→38 -123、sw_801030 160→19 -141，共 -264，其他品类不变）/ sell 3268（不变）。signal_stats.json diff 验证：差异 iids 仅 sw_801140 / sw_801030（sw_801110 未受影响 f=+16.2% 维持，其他 57 品类零改动）。REQUIREMENTS §7 更新（§7 概述 / §7.1 参数表 / §7.3 reason 加反弹[2%] / §7.4 变更历史）。验收：py_compile + node --check 通过。
- **2026-07-07（per-index buy_aux 增强 · csi1000 方案B）**：依据 `17-A股宽基buy_aux回测-sh-csi1000-hs300.md`（sh/csi1000/hs300 三宽基 buy_aux 全史 3 方案 A/B/C 回测，sh/hs300 维持基线 B1 不动，仅 csi1000 选方案B）。新增 1 个品类 per-index 增强：**csi1000 中证1000** 配 `buy_aux_filter: rsi_cross_40`（同 sw_801110 家电 / sw_801140 轻工，复用方案B）。**signals.py 零改动**（rsi_cross_40 分支早在 sw_801110 时已实现，仅 yaml 加一行配置）。凯利 f **-19.5%→+3.47% 转正**（10d 主），胜率 49.3%→47.4%（-1.9pp），盈亏比 0.74→1.20（+0.46），均值 -0.77%→+0.15%，n 71→19（-73.2%）；**三 horizon 一致转正**（5d +20.91% / 10d +3.47% / 20d +21.98%，全 f>0），8 品类里继家电后第二个三 horizon 一致转正的稳健品类。reason：csi1000 buy_aux 加 `RSI[上穿40]` 段（同家电/轻工）。**只 csi1000 生效**，其他 59 品类 buy_aux 不变。重算后：buy 3861（不变）/ buy_aux 5416→5364（csi1000 71→19 -52，其他不变）/ sell 3268（不变）；signal_daily 总 12545→12493。signal_stats.json diff 验证：差异 iids 仅 csi1000（buy/sell 未变只 buy_aux 变；sw_801110/sw_801140/sw_801030 未受影响 f 维持，其他 56 品类零改动）。REQUIREMENTS §7 更新（§7 概述 / §7.1 参数表 3→4 品类 / §7.3 reason / §7.4 变更历史）。验收：py_compile + yaml 解析 + node --check 通过，diff 验证仅 csi1000 改动。
