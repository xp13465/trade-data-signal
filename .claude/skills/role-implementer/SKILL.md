...
- **上线前必填数据就绪清单(缺一=不上线)**:
  ...
  4. **同构对账互证(数据含回测/复现数字时)**:经 §3.2 同构对账机检(§5.4⑦)确认与页面一致,残缺口径数字不进生产
...
> 蒸馏来源:开源项目 DietrichGebert/ponytail(~118k star)主 skill 的「7 级阶梯」「根因修复」「少写抽象」三条,用户拍板蒸馏而非直接装 plugin。与 Karpathy 的 Simplicity/Surgical 高度重叠(主控已复核),此处以 ponytail 为主落点。**关联规范源**:CLAUDE.md §23.2(修 bug 三铁律)/L11(不加需求外改动,见 §7 过错清单)/§5(默认准则「以工作量为衡量偷懒」);根 CLAUDE.md §5.1 已被主控补「开源评估跨角色穷举」。改了对应源头时反向同步本节。

**核心一句话:写码/改码前自问「能否不写/少写」,先理解问题再爬阶梯;修 bug 修根因不修症状;删除优先于添加。与现有「修完整+举一反三+不加需求外改动」是同一精神的两个面——本节是「写之前省」,§5/§6 是「改之时查全」。**

- **7 级阶梯(写任何代码/加任何逻辑前,自上而下停在第 1 个成立的层,不跳过前面的层)**:
  1. **需要存在吗** → 不需要=YAGNI 跳过,不做(接 L11「不加需求外改动」,先复述需求确认要不要)
  ...
  - **强调:先理解问题再爬阶梯**。不跳过"读现有代码/读需求"直接开写——看不懂问题就写,容易在第 2 层误判"库里没有"而重写(§5.1 认知有限→先调研/查现有,不闭门重造)
...
- **少写抽象(不写"可能用到"的抽象层)**:不写单实现的 interface、单产品的 factory、永不变化的 config;抽象只有在出现第二个真实使用者时才提取,不提前造。**删除优先于添加**——改代码先想"能不能删掉/简化现有"而不是"再加一层"(接 L11「不加需求外改动」,§5.3「精简保核心」)
- **验收口径**:实施任务自验须含「写码前过 7 级阶梯自问(第几层成立/为何不再低层)+ 复用点清单(库/数据源/组件已有复用)+ 无多余抽象确认」;修 bug 任务须含「根因修复点(单点守卫 vs 逐 caller)+ 每个 caller 影响面已 grep」。reviewer 查阶梯复用覆盖+无过度抽象,漏=验收不过
...
- **⑤ 简洁输出·结论优先(2026-09-17 用户定,Concise 语义落地)**:完成报告/每步汇报**先给结论**(第一句=结果/发生了什么),跳过开场白和结尾复述,短默认为主。**但结论后必须带证据点**——改了哪些文件(路径/行号)、自测逐项结果、口径标注(非基准须声明)三点是验收命门,**简洁砍的是废话,不砍证据**;主控要验收时按 §5/§6.5 自验清单逐项给得出。
