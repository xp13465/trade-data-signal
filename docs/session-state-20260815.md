# 会话状态快照(2026-08-15 17:20,持续更新)

> 目的:主控会话 P0-1 落地(clear/compact 频繁化)后,**clear 前必落档关键状态**。本文件 = 2026-08-15 会话当前全部关键状态,clear/compact 后靠它恢复。用户定(2026-08-15):「省 token 是要素,但是丢东西就得不偿失」+「clear 和 compact 由主控主动做,因为你能确保做之前都落档」。

## 会话瘦身机制(用户定:主控主动做,已落档 memory persist-before-clear-compact)
- **由主控主动判断时机 + 落档 + 提醒**:主控无法直接敲 /clear,落档完提醒用户执行,或等系统 auto-compact
- **每次 clear/compact 前必落档**:本文件(在跑agent/待办/决策/待上线)+ 新规范进 memory
- **判定标准(调研落档)**:compact=任务中黄区50~75%/子阶段切换(趁缓存热,先落档);clear=任务彻底收尾/开新任务/纠正同一问题≥2次(必落档+commit);主控多agent会话不适合频繁clear(丢agent id/进度/派单),用"子任务间compact+大任务收尾开新会话";别等auto-compact(最差时机)
- **当前建议**:等 #90 修复 agent 回来后,所有记录任务齐 → 落档最终状态 + 提醒用户 /clear 测试方案(或先 /compact 若上下文尚可)
