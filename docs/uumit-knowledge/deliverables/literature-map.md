...
- **定价建议来源**:`GET https://api.uumit.com/api/v1/pricing/suggestion`(knowledge/one_time,2026-08-17 实测返回建议 150 UT/区间 100-500/同类 53,description 不同建议值在 100-500 区间内浮动)+ 定价调研(调研区间 30-80 UT 或 0 引流)。**用户拍板 0 UT 引流定位**,非平台建议价;但平台 publish 校验拒绝 0 UT,实际定价待用户确认(见「定价定位」段上架实测)。
