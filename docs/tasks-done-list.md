...
## 2026-09-07 会话移入(#29 双视角引导完成)
- [x] **#29 页面展示方法论:近1年看A/长期看H 引导展示位(完成上线 2026-09-07)** — 调研 docs/lab/strategy-view-guide-research-20260906.md(4 落点排查,权威口径 A=29.62% 全模式第1/y1、H=230.83% 全周期第1 +115,416 兜底态 V2)→ 实施 feat/strategy-view-guide-29(b633aeccf+ebf37b1ae):16 处 H 数字 224.92→230.83 全站对齐(§22)+ 首页 help 双形态(hoverpop `_sigHelpPopHtml` + 弹窗 `_openRefHelpModal`)各加「📐 按投资周期选」小结卡 + 凯利区 `_kellyComboAdviceHtml` 同款卡 + purpose-notes.js §21 公示同步 + README §23.1(grep 224.92=0/230.83=19/29.62=4,node --check 三文件全过)→ reviewer PASS(7 维,唯一提示=凯利区全角 vs app.js 半角标点纯排版)→ main-merge 统一 bump **a548**(d08cb34c4)→ deploy(04:18 退出码 0)→ §0 三查 PASS(三域名 app.min.js 含双视角+H230.83,lab.min.js 含卡)。⚠️ 低危遗留:deploy R2 部分大 JSON 类别(8 个 -all.json 等)best-effort 上传失败保留旧版,已记录待后续补跑,不影响本功能。→ 巡检 cron 7e468318/adbc3966 已删
...
## 2026-09-11 会话移入(#105/#106/#107 完成)
- [x] **#105 实操步骤增强(完成上线 2026-09-11)** — PRD §6 阶段一补全(用户 09-10 澄清=非新需求直接做)。implementer→reviewer 首轮 FAIL(P0 seq5 缺 sell_date 结构化键+P1 主表摘要最高seq误判)→修复 3a04cca38→复审 PASS→main-merge a7b31bdee(版本串 20260911-a578)。四项落地:①生成器全 seq 链(seq1 09:15挂单/seq2 09:25竞价判定/seq3 14:55兜底/seq5 D+10卖出,sell_date=信号日后第10交易日)+历史 seq1 迁移 ②前端时间派生推进 ③「✓我已操作」localStorage 持久化 ④盘中现价腾讯 qt.gtimg.cn 60s 轮询+点 ETF 走势弹窗。§0 三查 PASS。
- [x] **#106 实操步骤表回填历史持仓卖出行(完成 merge 2026-09-11,数据 20:55 定时首跑后生效)** — 用户拍板「回填历史持仓」。生成器新增回填窗口 BACKFILL_WINDOW_DAYS(信号日+10交易日补登未到期卖出,幂等 date+seq1);主链抽 _build_plan_for_day 共用;R2 上传判断改 steps_changed 修空计划日漏传。commit 3fa8c62d4。P2 跟单:回填行「历史回填」角标(lab.js 主表+弹窗)+ sw_行业补点 close round(4) 对齐,commit 5fd1d956f,main-merge 2fcad37f1(版本串 20260911-a580)。
- [x] **#107 首页历史信号走势图盘中缺当日点修复(完成上线 2026-09-11)** — 用户两次反馈「历史信号走势图看不到当日点」。#107 A+B:生成侧盘中补采 signal 宇宙腾讯可采 48 指数(intraday_snapshot _backfill_signal_universe_tencent,开关 INTRADAY_SIGNAL_UNIVERSE_PATCH)+ 展示层 sw_/thsc_ 补点+「该指数盘中暂无当日实时价」滞后提示。commit fdc3b497f,main-merge 593c74f38(版本串 20260911-a579)。§0 三查 PASS(线上 app.min.js 含滞后提示串)。
