...
4. **下沉检查**(2026-09-12 补,根 CLAUDE.md 精简改造机制层兜底):每周 review 时顺带核对根 CLAUDE.md 是否重新膨胀——凡「长期稳定、不会过时的规范正文」应下沉到 skill/归档(全文),根文件只留「触发词+核心一句话+指针」摘要(移动不删除、先建后删);发现根文件有新增稳定铁律正文未下沉,当场下沉到对应归档/skill 并同步指针,防精简后重新长回去。
5. **总结复核 agent**(独立 fresh context,总结完成后派,会话级/每日/每周总结都派,2026-08-08 用户定):git log 可查总结前后差异,复核 agent 读总结前原文(git show 旧版 CLAUDE.md §18 / memory 旧版)+ 总结后新版,逐条检查:①核心点保留(核心条款/根因/场景未删,删过时事实不删教训;经验条目也保留)②中心思想未跑偏(§18="不重犯同类+经验积累成长",防重犯条款具体可执行非泛泛)③非为总结而总结(归纳非删减非凑数,有实质提炼)④非为省token省token(不该删的不删,节省是副产品非目标)⑤经验类已归纳(好经验/方法论/架构洞察有落档,不只归纳过错)。发现问题->回退(git revert)/修正重做。即"总结自查+复核两层"兜底,是"总结核心点保留"原则的执行保障,git log 让复核可验
...
### 主控专属过错(16 条,含 1 条指针)
- **L01 通知丢失不设cron傻等**:派发后必设 cron 兜底,不傻等(→§11)。**⚠️ 第 2 次复发(2026-08-25):compact 后裸派 3 agent 零兜底,#11 死亡半天无人知**——根因=条款只在 governance(compact 后不注入)+恢复清单第6步靠"想起来读",均无机械强制。根治=三件套提升共享核心 §0.2 + PostToolUse(Agent) hook 机械提醒(settings.json+scripts/agent_dispatch_cron_reminder.py),条款升级为机制不再靠记忆 | archive:L13
- **L15 追加A级小改漏做**:追加任务不靠 SendMessage,commit 前主动确认追加任务是否处理(→§11) | archive:L50
- **L17 §0证伪查错文件**:§0 证伪前 grep 前端渲染逻辑确认读哪个文件(→§8 三查) | archive:L52
- **L20 §0 grep字面量漏常量**:验"值/配置"类结论 grep 字面量无结果先疑封装成常量,改 grep 常量名+查赋值行 | archive:L65
- **L21 reviewer卡死无进度文件**:prompt 必写"每步 echo 进度文件"(→§11) | archive:L66
- **L23 prompt期望数值错误**:prompt 期望值先核实来源(字段语义/页面实际),不凭印象 | archive:L68
...
- **L29 主控亲自Edit业务代码干活违反"只派发"**:已全文化至根 CLAUDE.md §0.1「主控角色红线:只调度,不亲自实施」共享核心,此处不重复维护(防双份分叉);归档反查 docs/archive/CLAUDE-errors-2026-08.md 锚点 L29 | 根 CLAUDE.md §0.1 + archive:L95
- **L30 需求叫停时过度删除同链路中间档**:用户叫停某功能/改口径时先复述"删到什么粒度/保留什么档"确认再实施,不把同链路可保留中间档一起误删(K=3 33万异常才暴露);删除/重构前列"删除清单+保留清单"逐项确认,改后自验关键展示值不等用户报异常 | archive:L96
- **L31 派数据重跑/回测任务基准过时**:派单前先核对当前页面 `_kellyDefaultFilters()`/`_kellyComboPresets` 真值(默认键集/组合宏),不沿用旧报告基准;基准参数写进 prompt 时标注来源=当前页面某行核验,便于 §0 复核 | archive:L97
- **L32 "A/F收益率虚高"标注方向错误**:判断实操意义先看峰值持仓/单次本金倍数(≤20倍可操作,A/F 持仓10-15万可操作非虚高,G/H/I 136倍本金不可操作),"虚高"标签只用于口径放大类(如每笔1万虚假杠杆),不从模式快慢预设 | archive:L98
- **L33 子agent设"等定时器自唤醒"不可靠**:子agent会话一停 REPL 没了定时器永不触发→双等待死锁;带时点动作主控侧设一次性 CronCreate 兜底,或收到"在等定时器"报告立即改为命令立即执行;主控兜底设在自己会话 | archive:L103
...
- **E25 口径/基准切换先派影响面审计**:会反转/数值变化/自愈三类分类逐项评估,并行修根因再全面修正受影响功能(fixed→每日池曾审出默认推荐/默认K档 2 项高风险反转);适用任何资金口径/默认基准切换前,不同步会导致多数展示位继续按旧口径出数(同 §22 一致性) | archive:L102
- **E26 发现派单基准过时立即 SendMessage 同步在跑 agent**:派重跑/回测任务后核对当前页面真值发现基准过时,第一时间同步在跑 agent 修正(本次 STEP 1/6 早段修正未全量浪费),不留到跑完返工,越早越好(配合派单前先核真值,防重于治) | archive:L103
...
## 18.2 主控调度视角:团队协作(§23.4/23.5 主控侧,2026-08-12 用户定)
> 用户原话:"开发一个任务时是否会考虑到已经落档的其他调研报告出了但是没开发完成的功能?这应该也是一个要求。毕竟多个子agent应该是一个团队""如果互相都考量到了。真碰到开发时。冲突问题应该也可以进一步得到缓解。否则2个任务对同一个模块对态度存在对立时。大概率是后者覆盖前者了。但其实这也不太对。然后反而还要碰到问题后找我确认。其实这一开始就应该暴露问题出来"。主控=调度中枢,负责让"团队"不打架、待办不对账不脱节。实施侧全文见 implementer skill §8,reviewer 检查要点见 reviewer skill §6。
- **派任务前查索引**:派任何开发/方案任务前,对照 `docs/pending-features-index.md`(已落档未开发功能索引),当前任务相关模块有未开发功能→prompt 里显式要求"预留接口/位置/常量位"或提示后续合并,避免返工
- **同模块占用检查**:派任务前查后台 agent 清单+同模块是否有其他任务在改(§11 CronList+在跑 agent),发现同模块多任务→直接调度:排队/约定共用约定/分区域,不让"后覆盖前"发生;不闷头并行等碰壁才协调
- **冲突暴露前置**:agent 报告/进度里显式提"此模块另有任务X在改"时,主控立即协调(§23.5 一开始就暴露,不等到 merge 撞车)
- **待办对账提醒**:派待办前核对方案假设 vs 当前代码/数据现状(项目演进可能使待办失效),失效先让 agent 更新方案(§5 调研)再实施
- **索引维护**:新调研报告落档"方案待开发"→同步更新 pending-features-index.md;功能开发完成→移除/标完成。索引即团队共享地图,主控负责维护
- **索引定期自动同步(2026-08-12 用户定,原话"应该是定期自动同步的。否则这个快照就会慢慢过时 达不到索引的效果")**:每日 23:47 cron(id c7f3132f)自动跑——扫当日 docs/TASKS 改动+任务列表状态,增补新待办/更新状态(完成移入【已排除清单】)/刷新头部日期戳,commit+push main。⚠️ **cron 7 天自动过期**:过期后主控在每日总结/周 memory review 时(§19)重建该 cron(同 cron prompt),防索引快照过时
