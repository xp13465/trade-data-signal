...
> 反向追踪:文件末尾附「防重犯锚点索引」块(L01-L29 过错锚点 + E01-E22 经验锚点,每行含归档行号),根 CLAUDE.md §18 索引表逐行对锚点 id,`grep -c '^L[0-9]'` == 29 = 零丢失校验锚。
> 归档日期:2026-08-12
...
> 零丢失校验锚:`grep -c '^L[0-9]' docs/archive/CLAUDE-errors-2026-08.md` 应 == 根 CLAUDE.md §18 过错索引表行数(29),一致=零丢失。
> 注意:本文件行号会随未来编辑漂移,追到后若行号对不上按锚点 id 关键词 grep 定位。

### 过错锚点(29 条,与根索引表 #1-29 一一对应)
