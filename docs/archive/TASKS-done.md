...
### 四、pending-features-index.md 治理关闭记录(2026-08-22,2 条废弃留档)

> 背景:docs/todolist-cleanup-20260822.md 全量盘查(researcher 三重交叉核实)+用户逐条拍板。此 2 条为**废弃**(非完成):前提不成立/价值不足,从 todolist 移出留档;**前提成立可重新立项**。§5.3 核心保留:标题+废弃理由+重启条件。

- [x] **#85 板块轮动(105 行方案)** — **废弃**:board_concept.db 为空库无任何表(sqlite_master 查询为空),板块行情采集链路从未建成,原前提「待板块历史数据攒够(6-7 个月)再实施」无从攒起不成立;static-site/data 仅 industry-*-concepts.json 概念映射非历史行情。**重启条件**:若仍想要,先立项建板块行情数据采集(新工程),老前提描述作废。
- [x] **#87 PWA 体验增强(150 行)** — **废弃**:App Shell CacheFirst+CACHE_VERSION 破缓存+SW 更新壳芯配套安全网均已就绪(sw.js §24 体系),实时行情站离线价值低,150 行增益项属锦上添花,与 2026-08-21 废弃批次「稳定系统不画蛇添足」同精神(PWA 三件套本身已上线不受影响)。**重启条件**:若未来离线场景需求明确(PWA 安装率上升/弱网用户反馈),再评估立项。
