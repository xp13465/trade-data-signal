# TASKS.md 历史叙述归档（2026-08-20）

> **来源**：TASKS.md（2026-08-20 历史叙述瘦身，手动精确归档）。
> **归档对象**：📍当前会话状态块旧滚动交接轮次（08-16~07-25 已完成工作日志）+ 各 level2 历史章节（07-21~08-16 旧需求/交接状态/已完成项，绝大多数已完成/过期/搁置）。
> **归档方式**：实施 agent 按块类型（`## ` 章节 vs 滚动交接段）逐块核对无活跃 `- [ ]` 待办后归档，非 tasks_archive.py 自动、非行号一刀切（避 memory[[archive-dispatch-by-block-type]] 教训）。原文区块逐字保留于本文件，可 git show 溯源、可反向查 TASKS.md 修改前 `git show HEAD~1:TASKS.md`。
> **对照表/自测/复现**：见 docs/tasks-narrative-slim-20260820.md。
> **活跃待办零丢失**：归档前 22 条活跃 `- [ ]`（L77 + 08-04 三章 611-632/655-662/689/690/694）全部核对保留在 TASKS.md。

---

## 归档批次 20260820（TASKS.md 历史叙述瘦身,手动精确归档）

> 本文件归档 TASKS.md「📍当前会话状态」块旧滚动交接轮次 + 各 level2 历史章节（2026-07-21 ~ 2026-08-16 的旧需求/交接状态/已完成项，绝大多数已完成/过期/搁置）。由实施 agent 按块类型手动精确归档（2026-08-20），非 tasks_archive.py 自动。主文件 TASKS.md 只保留最新 1-2 轮交接 + 大纲 + 工作约定 + 活跃待办（含 08-04 三章）+ 保留待办块。**每个归档章节原文完整保留于此可反查**（§5.3 核心保障）。注：本归档不含活跃 `- [ ]` 待办（活跃待办已核对全部保留在 TASKS.md，见报告 docs/tasks-narrative-slim-20260820.md 对照表）。

---

# 第一部分：📍当前会话状态块 旧滚动交接轮次（08-16 ~ 07-25，已完成工作日志）

**最后更新**:2026-08-16 12:49(**三条 P2 小观察已处理登记**——①README 宽基指数名:核对 overview.json `sentiment_*` 实测 6 个(上证50/沪深300/中证500/中证1000/创业板/科创50),README L87 "6 个宽基"数字=名单=代码一致,**原本就对,未改**;②死 CSS:lab.css 删 21 个全零引用死 class,lab.min.css 93,472→89,648B(-4.1%),commit aed34ba69 + merge 46c606b48,版本串 a279→a280,三域名 ?v=20260816-a280 验证一致(删除项线上 0 命中/假阳性 pc-lvl-* 保留仍在);③data-no-pop:overfit 监控卡 K 按钮无 title/data-tip、无 [title]/[data-tip] 祖先、无自有 hoverpop,_initTermPop 不捕获,**不需要加,未加**)。……【本行原 635 字符超长，已压缩为摘要（保留最新状态）；完整历史见 git log -- TASKS.md 或 NOTES §48】
...
# 第二部分：level2 历史章节（07-21 ~ 08-16 旧需求/交接状态/已完成项）
