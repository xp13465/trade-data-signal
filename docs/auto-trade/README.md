...
| nextday-plan-backend-implement-20260910.md | 阶段一实施报告(后端 v0): nextday_plan 计划生成器 + auto_trade_steps 状态机落档; 复用 kelly_posrating K=1 同构逻辑、双校验、R2/两树同步、22:30 launchd、复现段(⚠ v0 从 trades 选当日信号有设计缺口, 已由方案A根治) | 2026-09-10 v0 已实施(干跑), 2026-09-10 晚被方案A取代 |
...
PRD 阶段一后端产物的生产入口在 `scripts/nextday_plan_generator.py`(每日 22:30 launchd
