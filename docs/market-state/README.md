...
- **情绪日历**:`sentiment-signal-freeze-merge-research.md` — #19 首页近90日情绪日历(冰点+信号按 date 合并)研究/schema 提案;`sentiment-calendar-data-live.md` — **数据生效记录(2026-08-20)**:deploy 后 overview.json 带 sentiment_calendar 字段(27 天,冻结+信号结构,无 low_alert 污染),前端正式渲染;`emotion-popup-pin-tooltip-impl.md` — **#26 情绪分弹窗冰点 pin 恒显 + hover tooltip 增强实施(2026-08-20)**:s.* 弹窗冰点恒显(读 is_freeze=1 优先)+ tooltip 对齐指数曲线带标题("07-17 科创50 情绪分 15.82 / ─ 科创50指数:1715.40"),综合类带"情绪分"、全球extras 不误伤,仿真见 `scripts/emotion-*-sim.mjs`
