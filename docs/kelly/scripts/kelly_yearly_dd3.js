// ============================================================
// 用途: 2011 回撤 14.01% 复算(最终版): 峰值→谷底最大单段跌幅
// 日期/来源: 2026-08-14 / tmp
// 结论: 2011 A模式年内先涨后跌, 峰值→谷底最大单段回撤 14.01%
// 依赖: 无
// 输入/输出: 读 signal_kelly_trades.json(绝对路径), 输出 2011 峰值/谷底/最大回撤
// 复现: node kelly_yearly_dd3.js
// 注意: 本文件为该系列最终版(dd/dd2 为中间演进版); 含硬编码绝对路径
// ============================================================
const fs = require('fs');
...
function yearlyAll(mk){
  ...
  for(const yr in ym){const v=ym[yr];const dd=maxDD(v._trades),pcc=maxConcCap(v._trades);
    v.peak_dd_pct=pcc>0?Math.round(dd.abs/pcc*100*10000)/10000:0;
    v.inv_dd_pct=v.totalInv>0?Math.round(dd.abs/v.totalInv*100*10000)/10000:0;
    v.dd_abs=dd.abs;v.peak_capital=pcc;
    v.peak_return_pct=pcc>0?Math.round(v.profit/pcc*100*10000)/10000:0;
    delete v._trades;}
  return ym;
}
for(const mk of ["A","G"]){
  const ym=yearlyAll(mk);
  console.log(`\n==== ${mk} 模式 · etf_def(ETF默认 万3最低5) · K=1 · AI宏7键 按年 ====`);
  console.log("年份 | 净盈亏(元) | 峰值持仓(元) | 峰值资金收益率% | 峰值资金回撤%(=回撤/峰值持仓) | 回撤/总投入% | 回撤绝对额(元)");
  for(const yr of Object.keys(ym).sort()){const v=ym[yr];
    console.log(`${yr} | ${Math.round(v.profit)} | ${Math.round(v.peak_capital)} | ${v.peak_return_pct.toFixed(2)} | ${v.peak_dd_pct.toFixed(2)} | ${v.inv_dd_pct.toFixed(2)} | ${Math.round(v.dd_abs)}`);}
}
