...
## 〇、数据版本与基笔数说明(2026-09-20 reviewer 勘误后补)

**基笔数随数据版本动态增长**,同一逻辑不同数据时点 n_base 不同,三组已核实:

| 数据版本 | 来源 | 主档 n_base | sdc n_base |
|---|---|---|---|
| 9-13 05:10 | static-site/data 展示档(git 跟踪的线上产物,本地) | 7,608 | 7,612 |
| 9-19 12:27/12:28 | trade-data 实时主库(本地最新 update_all 产物,本文档体积/对账实测所用) | 7,624 | 7,758 |
| 9-20 盘后 | 云上 update_all(ssh 实测) | 7,648 | 7,767 |

- 本文档 §一 字段实证、§三 对账行数与 schema 示例 `n_base: 7608` 均基于 9-13 展示档;实施报告中出现的 **7624/7758 = 9-19 trade-data 主库档**(trade-data/static-site/data/signal_kelly_trades{,_sdc}.json,generated_at 2026-09-19 12:27/12:28,distinct base 实测 7624/7758)。三者均为合法数据版本,差异属「基笔随交易日动态增长」的正常漂移,非口径变化。
- 主档基笔集 ⊂ sdc(§五 双套口径),但两档各随各自数据时点独立增长,三组数字不共用。
...
| 三表 unique | 5,892,709 (5.6 MB) | 1,272,071 |
| 压缩率 | **13.3x** | **61.6x** |

- 实测口径:`_build_unique_tables` 三表 + `json.dumps(ensure_ascii=False, separators=(",", ":"))` 序列化落盘字节(与生成端 L2279 同参数),gz 用 gzip.open 压缩(实测 1,272,071,与 reviewer 独立生成同值;±24B 为 gzip 头元数据差异,量级以 ~1.27MB 为准)。
- 勘误说明:早期稿件写 5,246,852 B / 14.9x,与该口径实测(5,892,709 B / 13.3x)不符(差 646 KB,疑序列化口径差异),reviewer 复核发现,以本表为准。
- **体积随数据版本浮动**:基笔随交易日增长,体积随之浮动。9-19 主档(7,624 笔)实测 unique = 5,906,116 B(13.3x);9-13 主档(7,608 笔)= 5,892,709 B。
- 比方案预估(15 MB)更好:多数卖出字段为短值/空值,紧凑序列化后实际 ~5.6 MB。
- 兜底语义(§24 教训):原 75MB 全量兜底将由 unique 全量 ~5.6MB 替代,「分片失败回退全量」保留。
