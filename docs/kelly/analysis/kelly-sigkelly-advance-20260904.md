...
>
> ⚠️ **#109 口径修正(2026-09-11,现网以此为准)**:本文档方案阶段的突变告警口径「滚动窗 mean±3std 或 单日 Δ>20pp 且 n≥20 且连 2 日同向」已在实施中修正为**相对口径**——`(std_jump and pp_jump) or (pp_jump and dir_confirmed)`,其中 pp_jump=单日｜Δ｜>max(窗口均值×MUTATION_PCT(0.05), ABS_FLOOR_DELTA(200 元, 当窗口均值<ABS_FLOOR_MEAN=500 元))、dir_confirmed=前一日同向｜Δ｜>窗口均值×MUTATION_DIR_PCT(0.01);多 mode 命中合并单条发送。下文 L68/L123/L140 的「Δ>20pp」「连 2 日同向」为方案阶段旧描述,已不反映现网实现。
