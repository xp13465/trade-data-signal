...
服务器建两个目录:代码仓 `/home/ubuntu/code/trade-data-signal`(GIT_REPO,git 仓)+ 数据目录 `/home/ubuntu/code/trade-data`(REPO=MAIN_REPO,非 git)。数据目录顶层 6 软链 app/config/scripts/web/docs/a-stock-data 指回代码仓,static-site 内部 git 源文件逐个软链回代码仓、data/ 为真实目录;.venv 用软链(不 mv+sed,回滚零风险)。本机双仓结构在服务器上 1:1 复刻。路径映射:

| 本机路径 | 服务器路径 |
|---|---|
| `/Users/linhuichen/code/trade-data`(REPO) | `/home/ubuntu/code/trade-data` |
| `/Users/linhuichen/code/trade`(GIT_REPO) | `/home/ubuntu/code/trade-data-signal` |
| `/Users/linhuichen/code/trade-data/.venv/bin/python` | `/home/ubuntu/code/trade-data/.venv/bin/python` |
...
inventory §6.4 写「PURGE_SECRET 逐项迁移(backfill-evening/etf-national-team/etf-track-index 三任务)」——**实测 PURGE_SECRET 实际嵌在 24 个 plist 里**,不只 3 个。本节已统一改为对应 .service 的 `EnvironmentFile=/home/ubuntu/code/trade-data/.env`(值从服务器 .env 读,不进 git)。24 个任务清单:
...
PURGE_SECRET 值(本机全部 plist 一致):见 `/home/ubuntu/code/trade-data/.env`(scp 合并 21 键,值不进 git;此处不落明文)。
> ⚠️ 这是敏感凭证。本文件已统一改为 `EnvironmentFile=/home/ubuntu/code/trade-data/.env`(systemd 从服务器 .env 读,值不进 git),不再落明文。
...
| REPO | /Users/linhuichen/code/trade-data | /home/ubuntu/code/trade-data |
| MAIN_REPO | /Users/linhuichen/code/trade-data | /home/ubuntu/code/trade-data |
...
- 脚本:`fapi_daily_syn.sh`(源路径在 trade/scripts,单仓后同 /home/ubuntu/code/trade-data/scripts)| 时点:18:10 | ExitTimeOut=600
...
Environment=PATH=/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin
TimeoutStartSec=300
StandardOutput=append:/home/ubuntu/code/trade-data/data/logs/turnover_backfill_launchd.log
StandardError=append:/home/ubuntu/code/trade-data/data/logs/turnover_backfill_launchd.err
```
...
Environment=PATH=/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin
TimeoutStartSec=600
StandardOutput=append:/home/ubuntu/code/trade-data/data/logs/nextday_plan_launchd.log
StandardError=append:/home/ubuntu/code/trade-data/data/logs/nextday_plan_launchd.err
```
...
Environment=PATH=/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin
TimeoutStartSec=600
StandardOutput=append:/home/ubuntu/code/trade-data/data/logs/s06_snapshot_launchd.log
StandardError=append:/home/ubuntu/code/trade-data/data/logs/s06_snapshot_launchd.err
```
...
Environment=REPO=/home/ubuntu/code/trade-data
Environment=MAIN_REPO=/home/ubuntu/code/trade-data
ExecStart=/bin/bash /home/ubuntu/code/trade-data/scripts/check_data_gap_alerts.sh
Environment=PATH=/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin
TimeoutStartSec=600
StandardOutput=append:/home/ubuntu/code/trade-data/data/logs/check_data_gap_launchd.out
StandardError=append:/home/ubuntu/code/trade-data/data/logs/check_data_gap_launchd.err
```
...
Environment=REPO=/home/ubuntu/code/trade-data
Environment=MAIN_REPO=/home/ubuntu/code/trade-data
ExecStart=/home/ubuntu/code/trade-data/.venv/bin/python /home/ubuntu/code/trade-data/scripts/daily_summary_email.py --mode supplement
Environment=PATH=/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin
TimeoutStartSec=600
StandardOutput=append:/home/ubuntu/code/trade-data/data/logs/daily_summary_supplement_launchd.log
StandardError=append:/home/ubuntu/code/trade-data/data/logs/daily_summary_supplement_launchd.err
```
...
Environment=PATH=/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin
TimeoutStartSec=900
StandardOutput=append:/home/ubuntu/code/trade-data/data/logs/overfit_monitor_launchd.log
StandardError=append:/home/ubuntu/code/trade-data/data/logs/overfit_monitor_launchd.err
```
...
Environment=REPO=/home/ubuntu/code/trade-data
Environment=MAIN_REPO=/home/ubuntu/code/trade-data
ExecStart=/bin/bash /home/ubuntu/code/trade-data/scripts/schedule_monitor.sh
Environment=PATH=/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin
EnvironmentFile=/home/ubuntu/code/trade-data/.env
TimeoutStartSec=600
StandardOutput=append:/home/ubuntu/code/trade-data/data/logs/schedule_monitor_launchd.log
...
- 参数:`REPO=/home/ubuntu/code/trade-data`(Environment 注入,一行不改脚本默认值);`RETAIN_DAYS=7`(脚本 L23 默认 14 → 注入 7,改后 7 天约 2.1G)。
...
5. **PURGE_SECRET 明文**:§1.6 已纠正覆盖 24 任务,统一改为 `EnvironmentFile=/home/ubuntu/code/trade-data/.env`(值不进 git);勿推公开仓库。
