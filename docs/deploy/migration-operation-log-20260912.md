...
9. **云上→github 限速 33-50KB/s 且不稳定**(SSH 22 端口已是云上最快通道;HTTPS 直连 3.2KB/s / ghfast 镜像 2.4KB/s / github.com:443 超时 / ssh.github.com:443 更慢;实测 24min 断线失败)。clone 与未来每天 push staticdata 开源数据都受此影响。**已验证可行替代=本机中转**(本机 shallow clone 出 HEAD→rsync 传 .git pack 298MB,583KB/s 快 10 倍→云 checkout),clone 已用此路成功;未来每天 push staticdata 可评估同样走本机中转或配代理(checklist §7)
...
| 19:40 | clone staticdata | 云上 SSH 直连浅 clone 限速 33-50KB/s 且 24min 断线失败;改**本机中转**成功(浅 clone→rsync .git pack 298MB/583KB/s→云 checkout,2.0G,HEAD=93fe1e15) |
...
- [x] staticdata clone(改**本机中转**:原云上直连 github SSH 24min 断线失败且清进度;本机浅 clone 出 HEAD 快照→rsync 传 .git pack 298MB(583KB/s,快10倍)→云上 checkout,2.0G 浅 clone,HEAD=93fe1e15 同步 github main 最新,工作树 clean)
