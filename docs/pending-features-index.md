...
| # | 功能 | 出处 | 方案摘要 | 依赖/前置 | 状态 |
|---|---|---|---|---|---|
| 3 | **daily_brief P1-1 周期定位/钟摆位置模板**(恐贪分位+极端提示) | docs/daily-brief-optimization.md §3 P1-1 | trend 段加周期定位模板:恐贪/情绪/新高新低在历史分位,极端值逆向提示 | 需 30 日内 summary_history 支持 | **已作废**(09-06 用户拍板砍掉 AI 预测,89e6d07fb) |
| 4 | **daily_brief P1-3 公募基金持仓/行业配置注入**(中期风格参考) | docs/daily-brief-optimization.md §3 P1-3 | 注入 public_fund_summary 加仓/减仓行业 top 到【趋势研判/中期】,标注季报滞后 | 数据已有(public_fund_summary) | **已作废**(09-06 用户拍板砍掉 AI 预测,89e6d07fb) |
| 5 | **daily_brief P1-4 明日关注排序分**(胜率×凯利×一致性×近期确认) | docs/daily-brief-optimization.md §3 P1-4 | 用站点回测数据把 AI 关注列表从"模型猜"变"数据排序" | signal_kelly_backtest + signal_stats 已有 | **已作废**(09-06 用户拍板砍掉 AI 预测,89e6d07fb) |
| 6 | **daily_brief P1-5 日历效应/节假日/月末季末提示** | docs/daily-brief-optimization.md §3 P1-5 | 注入「明日是否月末/季末/长假前/财报季」,仅提示性段落 | 硬编码节假日表,成本低 | **已作废**(09-06 用户拍板砍掉 AI 预测,89e6d07fb) |
| 8 | **多角色阶段三:事件/新闻面分析师** | docs/ai-predict-multiagent-plan.md §3.2 阶段三 + 数据源实测[`docs/ai-predict-news-macro-research-sources.md`](docs/ai-predict-news-macro-research-sources.md) | ④事件/新闻面分析师,输入当日快讯摘要 | 需事件面数据采集;注入侧 2026-08-16 已把 news 分布进 risk 域(fetch_news 采集已就绪) | **已作废**(09-06 用户拍板砍掉 AI 预测,89e6d07fb) |
| 9 | **daily_brief P1-11 reasoner(R1)深度辩论增强** | docs/daily-brief-optimization.md §3 P1-11 | 研究员角色可切 deepseek-reasoner 深度对抗,成本贵 3-5 倍 | 配置已支持(cfg.researcher_model,gen_daily_brief.py L188),默认 deepseek-chat | **已作废**(09-06 用户拍板砍掉 AI 预测,89e6d07fb) |
...
| # | 功能 | 出处 | 方案摘要 | 依赖/前置 | 状态 |
|---|---|---|---|---|---|
| 10 | **ETF 弹窗 30 天外长历史(需求2)** | docs/chart-refactor-config-plan.md 需求2(行29) + docs/chart-p2p3-data-source-research.md P2 | ETF 侧缺:etf_score_list.json 只近 30 日 ohlc,需新数据产物 etf/{code}-all.json(数据源 fund_etf_hist_sina 等)+ 弹窗 period tab | **数据源待调研(阻塞项)**;P1 chartLite 组件 | **已完成**(08-22 fa1ca6e3b) |
| 11 | **场外基金净值走势 + 弹窗历史 + 指标介绍(需求3)** | docs/chart-refactor-config-plan.md 需求3(行30) + chart-p2p3 P3 | 场外基金 tab 无任何走势图;需 fund_nav/{code}.json 导出(fund_daily_nav 2153 万行已有)+ 行点击详情弹窗 | **依赖 fund_basic 字段补齐**(pf-fund-screener-real-requirements,TASKS L1213-1214) | **已完成**(08-25 a4fc234d4) |
...
| 13 | **SVG 轻量版低优先级 fidelity 差异** | docs/home-svg-lite-fidelity-check.md L130(hideOverlap 未实现) + home-svg-fix-review.md P2-1~P2-4 | hideOverlap / zoom 后 tooltip 残留 / tooltip 底部 clamp / slider preventDefault 等小项 | 无 | **已完成**(08-25 9cefef692,feat f7d16b007+4f742101a) |
...
| 15 | **凯利回测「次日开盘」口径(✅ 已完成,2026-09-12 核销)** | docs/kelly/position/kelly-nextday-open-backtest.md + TASKS.md L55 | 建议①后续回测/前端展示默认改「次日开盘」口径(收盘固化、次日开盘买入,数据 100% 覆盖)②操作建议:开盘挂 -1% 限价单 | 无 | **已完成**(同项 #91 已核销:后端默认次日开盘 v1.1.4 + 前端「买入口径」双档按钮 2026-09-06 上线,详见 #91) |
...
| # | 功能 | 出处 | 方案摘要 | 依赖/前置 | 状态 |
|---|---|---|---|---|---|
| 26 | **R2 P1:board_etf_map 与 overview 同步(刷新后自动触发重算)+ track_score 百分位基线固定化** | docs/r2-migration-implementation-report.md §3.2/§6.1 | build_board_etf_map.py 刷新后自动触发 export_overview 重算;百分位基线预计算固定化(工作量大可选) | deploy.sh step 0.8 已刷 map;自动联动未做 | **已废弃**(08-21 移入 docs/abandoned-features.md L18-20,fac9d0b75) |
| 27 | **R2 P2:R2 上传失败阻断 push + 版本校验** | docs/r2-migration-implementation-report.md §3.3/§6.1 方案3 | deploy.sh 关键文件 R2 上传失败则阻断 push;dataRewriteHandler 对关键文件加 last-modified 校验 | P0 方案1 已加"upload_r2 空时 notify 告警" | **已废弃**(08-21 移入 docs/abandoned-features.md L18-20,fac9d0b75) |
| 28 | **R2 P2:edge cache purge 兜底(deploy.sh 末尾统一 purge / Worker 定时清理 / HIGH_FREQ TTL=5s)** | docs/r2-migration-implementation-report.md §3.3/§6.1 方案5 | deploy.sh 末尾统一调一次 purge_cache 或 Worker 定时清理 | upload_r2 各命令已 purge;deploy 末尾统一 purge 未加 | **已废弃**(08-21 移入 docs/abandoned-features.md L18-20,fac9d0b75) |
| 29 | **R2 审计 P1:159335 track_score 跨文件不一致 + 百分位基线动态变化** | docs/r2-migration-implementation-report.md §3.2/§6.2 | board_etf_map=30.2 / index_detail=30.2 / overview=30.9(match_method 不同);基线随候选集变化 | 非 bug 但需关注一致性 | **已完成**(08-22 6e0f70eb6) |
...
| 112 | **staticdata_sync.sh 补大 JSON 守卫 + 守卫判定下沉单一源** | 2026-09-26 会话;报告 docs/ops/large-json-guard-sync-20260926.md | staticdata 备份 git 有两个 commit 入口,原只有 `staticdata_backup_async.sh` 有守卫,`staticdata_sync.sh`(intraday 每 ~30min / fetch-news / daily-brief 触发)会 `git add -A` 静默提交。两缺口:①已跟踪文件涨过 20MB(`.gitignore` 对已跟踪文件无效)②新增未跟踪 >20MB 文件不进 `large_json_excludes.py` 的 `desired` 集合。实施=3 文件 5 项:default_mode 加"未跟踪且未被 ignore 的 >20MB 文件"、积压两阈值下沉单一源、新增 `--check-staged` 模式(rc 0/1/2)、sync commit 前三段式、async 内联守卫改调它 + 修 bash 3.2 空数组致三处 notify 静默丢失。实测新增耗时 ≈0.25s | 无 | **已完成**(2026-09-26 merge `1a70665b3`,reviewer PASS:8 项必查全部实跑,等价性逐行对照/D 自锁 rc=0/缺口 B 闭环/bash3.2 空数组/退出码契约/只读确认/遗漏入口 grep 均通过;云上已同步)。审查发现 F1→#113、F2(rc=2 降级路径原 500MB 兜底失效,有意识变更建议不改)、F3(sync `--all` 无活调用方,文档债务)、F4(docs/ops/large-json-out-of-git-20260925.md 未提 sync) |
| 113 | **async step3.5b 缺 R2 上传的 dry-run / 目标隔离钩子(集成测试必污染生产桶)** | 2026-09-26 事故 | `staticdata_backup_async.sh` step3.5b 直接调 `upload_r2.py upload-large-json` 写**真实**私有桶 `signal-backup`,脚本只有 notify 的 dry-run 钩子(`STATICDATA_BACKUP_NOTIFY_DRY_RUN`)和 staticdata 仓库路径可覆盖(`STATICDATA_REPO`),**R2 上传这步无任何隔离开关** → 任何集成测试都必然写生产(实际把 20MB 零文件传到 `large-json/2026-09-26/small.json.gz`,已确认删除)。修法候选:加 `STATICDATA_BACKUP_R2_DRY_RUN` / 允许覆盖目标桶 | 无 | **待实施**(2026-09-26 reviewer 已给最小修法:实测 `R2_BACKUP_BUCKET` env 可隔离桶(404 无污染)但**未文档化**;且 upload-large-json 即使全失败也会重写 manifest + 跑 _prune_large_json(测试会把 manifest 重写成 0 行)。修法=async 加 `STATICDATA_BACKUP_SKIP_R2_UPLOAD=1` 或 upload_r2.py 加 `--dry-run` + 文档化 env 隔离) | **已完成**(2026-09-26 merge `c921af9f6`:`upload-large-json --dry-run` 复用库内既有 `_DRY_RUN` 全局 + async step3.5b 加 `STATICDATA_BACKUP_SKIP_R2_UPLOAD=1` 跳过钩子;reviewer 独立复核 PASS:五条写路径(PUT/HEAD/DELETE/prune/manifest 重写)全在 dry-run 分支后、monkeypatch 计数 `{s3_request:0,s3_head:0,prune:0,manifest_write:0}`、async 全隔离 exit=0 且只跳 3.5b 不误伤 3.5a、非 dry-run 日常路径零行为变化) |
| 114 | **本机(mac)staticdata 写入路径治理:杜绝非权威全量覆盖生产 DR 仓库 + 本地副本对齐远端** | 2026-09-26 事故 | mac 侧 local commit `a2a57a0a0c`(1963 文件 = 1955 个 data 文件 + 8 个迁移 D + .gitignore)待推,mac 副本停在 9-13 旧版 → 推上去会用 13 天非权威数据覆盖共享的生产 DR 仓库,已决定**不推**。结构根因:mac 的 staticdata commit/push 路径是"mac 还是生产机"时代遗留,`staticdata_backup_async.sh` 做全量 `git add -A` + 全量 push。C=根治(用户已拍板做);B=本地副本 `git reset --hard origin/main` 对齐,当前被 fetch 失败挡住(`early EOF` / `invalid index-pack output`,3.2G 仓库) | 需先查清 mac 是否还有**合法**写入路径 | **已完成**(2026-09-26 feat/staticdata-write-guard:C 三件套实施+三件自验实跑全过,报告 docs/ops/staticdata-write-guard-20260926.md;调研前文 docs/ops/staticdata-mac-write-path-research-20260926.md)。结论:①全仓穷举确认**只有 2 个 git 写入路径**(sync + async),无第三个;②mac 数据**默认旧**(7/7 抽查全部 mac 旧云新;9-13 迁云后 mac 无自动任务);③**9-26 14:52 那笔 push 是被 non-fast-forward 偶然拒绝才没覆盖,失败是偶然不是保护** —— 下次远端落后时再跑就能推上去覆盖 DR 仓库。**C 方案(三件套,researcher 推荐)**:1)新建 `scripts/staticdata_write_guard.py` 单一源(风格同 `large_json_excludes.py --check-staged`),`--check-write-auth` 判定非生产机(路径非 `/home/ubuntu/`)默认 rc=1 → 跳过 git commit/push 段(仅 rsync 磁盘 + R2 + 降级通知),云上行为零变化 2)数据闸门:显式 `STATICDATA_ALLOW_PUSH=1` 时先 `git fetch origin`(失败即拒),只推"远端不存在的新路径(A)",M 中本地旧于远端者排除 add 清单 → 只增不覆盖 3)两脚本 git 段前调守卫(共用单一源不复制)。**B 解法已实测**:`git clone --filter=blob:none --no-checkout --single-branch -b main` → .git 仅 **13MB**、HEAD 与云上最新一致(3.2G 全量 fetch 老失败的根治);数据层从云上 rsync,日常只读用 blobless 增量 fetch;替换前先把 mac 旧副本 1963 文件工作树留档到 /tmp。**⚠ 实施时序:必须等 `feat/large-json-followups` 落地后再开分支**(两者都要改 `scripts/staticdata_backup_async.sh`,并发会冲突,§23.4)。**B 已完成**(2026-09-26):mac 镜像换 blobless clone,`.git` 3.4G→**17M**(约 200 倍),HEAD 与云上/GitHub **三方逐位一致**(`c8a0a46`);旧镜像改名保留为 `…-old-20260926`(**改名回来即可回退**);留档 `~/staticdata-mirror-archive-20260926/`(本地独有 commit bundle 20.8MB + 旧工作树 tar 4.9GB,均已校验可读);报告 docs/ops/staticdata-mirror-blobless-20260926.md。**⚠ 手法偏差(实施方已显式上报)**:纯 `--no-checkout` 会留 31548 行 D,补 `sparse-checkout set`(顶层文件 + config/docs/scripts)后 status 归 0 → 镜像现为 **sparse**,`data/` 由 rsync 段填充、git 层 untracked 属预期;日常跟进用 `git fetch --filter=blob:none` 增量,别全量 fetch |
| 115 | **large-json-backup-manifest.md 生成器输出与仓库版本不一致 → 每次 async 跑留永久脏文件** | 2026-09-26 发现 | `_write_large_json_manifest`(upload_r2.py:2113)写的是"薄表头 + 带 sha256 的完整快照表",而仓库里存的 `docs/large-json-backup-manifest.md` 是手改的"厚表头 + 空表骨架"→ 生成器产物与 HEAD 永不相等 → **每次 async 跑完必留一个 M 脏文件**。修法:把生成器输出提交掉(HEAD 对齐生成器),信息量无损失。附带:`feat/tier-card-mobile-height` 的 CSS 注释引用了过时的任务号 `#74`(该编号属模块三且已完成),同批顺手改掉 | 无 | **部分完成**(2026-09-26 merge `c921af9f6`:生成器头部时间戳改仅日期 `%Y-%m-%d`,同日同内容连跑两次 md5 逐位一致、跨天仍会重写,reviewer 独立复核 PASS,消除了**同日重复跑**的 churn)。**⚠ 核心症状未根治(主控 §0 验收发现)**:HEAD 存的仍是手写"厚表头+空表骨架",而生成器输出是"薄表头+带 sha256 的完整快照表"→ 内容仍不相等 → **每次 async 跑完 trade 仓库照样留一个 M 脏文件**。根因=该文件写在 **trade 仓库**(`upload_r2.py` 里 `ROOT/docs/`),而 async 提交的是 **staticdata 仓库**,没有任何环节提交它;且表体是每日快照索引,内容天然天天变,光"提交一次"治不了。**剩**:①manifest 归属决策(建议改写到 staticdata 仓库,async 本就提交它;restore 脚本读旧+新双路径兼容)②`style.css` 注释里过时任务号 `#74`(改前端源需同 commit bump+重建 min,建议随下次前端改动顺手改) |
| 116 | **`--dry-run` 对不消费它的命令静默失效(仍真写生产桶)** | 2026-09-26 reviewer 中危发现(pre-existing,非本次 diff 引入) | `scripts/upload_r2.py` L2608:`--dry-run` 是全局 flag,`__main__` 会先把参数移除;对**不消费 `_DRY_RUN` 的三条私有桶写命令**(`upload-db` L1917 / `upload-decommissioned` L1958 / `upload-claude-backup` L1991)会**静默移除参数 + 仍真写生产桶**,无任何警告。本次 F1 给 `upload-large-json` 立了"零 R2 接触"先例后,未来测试 agent 很可能对其它私有桶命令也用 `--dry-run` 而踩真写 | 无(非测试误触路径:这三条由 `backup_claude_self.sh` 等专门链路调用,不在 async/deploy 集成测试面) | **待实施**(建议二选一:①在 L2608 移除 `--dry-run` 时对非消费命令打印一行"⚠ --dry-run 对该命令无效,仍会真执行"②把三条也接入 dry-run。另需在文档/role skill 写明:私有桶命令的唯一隔离手段是 `R2_BACKUP_BUCKET=<不存在的桶名>`) |
| 117 | **staticdata 写入守卫 reviewer 遗留 5 条(2 中修 + 3 文档化)** | 2026-09-26 reviewer 报告;报告 docs/ops/staticdata-write-guard-20260926.md | merge `d22e2e4b4` 后遗留,均在罕见/降级路径,不阻断。**中 M1**:async 数据闸门内部错误 rc=2 分支(L219-226)同时设 `STATICDATA_FAIL=1` 和 `_SKIP_NONPROD=1`,收口心跳优先级(L381)`_SKIP_NONPROD` 排最前 → 心跳写 `skip_nonprod` 而非 `fail`,**故障类型被标成"非生产机跳过"**,monitor 语义混淆 + 36h 停摆兜底延迟(进程 exit 1 + notify 已发,非完全静默)。修法=rc=2 分支不设 `_SKIP_NONPROD`。**中 M2**:async 顶层守卫 rc=2 分支(L202-204)只有 echo + FAIL=1,**无 notify**;schedule_monitor 对 fail 不重复告警(设计依赖脚本内 notify)→ 即时告警缺口,靠 36h 兜底。修法=补降级 notify。**中 M3**:`--check-fresh` 的 M 判定用 `os.path.getmtime < 远端 commit ts` 近似,且**对每个候选路径跑一次 `git log -1 origin/main -- path`**(`_remote_last_commit_ts`),3.2G/数万文件下显式放行路径可能数十分钟~小时级;mtime 近似同时存在误伤(安全方向)与误放行(危险方向,少见)。修法=文档化性能边界或改 `git diff` 批量比对。**低 L2**:D 永远排除 → DR 仓库只增不缩(可接受,建议文档化)。**低 L3**:fail-open —— 守卫脚本 crash(Python rc=2)或 PY 缺失(rc=127)会落入"按生产机继续",等于守卫失效(有意识 best-effort 取舍,建议显式注释) | 无 | **待实施**(建议与 #115 剩余项打包:同改 `scripts/staticdata_backup_async.sh`) |

## 六、管理端 / 新功能

| # | 功能 | 出处 | 方案摘要 | 依赖/前置 | 状态 |
|---|---|---|---|---|---|
| 34 | **场外基金阶段3:场内外联动(ETF 联接跟踪误差)** | TASKS.md L65 | 阶段1 评分引擎+阶段2 前端 UI 已实现;阶段3 场内外联动未做 | 阶段1/2 已上线 | **已废弃**(08-21 移入 docs/abandoned-features.md L21,96ee60bf7) |
...
| # | 项 | 出处 | 说明 | 状态 |
|---|---|---|---|---|
| 37 | **美股 VIX 采集** | docs/理财专员使用指南.md §5.6(L458) | 无直接 akshare 函数,未采集 | **已废弃**(08-21 移入 docs/abandoned-features.md L15-16) |
| 38 | **乐咕活跃度 / 东财情绪源** | docs/理财专员使用指南.md §5.6(L459) | 源不稳定/接口待验证,已禁用 | **已废弃**(08-21 移入 docs/abandoned-features.md L15-16) |

## 八、会话新待办增补(2026-08-12,来源:TASKS #4/#16/#17 + 当日会话产出;由每日 23:45 cron 自动同步维护)

| # | 功能 | 出处 | 方案摘要 | 依赖/前置 | 状态 |
|---|---|---|---|---|---|
| 42 | **上下文优化 3 项** | docs/context-optimization-research.md(2026-08-12) | ①OPT-2 MEMORY.md 索引瘦身 19.8KB→~8KB ②OPT-1 轮询/编排降本(状态门控+夜间降频)③OPT-3 主会话 /clear 分会话+Compact Instructions(context rot,保质量>省token) | 无 | **已完成**(08-18 OPT三连:OPT-1 f89b06481/OPT-2 MEMORY 瘦身/OPT-3 e24d42a54) |
...
| # | 项 | 出处 | 说明 | 状态 |
|---|---|---|---|---|
| 108 | **四档升级 v2:excludeSpecialBear 判定源换 cyb** | docs/market-state/kelly-fourtier-v2-multiindex.md | 研究报告已出结论推荐 cyb(+16,720/+12.86pp,G 模式,9 模式 8 正 1 小负);excludeSpecialBear 判定源 hs300→cyb 属改历史功能 B+ 级,须用户拍板后实施,§5.4⑥ 动核心键发版本 | **已拍板不动**(2026-09-17 用户:报告已阅,收益更大(+16,720)但回撤也更大,先不动,留历史对照) |
...
| 87 | **PWA 体验增强(150 行,未实施)** | docs/archive/TASKS-history-archive-20260820.md L464(价值排序表 A6 PWA 未实施)+ L145(A6 PWA 三件套修正已上线) | 已存在(PWA 三件套 044fd34d 已修正 theme_color+sw.js 重写 App Shell CacheFirst);**剩余 PWA 体验增强**(150 行)未实施(价值排序表中 C PWA 位列靠后) | 无(App Shell 已就绪) | **已废弃**(08-22 PWA 三件套已就绪,TASKS-done.md) |
| 88 | **订阅推送(410 行,待实施)** | docs/archive/TASKS-history-archive-20260820.md L464 | 订阅推送(410 行)为剩余 6 方向大工作量项;第一批已做(留言箱/浏览器通知等),订阅推送未派 | 无 | **已关闭**(08-22 b566e1e44,订阅推送关闭 A12 已实施 c703a584f+3d29c05c4) |
| 89 | **overlap delta 可比口径(进行中→派实施)** | docs/archive/TASKS-history-archive-20260820.md L97-114 | overlap delta 中报 vs 年报披露范围差异导致偏大(中报2835只 vs 年报5285只);调研产出 2-4 个可比口径方案+推荐+工时,调研完派实施 agent | 调研报告出后派实施;项目 #44 可用口径待用户选 | **已废弃**(08-21 13705c705,被 ui94 可比口径优化覆盖) |
...
| 91 | **次日开盘口径确认(✅ 已完成,2026-09-12 核销)** | TASKS.md「次日开盘口径」待办 + 模块三 #15(同项重复登记) | 信号收盘后固化、次日开盘买入是真实可执行口径,成本极低(每日池净利差 -0.01%、每笔1万 -0.57%,跳空均值 +0.031%,-1% 限价 SOP 触达率39.8%)。**✅ 已完成**:后端 `signal_kelly_backtest.py` `KELLY_BUY_NEXTDAY=1` 默认次日开盘(v1.1.4 起),前端 2026-09-06 上线「买入口径」双档切换按钮(`labSigKellyBuyBasis`,默认 next_day_open,可切当日收盘),双产物 `signal_kelly_backtest.json`+`_sdc.json`(signal-day-close)齐备,README L70 公示。报告 docs/kelly/position/kelly-nextday-open-backtest.md | 已按次日开盘默认落地,无需再拍板切换 | **已完成**(2026-09-12 经 pending-index-drift 核查实物销号:v1.1.4 默认+前端双档按钮+双产物+README 公示沿线各点均实际存在) |
| 93 | **首页 AI建议N 无 `tds_poscap` key 时不显示(存量非回归)** | 2026-08-20 tester Playwright 线上核验 #29 时发现(上报 §23.7,未改代码) | 首页 AI建议 在 localStorage 无 `tds_poscap` key 时(首次访问)→ kept map 不 build → **AI建议N badge 不亮(显示 0)**;代码 L2666 注释声称"无 key 保持 K=1 主推高亮"但 L2669 `if(_raw)` 实际需要 key。**不影响回测宇宙/推荐算法核心判定**(判核心全 PASS),独立展示层小茬 | 无 | **已完成**(2026-08-21 507b86f00 已修已上线;索引描述过时——key 实为 `tds_home_poscap`、位置 L6266 `if(!_raw)`,2026-09-17 核销) |
| 98 | **CodeGraph `codegraph ui` 浏览器可视化启用(待观察官方发包)** | 2026-09-04 用户指出 codegraph 官方 README 有 `codegraph ui`(之前评估结论"无 UI"是误判——本地装 npm 1.6.0 旧版,而 ui 功能 08-26 后才合入 GitHub main 未发 npm;远端 README 证实:`codegraph ui` 打开 http://127.0.0.1:4747,callers 左/源码中/callees 右,`codegraph web` 别名,--port/--no-open/CODEGRAPH_BROWSER 参数,只监听 127.0.0.1 只读)。评估落档 docs/codex-reviews/codegraph-eval-20260901.md + memory codegraph-eval-conclusions | **待办**:①观察几天官方是否发 npm 新版(含 ui)→ `npm i -g @colbymchenry/codegraph@latest` 升级 ②或 git clone main 源码自建;启用前须知 3 坑仍在(坑1 app.js>1MB 静默跳过最重,UI 读同一索引同样查不到;坑2 同名污染;坑3 局部符号) | 官方发版或用户拍板源码装 | **待观察**(用户 2026-09-04 定:等官方发包+看坑修复,几天后复查) | | TASKS.md「SVG P1」待办未做部分(2026-08-20 治理全判远期移出)+ pending #80 P2-11 | **P1 全站扩展已上线(2026-08-20 home SVG 批 commit 293b1d101 + P0 site-config/ETF评分弹窗/皮肤切换 d9a465dc6)**,剩**大盘 tab renderAStock 30+ echarts 改 SVG 或 IntersectionObserver 懒渲染(P2-11,3-6h,切 tab -500~1000ms)**。用户 2026-08-11 追问状态(此前质疑"首页没效果"即 P0 只接 1 消费点,§23.3 举一反三要求覆盖全站走势图渲染点)。⚠**重复登记:同项 = #80 P2-11(本行为待办移出确认,见 #80)** | 与 #80 P2-11 同项(大盘 tab 展示优化) | **已上线/见 #80**(08-22 大盘 tab 懒渲染 e01de0423;主登记 #80 P2-11,本条为待办移出确认) |

| 99 | **右下角「更新新版」气泡 hoverpop 预览本次更新内容** | 2026-09-05 用户「每次出现更新新版的时候,是否可以 hoverpop 预览到更新新版的改动是哪些,方便对齐用户心智预期」(09-05 曾落档 TASKS 交接日志,09-05 晚接力轮重写顶部日志时被覆盖丢失,09-06 重新登记防再丢) | 现状:右下角已有「🔄 已有新版+刷新」toast(index.html #sw-update-toast),但只提示"有新版"不展示"变了啥"。目标:气泡 hover/点击时弹出本次更新内容清单。**核心卡点=改动说明从哪来**,候选:a.发版时 main-merge.sh 自动从 merge commit 链生成 changelog 初稿(纯自动、零人工、和版本一一对应,难点 commit 名→人话翻译质量)/ b.人工维护 CHANGELOG 文件(最准最像发布说明,靠人记易漏)/ c.折中=自动生成初稿+人工润色。推荐 c 或 b 起步(sw.js CACHE_VERSION 注释已逐版记 changelog,可作数据源雏形)。实现面:新建轻量 changelog 数据文件 + sw-update-toast 加 hoverpop(复用既有 hoverpop 机制)+ 发版时登记一行 | 需先定改动说明数据源方案(推荐 c/b);改 index.html+app.js 纯前端 | **已完成**(2026-09-06 上线:index.html #sw-update-changelog + static-site/changelog.json 版本串→条目 map,数据源=人工维护 changelog.json;2026-09-17 销号) |
| 100 | **信号凯利回测「近1年按需先算先展示 + 后台全量覆盖」渐进加载实施** | 用户 09-04 拍板方向 + 可行性评估已落档 docs/kelly/analysis/sigkelly-progressive-y1-load-20260904.md(09-06 commit 1918bdb7c) | 结论:技术可行准确无损。y1 窗口基笔 100% 只落 t2025+t2026 两片,两片 vs 全量 kept K=1/2/3/4 逐位一致。落地=报告§③最小方案:①前端两阶段加载(_labKellyLoadYearParts 先拉 2025+2026 两片→y1 先渲染,后台拉齐其余 14 片→全量覆盖)②就绪状态机(未就绪周期只显示占位不显残缺数)③K 档评级必须门控(阶段1 不发布动态源,保持 null→回退静态快照并标注)④缓存签名加 partsReady(防阶段1 result 被阶段2 误命中)⑤按年聚合表阶段1 只有 2025/2026 两行,全量后补行。首屏下载 ~30MB(两片)vs 全量 69MB 省 57%。风险:缓存污染(最高)/_tradeDims 重建/K 档残缺发布/切周期重入 | 评估已完成(报告在 main);须在 impl-54 方案B(首页 K 档动态源)merge 后同基线上改(lab.js/app.js/common.js 同一批文件) | **已完成(2026-09-06)**。F2-A 修复(5dbaa63a5:guards+data在册条件+catch 保留 y1+重试重置状态机)+ 同类边缘修复(ccafb9c8f:阶段2失败+全量兜底失败后长周期永久占位→_labKellyMaybeResumeAllBackground 自愈重触发+15s 冷却防风暴),main-merge 统一 bump a544(47e371c95),§0 三查 PASS(线上 lab.min.js 含 _labKellyMaybeResumeAllBackground,index?v=20260906-a544) |
| 101 | **北交所宽度宇宙纳入:方案 C 单独出北交所宽度(用户已拍板 C)** | 拍板材料 docs/analysis/beijiao-exchange-width-universe-20260902.md(commit 5e3122ace,三方案推荐C);2026-09-06 用户确认「不是已经拍板 c 了么」=已拍板方案C | 方案 C=新增 a_bj_width_* 独立指标组(北交所 339 只自己的上涨/下跌/涨停/成交额/AD 线),**不动现有主宽度**(AD线/恐贪/涨停判定全冻结,§23.7 只增不改),历史独立可比,管道与方案 A 同构未来可升级。前置验证项:FAPI daily-k 全量抓取验证 920 段历史覆盖起点(2024 前北交所是 43/83 段)。落地要点:①挂 FAPI 定时(18:10,已有 launchd)②FAPI 北交所日线→a_bj_width_* 计算③30% 涨停档判定(北交所 30% 规则,主板 10% 档会每天误判 2-14 只)④新前端展示位⑤宽度口径公示文案修正(app.js L6756 _WIDTH_CALIBER_TIP 现写"akshare 新浪源"过时,实际收盘 mootdx;无论选哪个方案都该修) | **已完成(2026-09-06)**。落地 5 要点全齐(8e1cf23b2+93295a2f3+44dcd87bb+0e6b4e986,bj-fapi 分支):①fapi_daily_syn.sh 链式 18:10 ②bj_width.py a_bj_* 计算(30% 档,dry-run 20260904 zt=1)③30% 涨停档④前端 app.js 9 处+queries KPI ⑤_WIDTH_CALIBER_TIP mootdx 修正(不含北交所)。数据层 a_bj_* 主库在位,18:10 链式导出当天上线;注意:**main-merge 首次漏带 3 后续 commit,已 cherry-pick 补齐(1d85e0fae/ea2b27aca/f26777b59)** | **已完成** |
| 102 | **FAPI 数据源观察期转正为正式兜底源** | FAPI 三天观察期评估报告 docs/fapi/fapi-observation-eval-20260904.md(commit 377283f4e,09-06 落档) | 观察期全部维度通过(概念 27/27 100% 对齐、T+0 vs mootdx 两日 0 不一致、延迟秒级 p50−234ms、北交所 340 唯一覆盖、采集稳定)。转正=从"观察期试点"提升为"mootdx 断片/空值/北交所缺口时的自动兜底 + 每日 T+0 互证校验",**不动主链**(主宽度/AI 推荐零改动)。转正条件 5 条:①fapi_daily_raw 作 mootdx 空/断片自动兜底(挂 runner/width 链)②互证校验脚本挂 deploy 前 check(fapi vs mootdx close 差异>0.5% 告警)③北交所宽度宇宙口径=#101 已拍板 C④概念换官方独立走 1 周观察(09-08 到期,与转正解耦)⑤THS 概念/README §21 公示同步。风险:单一外部依赖(同花顺)但只做兜底;待长期验证项(兜底真实触发/30% 档/北交所全量历史)不阻断转正 | **已完成(2026-09-06,仅①+②)**。①fapi_daily_raw 作 mootdx 兜底 runner.py step9b+width 链 ✓ ②check_fapi_mutex 互证校验挂 deploy 前 check(实测 PASS:20260904 5174 只 close 全一致)✓ ③④⑤ 解耦:cron 1e0ef6fb 观察期 09-08 到期(周一收尾一次);FAPI 概念观察 09-08 到期与转正解耦 | **已完成** |
| 109 | **codex 外审站点 lab/sigkelly 评测处置(排凯利任务后)** | docs/codex-reviews/site-review-sigkelly-20260917.md(commit 3d6a486ae);用户 2026-09-17 拍板「结论1修/结论3不采纳/结论2·4·5排期」 | 外审 5 条整改处置:①**结论1 修**=worker/headers.js L44 immutable 正则补全版本化静态资源(style.min.css/lab.min.css/common.min.js/purpose-notes.min.js/i18n.js/inline-init.js/manifest.json/changelog.json,1 行,现漏配致每次 revalidate);②**结论2 排期**=数据瘦身(构建期预聚合摘要+明细按需/范围 API,100MB→估 1/10,最大提速);③**结论3 不采纳**=「摘除 78MB 全量 signal_kelly_trades(_sdc).json 死数据(156MB)」系误判——app.js L3271/L3599 全量文件是分片失败兜底路径,非死数据不能摘;④**结论4 排期**=移动端字号/点击目标上调(标签≥11px/点击≥44px);⑤**结论5 排期(中期)**=app.min.js 按 tab 拆包+echarts/core 按需引入。附:codex/fix-watcher-sync-git-refs 分支 1 commit 618e9d660(.failed retry 不再被 sync_git_refs skip 阻塞)未进 main 待用户定 merge;其余 watcher 修复已全进 main | 排在凯利 ③④⑤ 根治 + ①② review/merge 之后;结论1 是纯前端一行小改可顺手 | **待排期**(结论1 已完成 2026-09-18(immutable 正则补全 4b59236ea,线上 header 已 immutable);结论3 不采纳已复核;结论4 已完成 2026-09-19(字号≥11px/点击≥44px,main v20260919-a602/a603);结论2 lab_ 切片摘除已完成(2026-09-20 merge f286f7d10,云上+R2 739 片删净),基笔唯一化生成端三表已完成(步1 全量 merge b2ccb72d7 + 步2 分片 merge 2a1fcf251,双 reviewer PASS,76.6MB→5.6MB 13.3x),前端适配+兜底切换仍排期;结论5 排期;sync-git-refs 1 commit 待定) |
