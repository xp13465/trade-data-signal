...
| 61 | **邮件/飞书信号带「回测宇宙+AI建议/AI警示」标记(提高可信度)** | 2026-08-17 用户定(用户原话:"邮件抄送信号时也要参考首页的回测宇宙 有一个标记。不在回测宇宙和ai建议ai仓位的都要明确标注。反过来在的就要强化展示 类似首页的当日已满 或被什么降亏标志过滤了...提高可信度 加强对就和首页一样 ai警示 ai建议123") | 发盘中信号邮件/飞书时,每个信号带首页同款标记:①**回测宇宙**:不在 `_bt_in_universe` → 明确标注「未入回测宇宙」(历史表现不背书)②**AI 警示**:命中 AI降亏过滤键(filters,如 excludeSpecialBear)→ 标注「AI降亏/建议回避」(类比首页删除线+AI降亏 badge)③**AI 建议 top-K**:在 AI 建议/AI 仓位(top-K)→ **强化展示**(类比首页「AI建议 1/2/3」badge)④当日已满/未入样等状态邮件体现。数据源=打通 overview.json 的 `_bt_in_universe`/`ai_macro`(queries.py 从回测侧注入)到 check_signals.py 邮件/飞书。目的:明确告诉用户"这个信号回测认不认、AI 认不认",提高信号可信度 | **未实施**(依赖 check_signals.py 当前 #58 系列改动完成后派,同模块排队防冲突) |
