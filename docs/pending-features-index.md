...
| 94 | ~~**AI 降亏 AUTO 模式(适时自动切换)**~~ | — | — | — | **已关闭销号**(2026-08-25 用户核实+主控查证:regime 轮动穷举回测已出结论=**不存在样本外成立的切换器**——38,220 组合穷举,验段最优规则净利落后最优静态 A_on9 约19%且回撤深近九成,STRICT 判据 0 通过,头号假设「牛市主升→切A」亦 FAIL;报告 docs/kelly/analysis/regime-mode-rotation-research-20260823.md §一。**v1.1.5 基座定稿=NEW14**,观察期后切 14+1 打 v1.1.7 即定稿动作) |
