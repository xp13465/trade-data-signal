...
| # | 功能 | 出处 | 方案摘要 | 依赖/前置 | 状态 |
|---|---|---|---|---|---|
| 10 | **ETF 弹窗 30 天外长历史(需求2)** | docs/chart-refactor-config-plan.md 需求2(行29) + docs/chart-p2p3-data-source-research.md P2(L9) | ETF 侧缺:etf_score_list.json 只近 30 日 ohlc,需新数据产物 etf/{code}-all.json + 弹窗 period tab | 无阻塞(etf_daily 表 1520 只 ETF 2005 至今 21 年全史现成,不需新数据源;缺口仅导出产物 etf/{code}-all.json 复刻 index/{iid}-all.json 模式 ~29KB/只走 R2);P1 chartLite 组件 | **未派**(2026-08-22 刷新:阻塞解除——etf_daily 21 年全史现成,chart-p2p3 报告 L9,可直派实施) |
...
| 94 | **AI 降亏 AUTO 模式(适时自动切换)** | 2026-08-23 用户构想(汽车驾驶模式类比:标准/运动/越野之外加 AUTO) | 模式下拉新增 AUTO 档:按市场状态(t 时点可见信号:牛熊分界/QVIX/两融/北向等)自动在 7 方案间切换;**存在性验证回测进行中**(regime-mode-rotation-research,防前视+样本外 walk-forward 铁律,**效用=平稳优先**(memory user-risk-preference-stability-first),核心证据=条件前瞻收益「信号后窗口谁最强」);回测成立才立项实施(四消费点接入+状态信号后端预计算+§21 公示)。**用户定调(2026-08-23):本调研=v1.1.5 定稿「平台主推 AI 算法基座」的最后一次努力**——成立则 v1.1.5 基座=AUTO 方向,不成立则 NEW14 设默认(TASKS.md T5 已同步暂缓) | 依赖 regime 轮动回测结论(docs/kelly/analysis/regime-mode-rotation-research-20260823.md,2026-08-23 派) | **调研中** |
