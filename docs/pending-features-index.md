...
| 95 | **[P1] S06 第 8 票 mode_votes 与 fallback votes 快照契约不一致** | 2026-08-27 v1.1.7 审计(app.js:4828 vs 4843) | mode_votes 路径:日期超快照覆盖期→直接计 1 票(fail-open);fallback 路径:日期超覆盖期→逐键求交后判断(fail-closed)。同信号两条路径可能得出不同 Y 票数。修复方向:两路径统一 fail-open 契约——base 未知时同语义处理(保守放行计 1 票) | S06 模式 | **待排期** |
| 96 | **[P1] X 共识表快照未就绪时 fail-closed 静态兜底** | 2026-08-27 v1.1.7 审计(app.js:4892) | 注释声称 fail-open=new15,实际硬编码 new15 键集过滤(fail-closed),违反 common.js:903 fail-open 契约。快照加载失败时保持错误结果且无可见降级提示。修复:base=unknown 时该笔不拦截或显示可见降级提示 | S06 模式 | **待排期** |
| 97 | **[P1] 北向增量截断可导致递增丢日** | 2026-08-27 v1.1.7 审计(app/collector/direct.py:241,316) | 主源软限 60s 且缺口上限夹到 ≤10 自然日;节假日后可用工作日 >7 时单轮只采到 deadline 前;下次 _north_fund_gap_days() 只按剩余自然日差重新夹到 ≤10→更早日期永远补不上。修复:缺口上限改为可分轮累积 | 无 | **待排期** |
| 98 | **[P2] fund_manager UPSERT 清空同基金多经理行** | 2026-08-27 v1.1.7 审计(app/collector/public_fund.py:1963,2014,2036) | M2 按 fund_code 更新,主键是 (fund_code, manager_name);同基金多经理时最后一条匹配所有键→后写者清掉先写者的 managed_history 等采集列 | 无 | **待排期** |
| 99 | **[P2] stage0 attempt 使"合法空结果"永久免重采** | 2026-08-27 v1.1.7 审计(public_fund.py:1580,2009,2102) | 页面解析成功但返回空结果→记 attempt 为 valid/完成→上游后续补字段不会自动重采 | 无 | **待排期** |
| 100 | **[P2] fund_basic UPSERT 实际覆写全部扩展列为空值** | 2026-08-27 v1.1.7 审计(public_fund.py:2142) | 与注释声称的 UPSERT 保护语义不符,真实空结果会静默清掉已有字段 | 无 | **待排期** |
| 101 | **[P2] Fund Nav checkpoint 每 500 chunk 才保证磁盘一致性** | 2026-08-27 v1.1.7 审计(scripts/upload_r2.py:810-838) | 进程 kill 后 0~499 个成功 PUT 不落盘 checkpoint→下轮全量重建会重传(非丢传但浪费带宽) | 无 | **待排期** |
| 102 | **[P3] Worker/Upload TTL 双源漂移** | 2026-08-27 v1.1.7 审计(worker/headers.js:190 vs scripts/upload_r2.py:1305) | overfit_monitor.json Worker 给 600s TTL、Python 给 0;同类名单不完全一致。无直接数据丢失,增加维护成本 | 无 | **待排期** |
| 103 | **两处「停机>30天缺口不自愈」自适应根治**(etf_national_team accum_nav 近30天 NULL 扫描 + width_history run_recent days=30 写入限幅) | feat/north-fund-gap-accumulate(#97 修复)同类面扫描发现(2026-08-27),低危上报 | 向 #97 本尊修法看齐(state 前沿推进+撞库闭合探测,分轮累积),真·自愈;两处各有手动兜底通道(--lookback 参数 / 全量 run()) | **前置=当前批次版本稳定 + 用户明确启动**(2026-08-27 用户拍板「未来等版本稳定了我来明确启动b修复」);先行告警兜底批(选项A:窗口外缺口三态告警+progress断点挪持久目录)已在任务台账,当晚批次上线后派 | **远期·等用户启动**(触发条件见前置列) |
| 104 | 每日 ~200 metric 全量保鲜扫描扩面(数据断档检查器从北向/净值/宽度族扩到全量指标域) | 2026-08-27 用户拍板=先不做,待四检查器误报率晒几天后按白名单分批纳入,哪个域真出过事先纳入哪个(告警批 feat/alert-fallback-batch 报告第三节同结论) | **远期·白名单分批·晒几天再议** |
| 105 | deploy 链时序根治批(用户拍板=先止血后渐进):①D 方案=所有先export后deploy数据类(etf_score_list/etf_hist/fund_score/fund_nav/stage0-daily)统一挪 O1 纯部署闸门前(2026-08-27 fund_nav 两连拦根因,A 方案单挪 fund_nav 已先行)②public_fund_daily/deploy.sh public-fund/backfill 各槽共用闸门的时序适配③mootdx「半死」状态(部分成功不触发兜底)成功率型兜底要不要做④scripts/check_ds_resilience.py 挂进 lint_scripts.sh 检查链⑤codex review 进程与主控 agent 共享主树的分支劫持隐患 worktree 化(2026-08-27 实证 ff-merge 劫持一次已修复) | 2026-08-27 用户拍板「大动干戈的根治等清理出一个功能版本后再一点点修,避免多bug叠加不好定位真实问题」 | **远期·功能版本清理后分批** |
