...
| 114 | **本机(mac)staticdata 写入路径治理:杜绝非权威全量覆盖生产 DR 仓库 + 本地副本对齐远端** | 2026-09-26 事故 | mac 侧 local commit `a2a57a0a0c`(1963 文件 = 1955 个 data 文件 + 8 个迁移 D + .gitignore)待推,mac 副本停在 9-13 旧版 → 推上去会用 13 天非权威数据覆盖共享的生产 DR 仓库,已决定**不推**。结构根因:mac 的 staticdata commit/push 路径是"mac 还是生产机"时代遗留,`staticdata_backup_async.sh` 做全量 `git add -A` + 全量 push。C=根治(用户已拍板做);B=本地副本 `git reset --hard origin/main` 对齐,当前被 fetch 失败挡住(`early EOF` / `invalid index-pack output`,3.2G 仓库) | 需先查清 mac 是否还有**合法**写入路径 | **调研中**(researcher) |
