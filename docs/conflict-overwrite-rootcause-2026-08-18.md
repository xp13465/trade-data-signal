...
## 五.1 §23.11 用户原话(2026-08-18 用户定,最高优先级;来源:根 CLAUDE.md §23.11)
> 本次根因调研的直接触发是用户定下 §23.11「git 冲突/覆盖绝不静默铁律」,用户原话逐字如下(供反查,根 CLAUDE.md §23.11 已指针化到此文档):
> 「全做。尤其是 d。并且未来遇到冲突千万不能静默了。宁可卡住找我决策。也不能静默吃了 等我发现显性bug为时已晚 修复起来也更花时间和成本 到时候一样还是要我来决策。有要扼杀在摇篮里。不能事态扩大了再让我决策修。那还可能带来更大的问题」
>
> 核心一句话:任何冲突/覆盖/倒退,绝不静默吞掉——宁可卡住上报主控/用户当场决策,也不能等事态扩大成显性 bug 再修(修复成本更高、可能引发更大问题);扼杀在摇篮里。本节五、防再犯建议(A/B/C/D/E)即此铁律的机制化落地。
