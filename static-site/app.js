...
function ruleBar() {
  ...
  div.innerHTML = `
    ...
    <div class="rule-detail hidden">
      ...
      <div><b>卖点盈亏标注（方案 B，2026-07-06）</b>：卖点 reason 附 <code>vs前买{±X.XX%}[分类]</code> 标签，标注相对最近一次前置买点 close 的盈亏——<span style="color:#2e8b57">● 绿=止盈</span>（卖点 close &gt; 前买点 close）/ <span style="color:#9e9e9e">● 灰=买点失败</span>（卖点 close &lt; 前买点 close）/ <span style="color:#ff9800">● 橙=无前买点</span>（窗口内无前置买点，趋势中）。前端 markPoint 按此分色（买=红、卖止盈=绿、卖买点失败=灰、卖无前=橙）。</div>
...
async function renderAStock() {
  ...
  for (const [id, idx] of Object.entries(r.indices)) {
    ...
    if (idx.data.length) indexChart(idx.name, idx.data, sigs);
  }
}
