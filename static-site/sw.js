  ...
  if (url.pathname.endsWith('/overview.json')) {
    event.respondWith(
      fetch(req, { cache: 'no-store' })
        ...
        // P0(2026-08-14): 失败不再返 HTTP200 的 {"error":"offline"} 占位 —— fetchJSON(只查 r.ok)
        // 会穿透当正常数据,结果 r 无 today -> 首页 renderOverview 裸崩(r.today.scores)。
        // 改用 Response.error() 让 fetch 抛网络错误 -> fetchJSON reject -> 走 R2 兜底/调用方 catch,
        // 前端渲染失败态而非把一个 error 对象当正常数据解析去读 today.scores。
