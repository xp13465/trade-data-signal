...
const CACHE_VERSION = 'v6-20260813-a190';  // a189->a190 = #54 hoverpop动态化(bug1总开关联动扩7键+badge降级+重置为AI默认推荐按钮) + 前端动态重算管线; a188->a189 = 凯利回测区AI仓位建议布局调整(删除第一行纯文字标题"AI仓位建议"去重 + AI降亏过滤总开关/详情按钮并入第一行跟在关OFF按钮后); a187->a188 = K档补文案(首页AI仓位建议K档 title 档位语义诚实标注, 默认K=3稳健档非收益率最优) + AI建议编号修复(序号改质量序=当日跟踪分降序第N, 不随K档跳变) 两分支合并(#48#49); a186->a187=AI建议编号改质量序; a185->a186=凯利AI降亏过滤区融合(#39/#45)+§22 K档口径同步; a184->a185=皮肤弹窗样式修复; 基底=a177
