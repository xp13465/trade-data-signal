...
function _kellySharedPosCap() {
  var def = { on: true, k: 1 };
  try {
    var raw = localStorage.getItem("tds_poscap_lab");
    if (raw) { var p = JSON.parse(raw); if (typeof p.k === "number" && p.k >= 1 && p.k <= 4) def.k = p.k; def.on = !!p.on; }
  } catch (e) {}
  return def;
}
function _kellySetSharedPosCap(on, k) {
  try { localStorage.setItem("tds_poscap_lab", JSON.stringify({ on: !!on, k: k || 3 })); } catch (e) {}
}

// ===== ai长线模式(G/H/I)仓位管理 (2026-08-14 #49+#xx; 可扩展架构: 按钮=长线族群总入口, 内部按模式配置独立策略) =====
// 用户定义: 只记「ai长线模式(G/H/I)仓位管理」一个开关, 背后是 G/H/I 长线族群的完整交易方法论。
// v2.2 (2026-08-30 用户+数据定案, docs/kelly/position/trade-methods-principle.md): G/H/I 不再统一 FIFO 20万——
//   G = P≤3d「先卖年轻仓」(手段P)@10万; H = 满仓不买(手段A)@5万; I = P≤3d「先卖年轻仓」(手段P)@9万(由满仓不买改P法; 16/16全正 2026-08-30 真实价重算后作废)
...
const AIHLINE_STRATS = {
  ...
  // v2.2(2026-08-30 用户+数据定案; 2026-09-03 真实权威数重算后更新) G=P≤3d@10万 收146.72%(真实强平日价)/净+146,718/峰10笔/强平375笔; 峰持仓 10 倍=可操作; 负年2011/2021/2023
  p3d10w: { label: "P≤3d 10万", cap: 100000, method: "P", gTier: "10万", gTierNote: "定案档位·真实价口径" },
  // 手段A = 满仓不买: 到 cap 就停买(当日超容整批跳过), 不强制平仓, 自然卖出腾位再买(b0=b1 无强平)
  hold5w: { label: "满仓不买@5万", cap: 50000, method: "A" },
  // v2.2(2026-08-30 用户+数据定案; 2026-09-03 真实权威数重算后更新) I=P≤3d@9万(手段P, 由满仓不买A法改P法): 收159.63%(真实强平日价)/净+143,670/峰9笔/强平358笔; 负年2021/2023(16/16年全正已作废)
  p3d9w: { label: "P≤3d 9万", cap: 90000, method: "P", gTier: "9万", gTierNote: "I=P法真实价口径" }
};
// G 档位(2026-08-30 用户+数据定案: 只保留 10万一档)。函数兼容保留(返回固定 10万),
// localStorage tds_gih_g_tier 历史用户值(15万/20万/13万)自动归一为 10万, 不破坏已有数据
function _kellyGihGTier() {
  return "10万"; // 定案单档: G=P≤3d@10万(2026-08-30 定案, 真实价口径 146.72%, 峰持仓 10 倍=可操作)
}
...
// 开关状态(localStorage 独立键 tds_gihpos_lab, 默认开(2026-08-30 用户拍板); 影响实验室 G/H/I 卡片;
// 与首页 sim 弹窗开关互不联动, 各自管各自区域 §22/2026-08-30)
function _kellySharedGih() {
  var def = { on: true };
  try {
    var raw = localStorage.getItem("tds_gihpos_lab");
    if (raw) { var p = JSON.parse(raw); def.on = !!p.on; }
  } catch (e) {}
  return def;
}
function _kellySetSharedGih(on) {
  try { localStorage.setItem("tds_gihpos_lab", JSON.stringify({ on: !!on })); } catch (e) {}
}
...
// 模式当前策略的展示短线标签(卡片角标/水印/三玩法等共用, §22/§23.3): 返回如 "P≤3d 10万" / "满仓不买@5万" / "P≤3d 9万"
...
function _kellyGihStratExplain(modeKey) {
  ...
  if (key === "hold5w") return "满仓不买@5万: 到 5 万就停买、不强制平仓, 等有自然卖出腾出资金再买新信号(手段A, 无强平→完全确定 230.83%/+115,416, 全场主推)";
  if (key === "p3d9w") return "P≤3d「先卖年轻仓」@9万(I, 2026-08-30 由满仓不买改P法): 超仓先卖持有≤3天的年轻仓、无年轻仓才卖最老, 保老仓砍新仓(手段P; 真实价口径 159.63%/+143,670)";
  if (key === "p3d10w") return "P≤3d「先卖年轻仓」: 超仓先卖持有≤3天的年轻仓、无年轻仓才卖最老, 保老仓砍新仓(手段P; 真实价口径 146.72%/+146,718); 档位 " + (_kellyGihStrat(modeKey).gTier) + "(" + (_kellyGihStrat(modeKey).gTierNote) + ")";
  return "FIFO 强制平最久持仓";
}
...
async function _kellyApplyFeeRecompute(feeParams) {
  ...
  if (_labS06) {
    passesFade = function (t) {
      ...
      if (b0 && b0.ok && (b0.reason === "out_of_range_fallback" || b0.reason === "bad_mode_fallback")) _s6FallbackSet.add(_kellyBaseKey(t, fIdx));  // 兜底过滤轻标注计数(覆盖期外/基座未知)
  ...
  // S06 态: a9 基座含 bullAuxBackupStop、new14 不含 → A-F 桶 per-date 天然按日生效; G/H/I 长线恒豁免,
  ...
  if (_labS06) {
    var _s6NoBullCache = {};   // baseId -> 键集−牛停 共享只读变体(a9/new14 至多两份)
    ...
    passesFadeNoBull = function (t) {
      ...
      if (b.reason === "out_of_range_fallback" || b.reason === "bad_mode_fallback") _s6FallbackSet.add(_kellyBaseKey(t, fIdx));  // 兜底过滤轻标注计数(与主谓词同 Set, 覆盖期外/基座未知)
  ...
  if (_labS06) {
    ...
    if (_s6FallbackSet.size > 0) {
      ...
      _s6WarnParts.push("· " + _s6FallbackSet.size + " 笔超出 S06 快照覆盖期/基座未识别（快照自 " + (_fs6.coverageStart || "?") + " 起），已按快照记录基座的键集/防守兜底 NEW14·14键 规则过滤，非逐日动态切换；等快照覆盖期前滚补齐这段，这里会自动消失。举例：比快照最早覆盖日（" + (_fs6.coverageStart || "?") + "）更早的老行情、或快照某日生效基座读不出来（如换基座前旧快照）时，系统没把握逐日判风格，就按该基座键集（识别不了再按防守兜底 NEW14 的 14 个键）判断");
    }
...
async function renderSigKellyLab() {
  ...
  // 不存在"老用户localStorage旧filters", AI宏默认每次加载即生效。与 localStorage 相关的只有 positionCap/K(tds_poscap_lab, 独立键, 与 app.js 首页各自独立互不联动, 2026-08-30 用户拍板)。
  ...
  // 2026-08-12 默认开启 positionCap(用户:默认最优组合要开启): 首次访问(无 tds_poscap_lab)写入默认{on:true,k:3}(2026-08-13 K默认3), 独立键与 app.js 首页互不联动(2026-08-30 用户拍板)
  if (!localStorage.getItem("tds_poscap_lab")) _kellySetSharedPosCap(true, 1); // 默认 K=1 主推(2026-08-14 #BC)
  state.labSigKellyFilters.positionCap = !!(_sharedPC && _sharedPC.on);
  state.labSigKellyFilters.positionCapK = (_sharedPC && _sharedPC.k) || 3;
  // #49 ai长线模式(G/H/I)仓位管理: 开关写独立键 tds_gihpos_lab(默认开 2026-08-30; 只影响实验室 G/H/I 卡片, 与首页 sim 弹窗开关互不联动)
...
// 2026-09-11 修: 切片不足 N 时不再静默无声——曲线图上方加「共 X 快照日 · 生效范围」说明行,
// 不足 N 时 foot 提示「等快照累积到 N 日后自动生效」, 用户点范围按钮必有明确反馈。
function _labKellyEvoCurveHTML(idx, mode, range) {
  const days = (idx && idx.days) || [];
  const want = range === "20" ? 20 : (range === "60" ? 60 : days.length);
  const showAll = want >= days.length;
  const slice = showAll ? days : days.slice(-want);
  let rangeNote;
  if (range === "all") {
    rangeNote = "显示全部(全史)";
  } else if (showAll) {
    rangeNote = `近 ${want} 日(快照仅 ${days.length} 日, 显示全部)`;
  } else {
    rangeNote = `近 ${want} 日(共 ${days.length} 日, 取最近 ${want} 日)`;
  }
  const hint = (range !== "all" && showAll)
    ? `<div class="lab-kelly-evo-range-hint" style="font-size:11px;color:var(--text-3,#999);margin:2px 0 0">💡 快照仅 ${days.length} 日, 近 ${want} 日范围等快照累积到 ${want} 日后自动生效。</div>`
    : "";
  return `<div class="lab-kelly-evo-range-note" style="font-size:11px;color:var(--text-3,#999);margin:2px 0 4px">共 ${days.length} 快照日 · ${rangeNote}</div>` +
    _labKellyEvoSVG(slice, mode) +
    hint;
}
...
async function _labKellyEvoBuildTable(overlay, idx) {
  ...
  if (idx && idx.days) {
    for (var i = 0; i < idx.days.length; i++) {
      var dd = idx.days[i];
      if (!polluted[dd.d]) polluted[dd.d] = {};
      var md = dd.modes || {};
      if (dd.polluted) { for (var mkx in md) polluted[dd.d][mkx] = 1; }
      for (var mk in md) { if (md[mk] && md[mk].polluted) polluted[dd.d][mk] = 1; }
    }
  }
...
function _labKellyEvoPinRowHTML(modeKeys, finalStats) {
  ...
  for (var mi = 0; mi < modeKeys.length; mi++) {
    ...
    if (!st) { h += `<td title="无数据">—</td>`; continue; }
    ...
    h += `<td class="lab-kelly-evo-cell" title="当前全量: 净利 ${(tp >= 0 ? "+" : "") + tp.toFixed(0)} 元 · 峰值资金收益率 ${(rmh >= 0 ? "+" : "") + rmh.toFixed(2)}% · ${st.n || 0} 笔(含未平仓按最新价预估), 与全信号卡同源">` +
...
function _labKellyEvoRenderTable(host, evo, polluted, gran) {
  ...
  // 2026-09-10 问题4: 最新日期置顶——降序遍历(最新→旧), 近 N 行=最近 N 个轴点保持正确
  for (var i = total - 1; i >= start; i--) {
    var r = rows[i];
    var rowPoll = (polluted && polluted[r.d]) || null;
    var allPolluted = rowPoll && Object.keys(rowPoll).length >= modeKeys.length;
    body += `<tr${allPolluted ? ' class="lab-kelly-evo-polluted"' : ""}>` +
...
function _renderSigKellyBar(bar, data, period) {
  ...
  const fadeHowHTML =
    `<div class="lab-sigkelly-fade-how">` +
      `<span class="lab-sigkelly-fade-how-row lab-sigkelly-fade-how-ok"><b>✅ 默认即可</b> 默认基座已切 S06·大盘领先切换(v1.1.7 起, 动态按日切 A进攻王/NEW14 键集+AI仓位K=1主推), 别动下面开关; 信号枯竭≥20交易日会出提示 chip</span>` +
  ...
  const aiMacroResetBtnHTML =
    `<button type="button" class="lab-sigkelly-toggle-detail-btn" id="lab-kelly-ai-macro-reset" style="margin-left:8px;padding:2px 10px;border:1px solid #888;border-radius:4px;background:transparent;cursor:pointer;color:inherit" title="一键恢复 AI 默认推荐勾选(v1.1.7 起=当前默认基座 S06 动态模式, 按大盘风格按日切 A进攻王/NEW14, 含 1类回测剔除, 其中+1类只读不可勾选; + AI仓位建议 K=1), 重写本地记忆并刷新统计">重置为AI默认推荐</button>`;
  ...
  // #49+#xx ai长线模式(G/H/I)仓位管理: 按钮(长线族群总入口, 默认开, v2 三模式独立策略; 架构支持后续按模式独立换策略)
  // 数据定案(2026-08-30 用户三连拍板): G=P≤3d(P法)@10万 / H=满仓不买(A法)@5万 / I=P≤3d(P法)@9万; I 原"唯一16/16年全正"作废
  // 真实权威数(2026-08-30 拍板, 废弃 b0/b1 估算): 强平日卖出按真实净值价重算, 表格 b0/b1 两格同值=真实权威数
  const _gihOn = !!state.labSigKellyGihOn;
  const _gihGTierCur = _kellyGihGTier();
  // 对比表数据(新定案 2026-08-3 用户三连拍板, §21 公示; 三模式各自开仓位管位后收益, b0/b1 已统一为真实权威数)
  const _gihGTierB = {
    "10万": { b0: ["146.72%", "+146,718", "10万"], b1: ["146.72%", "+146,718", "10万"] } // 真实权威数(强平日真实净值重算, b0=b1)
  };
  const _gihRefRows = [
    { m: "G", mName: "G · 卖出信号中长线", strat: "P≤3d " + _gihGTierCur,
      b0: _gihGTierB[_gihGTierCur].b0, b1: _gihGTierB[_gihGTierCur].b1 },
    { m: "H", mName: "H · 卖出+追止损中长线", strat: "满仓不买@5万",
      b0: ["230.83%", "+115,416", "5万"], b1: ["230.83%", "+115,416", "5万"] },
    { m: "I", mName: "I · 追关注加追止损中长线", strat: "P≤3d@9万(手段P)",      b0: ["159.63%", "+143,670", "9万"], b1: ["159.63%", "+143,670", "9万"] }
  ];
  // #88(2026-08-15): G/H/I 对比表改「横向布局」——三模式(G/H/I)作为列横向并排铺满宽度, 行方向=「场景(开b0/开b1)×指标(收益率/净利/所需本金)」。
  //   2026-08-30 清理: 移除「关(基线)」行(旧基座 raw 已清), 只保留开仓位管位后 b0/b1 新定案数字。
  const _gihColHead =
  ...
  // 场景行间隔分组: 每组内 3 个指标行连续排列, 一一对应 3 模式同列; 仅 b0/b1 两场景
  ...
  const _gihCompareTableHTML =
    ...
    `</table>`;
  // tooltip 白话文案(v2.2, 按模式分写——G=P≤3d@10万/H=满仓不买@5万/I=P≤3d@9万, 2026-08-30 用户+数据定案)
  const _gihTipG = "【G】定案档位=" + _gihGTierCur + " → " + _kellyGihStratShort("G") + "「先卖年轻仓」: 超仓先卖持有≤3天新仓(砍掉刚买没攒利润的), 保21-100天利润引擎, 无年轻仓才卖最老。真实权威数(2026-08-30 拍板, 2026-09-03 重算)收" + _gihGTierB[_gihGTierCur].b1[0] + "/净" + _gihGTierB[_gihGTierCur].b1[1] + ", 峰10笔=10倍本金可操作, 年化6.00%, 强平375笔, 负年2011/2021/2023。P≤3d 全面超旧FIFO(15起始年全胜、随机30点0/30负)。⚠诚实标注(2026-08-30 用户拍板): P≤3d 强平结果随资金量不稳定(同池扫描 3万档强平合计+20,285反盈 → 10万档 -31,547亏损), 且操作复杂需严格执行「先卖年轻仓」规则、执行有偏差即结果不稳定——保留为可选档, 主推 H(满仓不买, 见 H 档)。定案只保留 10万单档(峰持仓10倍本金=可操作)。";
  const _gihTip =
    "⭐ ai长线模式(G/H/I)仓位管理(默认开, 2026-08-30 用户三连拍板): 对 G/H/I 三长线模式各配独立仓位策略(不再统一FIFO)——最终落地为三模式各自最优：\n" +
    _gihTipG + "\n" +
    "【H·全场主推】满仓不买@5万(手段A): 到5万就停买、不强制平仓等自然卖出腾位再买——实际卖出=自然卖出(卖出信号/追止损触发), 无强平=完全确定, 收益率全场最高 230.83%/净+115,416, 峰5笔=5倍本金(超低本金充分可操作), 年化7.90%, 负年2023。主推两条硬道理(2026-08-30 用户拍板): ①收益率最高 230.83% 且无强平=结果完全确定; ②操作最简单——满仓不买、无强平规则, 实盘执行不混乱。\n" +
    "【I】P≤3d@9万(手段P, 2026-08-30 由A法改P法): 超9万先卖持有≤3天年轻仓, 无年轻仓才卖最老。真实权威数 159.63%/净+143,670, 峰9笔=9倍本金, 年化6.35%, 强平358笔, 负年2021/2023(原「唯一16/16年全正」2026-08-30 真实价重算后作废)。⚠非主推理由(2026-08-30 用户拍板): P≤3d 强平结果随资金量不稳定(3万档强平反盈+20,285 → 10万档巨额-31,547)、操作复杂需严格执行先卖年轻仓规则, 执行有偏差即结果不稳定——保留为可选档, 明确主推 H。\n" +
    "真实权威数口径(2026-08-30 拍板, 2026-09-03 重算, 废弃 b0/b1 估算): 仅 P 手段(G/I)有强平日——强平日真实卖出按当日日级真实净值价重算(G 3负年2011/2021/2023、I 2负年2021/2023, 强平卖持有≤3天年轻仓多为实亏, 故真实数低于原估算); H(手段A)无强平=完全确定, 全场主推。\n" +
  ...
  const aihlineCompareHTML =
    `<div id="lab-kelly-gih-compare-body" class="lab-sigkelly-ai-macro-body" style="${_gihCompareOpen ? "" : "display:none"}">` +
      `<div class="lab-sigkelly-toggle-group lab-sigkelly-toggle-group-poscap">` +
        `<div class="lab-sigkelly-advice-li lab-sigkelly-advice-note">G/H/I 仓位管理 · 三档定案对比(推荐 K=1 口径, 2026-08-30 用户三连拍板, b0/b1 估算废弃)。G=P≤3d先卖年轻仓@10万(真实权威数 146.72%/+146,718/年化6.00%/强平375笔/负年3个2011/2021/2023), H=满仓不买@5万(真实权威数 230.83%/+115,416/年化7.90%/无强平完全确定, ✩全场主推), I=P≤3d@9万(真实权威数 159.63%/+143,670/年化6.35%/强平358笔/负年2个2021/2023)。收益率=净利÷峰值占用资金; 真实权威数=强平日按当日日级真实净值价重算(G/I 原"16/16全正"已作废)。✩H 主推两条硬道理(2026-08-30 用户拍板): 收 230.83% 全场最高 + 无强平=完全确定; 操作最简单=满仓不买、无强平规则, 实盘执行不混乱。<span class="lab-sigkelly-advice-warn-red">⚠G/I(P3d)红色警示——结果稳定性存疑+实操性差(2026-08-30 用户拍板): 强平盈亏随资金量剧烈波动(同池扫描 3万档强平+20,285 → 10万档 -31,547, 同方法不同资金量结果不一致), 且需严格执行「先卖≤3天年轻仓」规则操作复杂易执行偏差致结果不稳定——方法价值保留为可选档, 明确不主推, 主推详见 H。</span>【核实源:2026-09-03 GHI 真实权威数重算 + docs/kelly/position/kelly-ghi-avsp-method-sweep.md】。</div>` +
        `<div class="lab-sigkelly-table-scroll">${_gihCompareTableHTML}</div>` +
        `<div class="lab-sigkelly-advice-warn-red">诚实标注(2026-08-30 拍板真实权威数, 2026-09-03 重算): G/I 的 P≤3d 全面超旧 FIFO(15起始年全胜/随机0/30负, 强平的为新仓未攒利润), 但真实价口径下原"唯一16/16全正"作废——G 3 个负年(2011/2021/2023)、I 2 个负年(2021/2023)、H 1 个负年(2023); 强平卖持有≤3天年轻仓多为实亏, 故真实收益低于原估算; <span class="lab-sigkelly-advice-warn-red">⚠G/I(P3d)红色警示——结果稳定性存疑+实操性差(2026-08-30 用户拍板): 强平盈亏随资金量剧烈波动(同池扫描 3万档强平反盈+20,285 → 10万档巨额-31,547, 同方法不同资金量结果不一致), 且需严格执行「先卖≤3天年轻仓」规则操作复杂易执行偏差致结果不稳定——方法价值保留为可选档, 明确不主推; ✿主推 H(手段A)无强平=完全确定且收益率全场最高+操作最简单(满仓不买, 实盘执行不混乱), 峰5笔超低本金。</span> 三模式峰持仓≤档位(10/5/9笔)均≤20倍本金=可操作。</div>` +
      `</div>` +
    `</div>`;
  ...
  // 2026-08-13 调序+OFF: K按钮组加 OFF(写 tds_poscap_lab {on:false} 退化普通列表, 再点某 K 档恢复), 独立键与首页/交易页各自独立互不联动(2026-08-30 用户拍板)
  ...
  bar.querySelectorAll(".lab-sigkelly-kbtn").forEach(function (btn) {
    btn.onclick = function () {
      ...
      if (btn.dataset.k === "off") {
        // OFF 按钮(2026-08-13, 复用首页同款交互): 写 tds_poscap_lab {on:false}, 关闭 AI仓位建议 → 该区退化普通列表; 再点某 K 档恢复
  ...
  // #49+#xx ai长线模式(G/H/I)仓位管理: 开关(默认开 2026-08-30, 只影响 G/H/I) → 写独立键 tds_gihpos_lab + 重算(套各模式独立仓位策略) + 刷新对比表; A-F 不受影响
  ...
  if (_fadeModeSelEl) _fadeModeSelEl.onchange = function () {
    ...
    if (_pSel && _pSel.dynamic) {
      ...
      _kellyToast(mid === "s06p1"
        ...
        : "已切到 S06 · 大盘领先切换(v1.1.7 起默认基座): 按日动态切 A进攻王/NEW14 基座, 标签区为参考底座");
      return;
    }
  ...
  if (_aiResetBtn) {
    _aiResetBtn.addEventListener("click", function () {
      ...
      // v1.1.7: 重置同步回落当前默认基座 s06 动态模式(按大盘风格按日切 A进攻王/NEW14), 模式记忆一并重写(TTL 工具, ts 刷新=重置也续期)
...
var _KELLY_REPORTS = {
  ...
  "kelly-ghi-continuous-cap-sweep": {
    ...
    summary: "G/H/I 连续 cap 档位扫描 + 时间稳健性验证(2026-08-30 用户+数据定案覆盖旧结论): H 定案=满仓不买@5万, I 定案=P≤3d@9万(P法, 由A法改), 旧 H@7万/I@16万 作废。",
    toc: ["0 摘要(核心答案 2026-08-30 用户+数据定案覆盖旧结论)", "1 连续 cap 扫描(H/I × 5-20万 每1万)", "2 定案解析(2026-08-30: H=满仓不买@5万 / I=P≤3d@9万 取代旧 H@7万/I@16万)", "3 时间稳健性验证(排除偶然)", "4 结论与用户决策点+诚实标注"]
  },
  ...
  return (
    `<div class="lab-sigkelly-advice">` +
      ...
      `<details class="lab-sigkelly-advice-details lab-sigkelly-advice-outer"${state.labSigKellyAdviceOpen ? " open" : ""}>` +
        `<summary><span class="lab-sigkelly-advice-summary-short">🎯 全信号操作建议指南（v1.1.7 起默认基座=S06 动态 · G玩法P≤3d可操作）</span><span class="lab-sigkelly-advice-summary-full">🎯 全信号操作建议指南（v1.1.7 起默认基座=S06 动态, 快照日切 A进攻王/NEW14 键集；NEW14 为 v1.1.5~v1.1.6 历史可回选档：hist6+规则8 降亏键保留入样 + 1回测剔除波动相关/未入样本信号；G玩法P≤3d「先卖年轻仓」三档可操作）</span></summary>` +
        ...
        `<div class="lab-sigkelly-advice-body">` +
          ...
          `<div class="lab-sigkelly-advice-li"><b>三玩法并列</b>（实时随上方降亏组合勾选 / 费率档联动，全周期 all 口径（与下方总建议一致）；下方「最后结果」卡随周期切换，切到「全部」时同值；金额口径=每日资金池等分+top-K）：A=固定10天短线（快进快出）；K=固定9天短线（9天档，比 A 少持 1 天，与10天接近：全史略高、近5年略低）；F=持有15天短线；J=固定20天短线；G=卖出信号长线（指数卖出信号触发离场、无信号持有至回测结束）；长线主推见「总建议」行=H（满仓不买@5万，G/I 为可选档；G 用户建议开上方「ai长线模式(G/H/I)仓位管理」套 P≤3d 可操作档）。</div>` +
          ...
          `<table class="lab-sigkelly-table lab-sigkelly-advice-table"><thead><tr><th>投资习惯</th><th>建议</th><th>真实回测数据</th></tr></thead><tbody>` +
            ...
            `<tr class="lab-sigkelly-advice-hl"><td><b>总建议</b></td><td><b>全信号都看 + 完全遵守交易页面展示的交易方法（卖出信号 H 模式）</b></td><td>AI仓位建议 K=1（主推，每日池+费率重算口径）下，长线三模式 ai长线<u>主推 H（满仓不买@5万）</u>：真实权威数 收 <b>230.83%</b>、净 <b>+115,416</b>、峰 5笔=5倍本金可操作、年化 7.90%，无强平=完全确定，理由=两条硬道理（2026-08-30 用户拍板）：①收益率全场最高 230.83% 且无强平=结果完全确定；②操作最简单=满仓不买、无强平规则，实盘执行不混乱；G=P≤3d@10万 146.72%/+146,718、I=P≤3d@9万 159.63%/+143,670（见下方 ai长线 对比表，原 b0/b1 估算与"16/16全正"已作废）——<span class="lab-sigkelly-advice-warn-red">⚠G/I(P3d)红色警示——结果稳定性存疑+实操性差（2026-08-30 用户拍板）：强平盈亏随资金量剧烈波动（同池扫描 3万档强平+20,285 → 10万档 -31,547，同方法不同资金量结果不一致）+操作复杂需严格执行先卖年轻仓规则，方法价值保留为可选档、明确不主推</span>；按年（真实权威数口径）：G 3 负年（2011/2021/2023），I 2 负年（2021/2023），H 1 负年（2023）</td></tr>` +
          `</tbody></table>` +
          ...
          `<div class="lab-sigkelly-advice-li lab-sigkelly-gmethod"><b>定案档位（2026-08-30 用户三连拍板=只保留 10万单档；数字为真实权威数，废弃 b0/b1 估算）</b>：<b>G P≤3d@10万</b> = 146.72%（净 +146,718，峰10笔=10 倍本金=可操作，年化 6.00%，强平375笔，负年2011/2021/2023）；旧 13万档 188.88%/+245,538（v1.1.4 八键基座历史口径）与旧 189.29% 估算已作废——「10万份本金内先卖年轻仓」既压住可操作性（10 倍）又吃满收益率，数据定案后 15/20 万选项早删（可配合本面板「ai长线模式(G/H/I)仓位管理」开关联动看效果）。</div>` +
          `<div class="lab-sigkelly-advice-li lab-sigkelly-gmethod"><b>为什么可信（对比证明）</b>：P≤3d 在 若干档位收益率都高于旧 FIFO（卖最老＝卖掉了利润引擎本体）；15 个不同起始年 <b>全部</b>胜 FIFO（收益率均值 98.9% vs FIFO 62.0%）、随机 30 个起始点 <b>0/30 负</b>。且 P≤3d 强平的正好是 0-3 天新仓（还没累积利润）→ 强平日按真实净值价重算（2026-08-30 拍板真实权威数）＝<b>数字即真实、不再依赖估算模型</b>；反观旧 FIFO 强平的是最老仓（平均已持 73 天、自然利润合计 +45 万）→ 原估算区间宽 105pp 不可信。<span class="lab-sigkelly-advice-warn-red">⚠P3d 红色警示——结果稳定性存疑+实操性差（2026-08-30 用户拍板）：方法价值保留，但强平盈亏随资金量剧烈波动（同池扫描 3万档强平+20,285 → 10万档-31,547，同方法不同资金量结果不一致）+操作复杂需严格执行先卖年轻仓规则→明确不主推。</span><b>结论：G 用户若上仓位管理，用 P≤3d「先卖年轻仓」代替旧 FIFO（真实权威数 146.72%）；三模式中最稳最优推 H（满仓不买无强平 230.83%）——主推两条硬道理（2026-08-30 用户拍板）：①收益率最高且无强平=完全确定；②操作最简单=满仓不买、无强平规则，实盘执行不混乱。</b></div>` +
          `<div class="lab-sigkelly-advice-li lab-sigkelly-advice-note lab-sigkelly-gmethod">📌 G 方法三层流程（白话说一遍）：① 选组合=AI宏默认组合（S06 动态基座，以页面实时为准）；② G 玩法教学已定案（见上方「G 玩法完整交易方法」），2026-08-30 用户三连拍板 P≤3d@10万；③ 实盘仓位=每日池均分 + P≤3d「先卖年轻仓」@10万定案单档(2026-08-30 定案，15/20 档早删)。<span class="lab-sigkelly-advice-warn-red">⚠G/I(P3d)红色警示——结果稳定性存疑+实操性差（2026-08-30 用户拍板）：同方法不同资金量结果不一致（3万档强平反盈+20,285 → 10万档巨额-31,547）+操作复杂需严格执行先卖年轻仓规则，方法价值保留为可选档、明确不主推</span>；✿主推建议=H（满仓不买@5万，无强平 230.83% 全场最高+操作最简单，2026-08-30 用户拍板）；G 3 负年、I 2 负年。本段为研究结论（详见上方「G模式复核」报告），实际交易以页面「ai长线模式(G/H/I)仓位管理」开关勾选联动为准，仍需你盯盘确认信号。</div>` +
          ...
          `<div class="lab-sigkelly-advice-li lab-sigkelly-advice-note">⚠ 口径说明：本节「投资习惯」行为表格/总建议数字为<b>每日资金池等分+top-K（2026-08-13 恢复, 2026-08-14 #BC 对齐重算）口径</b>；页面实时 K 档评级/全信号表同为<b>每日资金池等分+top-K</b>，可对照。核心决策以每日池为准：A/F 短持→维持页面默认降亏组合；长线三模式 ai长线→主推 H（满仓不买@5万，无强平完全确定 230.83%+操作最简单，2026-08-30 用户拍板），G=P≤3d@10万 可操作档、I=P≤3d@9万 为备选（真实权威数；<span class="lab-sigkelly-advice-warn-red">⚠G/I(P3d)红色警示——结果稳定性存疑+实操性差：强平随资金量剧烈波动（3万档+20,285→10万档-31,547）+操作复杂，非主推</span>）。上方 A/F/G 三玩法表为全周期 all 口径(每日池,实时联动)，下方「最后结果」全信号表随周期切换（切到「全部」时两表同值）。</div>` +
        `</div>` +
      `</div>` +
    `</details>` +
    `</div>`
...
function _sigKellyAfgRealtimeHtml() {
  ...
  const modes = [
    ...
    { key: "K", name: "K · 固定9天短线", desc: "买入后固定持有 9 天卖出(9天档)" },
  ...
  // v2.2(2026-08-30 用户+数据定案) 只保留 10万 定稿单档(15/20档死数据已删, _kellyGihGTier() 恒 "10万")
  const _KELLY_G_TIER_REF = {
    "10万": { b0: ["146.72%", "+146,718", "10万"], b1: ["146.72%", "+146,718", "10万"] } // 真实权威数(2026-08-30 拍板, 2026-09-03 重算, 强平日真实净值重算, b0=b1 同值)
  };
  for (const m of modes) {
    ...
    if (m.key === "G") {
      ...
      const _gRef = _KELLY_G_TIER_REF[_gTier] || _KELLY_G_TIER_REF["10万"];
      ...
      if (_gOp && _gOp.n) {
        ...
        _gRow = {
          ...
          tip: `G=P≤3d「先卖年轻仓」${_gTier}（真实权威数，峰持仓≤20倍本金=可操作）；样本/胜率/盈亏比见「G/H/I 对比表」`
        };
      }
  ...
  return (
    `<div class="lab-sigkelly-wm-pop">` +
      ...
      `<div class="lab-sigkelly-wm-sec">` +
        ...
        `<div class="lab-sigkelly-wm-li lab-sigkelly-wm-li-x">推荐规则: ①先看可操作性(峰值同时持仓≤20万=≤20倍单次本金, 不可操作模式不推荐) ②再看收益率(峰值资金收益率 return_pct_max_holding) ③净盈亏/最大持仓只是佐证, 不比排序。X = 可操作层中收益率最高的方案字母。例(G/H/I 模式开关对比, 决策链走①②③)：关(旧FIFO)峰值持仓 136 万=不可操作, 即便净利 +64.2万 全模式最高也先被①淘汰; 开(满仓不买@5万)峰值 5 万=可操作, 再按②收益率排, 230.83%(真实权威数) 胜出→推荐 H@5万(2026-08-30 用户三连拍板, 全场主推), 净利/持仓只当佐证不当排序【核实源:2026-09-03 GHI 三档真实权威数重算】</div>` +
...
function _renderSigKellyCard(qk, q, period, cardCmp) {
  ...
  for (const m of modes) {
    ...
    const _gihBadge = _kellyIsGih(m) && _gihOnThis && _gihRow ? `<span class="lab-sigkelly-gih-badge" title="ai长线模式仓位管理已开: 本行套「${_kellyGihStratShort(m) || ""}」仓位法(${_kellyGihStratExplain(m)})后的真实权威数口径(2026-08-30 拍板, 2026-09-03 重算, 废弃 b0/b1 估算; H=全程主推 230.83% 无强平完全确定+操作最简单)">${_kellyGihStratShort(m) || ""}</span>` : "";
    ...
    rows +=
      `<tr class="lab-sigkelly-trade-row${_opRowCls}" data-quad="${qk}" data-mode="${m}" data-period="${period}" data-opelim="${_opElim ? "1" : "0"}" title="${_opRowTip}">` +
        ...
        `<td class="lab-sigkelly-tp-hl ${tp >= 0 ? "lab-sigkelly-pos" : "lab-sigkelly-neg"}">${tpStr}元${_p3dWarnBadge}</td>` +
...
async function _openSigKellyTradesModal(quadKey, modeKey, period) {
  ...
  state._sigKellyTradeFilter = { etf: "", profit: "all", elimReason: "all" };
...
function _renderSigKellyTradesModal(overlay, trades, fields, quadLabel, modeLabel, period, quadKey, modeKey, eliminated, gihMeta) {
  ...
  if (!state._sigKellyTradeFilter) state._sigKellyTradeFilter = { etf: "", profit: "all", elimReason: "all" };
  ...
  function _render() {
    ...
    const _elimReasonOptionsHtml = (function () {
      const s = {};
      eliminated.forEach(function (t) { if (t._elimReason) s[t._elimReason] = 1; });
      const order = ["AI降亏", "AI仓位", "AI长线·满仓不买"];
      let html = "";
      order.forEach(function (r) {
        if (s[r]) html += `<option value="${r}"${filter.elimReason === r ? " selected" : ""}>${r === "AI降亏" ? "仅降亏" : (r === "AI仓位" ? "仅AI仓位建议" : "仅AI长线·满仓不买")}</option>`;
      });
      return html;
    })();
    ...
    if (eliminated.length > 0) {
      ...
      if (filter.elimReason && filter.elimReason !== "all") {
        elimFiltered = elimFiltered.filter(function (t) { return t._elimReason === filter.elimReason; });
      }
    ...
    // 需求④: 淘汰原因下拉(按原因过滤淘汰区+触发该类盈亏统计; 只重置淘汰区页码, 主表不受影响)
    const elimSel = overlay.querySelector(".lab-sigkelly-filter-elimreason");
    if (elimSel) {
      elimSel.onchange = () => { filter.elimReason = elimSel.value; state._sigKellyElimPage = 1; _render(); };
    }
    // 分页
...
// ============ 需求B: ETF 走势 + 买卖/强平 pin(2026-08-30) ============
// 点 ETF 代码单元格 → 弹独立小窗: 复用 app.js 全局 _etfTrendLiteHTML/_etfTrendLiteBind/_etfTrendGeom
// (app.js 同页加载, mangle keep_fnames 保留函数名; lab tab 下 app.min.js 已全局可用)。
// 数据源: static-site/data/etf/{code}-all.json {date, code, name, ohlc:[[YYYYMMDD,o,h,l,c],...]}
//   线上 R2 直链 https://ss.fx8.store/r2/etf/{code}-all.json; fallback ./data/etf/{code}-all.json。
// pin 口径: ohlc 为前复权、交易价为不复权 → pin 的 y 锚用该日期在 ohlc 上的 close 坐标
//   (落在曲线当日), 标签文本显示真实成交价(buy_price/sell_price/强平日实价), 不混用。
// 视图范围: 主表 trades + 被淘汰区 eliminated 中该 ETF 的所有已展示交易事件。
// 事件种类: buy(买点, buy_date) / sell(卖点, sell_date) / force(强平日, _gihForced 标记)。
const _LAB_ETF_PIN_URL = (code) => `https://ss.fx8.store/r2/etf/${code}-all.json`;
const _LAB_ETF_PIN_FALLBACK = (code) => `./data/etf/${code}-all.json`;

// 收集指定 ETF 在视图内(trades+eliminated)的全部交易事件 → [{date, kind, price, navMissing?}]
function _collectEtfPinEvents(code, fields, trades, eliminated) {
  const evs = [];
  const fIdx2 = {};
  ...
  const push = (t, src) => {
    ...
    if (sd) {
      ...
      const navMissing = !!t._gihNavMissing;
...
async function _openEtfTrendPinModal(code, name, trades, eliminated, fields, srcRow, srcKey) {
  ...
  // 独立小弹窗
  let overlay = document.getElementById("lab-etf-pin-overlay");
  if (!overlay) {
    overlay = document.createElement("div");
    overlay.id = "lab-etf-pin-overlay";
    overlay.className = "lab-sigkelly-overlay";
    document.body.appendChild(overlay);
  }
  ...
  if (reqSeq !== _labEtfTrendPinReqSeq) return; // 已关/已换弹窗, 丢弃过期响应
  if (!hist || !hist.ohlc || !Array.isArray(hist.ohlc) || hist.ohlc.length < 2) {
    body.innerHTML = `<div class="lab-custom-error"><div class="lab-custom-error-title">⚠️ 走势数据加载失败</div><div class="lab-custom-error-detail">${_esc(code)}-all.json 不可用（R2 与本地均失败）</div></div>`;
    return;
  }
 ...
 * 交互(2026-09-06 改提醒视图, 用户拍板替代 #108 全量 16 行): 主表=提醒视图, 只显示「最近未过期交易日 T0 + 下一交易日 T1」两天,
 *   每天内按买入/卖出分类(该日 N 只 ETF 买入 = N 行买入, M 笔卖出到期 = M 行卖出);
 *   更早全部历史收「📄 查看全部计划」弹窗(含未到期, 卖出按 sell_date 独立成行); 点行弹所属日全时间线;
 *   「现在该干嘛」提示条+当日行高亮 / 盘中60s盘后5min轮询就地更新
...
// 注(2026-09-06): 主表已改提醒视图, _atRemindDayRows 按 ETF 分组调用本函数生成该日各买入组摘要; 全量构建/「查看全部」仍走本函数
...
// 全量行集合 = 计划行(当日) + 步骤摘要行(全史) + 卖出独立行(按 sell_date, #108),
// 按 date DESC, 同日有步骤则以步骤摘要为准(补齐计划缺的 etf_name/amount);
// 卖出日与买入日撞日期时买入行+卖出行同时显示(不互斥, 用户拍板语义)
// 注(2026-09-06): 主表已改提醒视图(_atRender 直接用 _atRemindDayRows), 本函数仍供「查看全部计划」弹窗全量列表使用
...
function _atTradeDates(planDoc, stepsDoc) {
  const set = {};
  const byDate = _atStepsByDate(stepsDoc);
  Object.keys(byDate).forEach(function (d) { set[d] = 1; });
  Object.keys(byDate).forEach(function (d) {
    (byDate[d] || []).forEach(function (s) {
      if (s && s.sell_date) set[String(s.sell_date)] = 1;
    });
  });
...
function _atRemindPair(tradeDates, today, hm) {
  ...
  if (T0) {
    for (let i = 0; i < dates.length; i++) {
      if (String(dates[i]) > String(T0)) { T1 = dates[i]; break; }
    }
  }
  return { T0: T0, T1: T1 };
}
// 某交易日提醒行: buys=买入行(该日 N 只 ETF = N 行买入, 用户拍板语义), sells=卖出行(按 sell_date 独立成行)
// 不受 _atBuildDays 纯 date 键「每日 1 摘要」限制, 直接按 ETF 分组展开; 摘要逻辑复用 _atDaySummary
function _atRemindDayRows(date, planDoc, stepsDoc) {
  const byDate = _atStepsByDate(stepsDoc);
  const daySteps = byDate[String(date)] || [];
  const buys = [], sells = [];
  const groups = {};
  daySteps.forEach(function (s) {
    if ((s.action || "buy") === "sell") return; // 卖出行统一走下方全量扫描(卖出日可能非 byDate 键)
    const k = String(s.etf_code || "_");
    if (!groups[k]) groups[k] = [];
    groups[k].push(s);
  });
  // 卖出到期行: 全量扫描所有买入组的 sell_date == 该日(卖出日独立成日, 可能不在 byDate 键中)
  Object.keys(byDate).forEach(function (d) {
    (byDate[d] || []).forEach(function (s) {
      if ((s.action || "") === "sell" && s.sell_date && String(s.sell_date) === String(date)) {
        sells.push(_atSellRow(s, String(date)));
      }
    });
  });
  Object.keys(groups).forEach(function (k) {
    const sum = _atDaySummary(String(date), groups[k]);
    if (sum) buys.push(sum);
  });
  // 计划源补位(该日有 plan 行而无步骤摘要时, 补齐字段或独立入行)
  _atPlanRows(planDoc).forEach(function (r) {
    if (String(r.date) !== String(date)) return;
    const hit = buys.filter(function (b) { return String(b.etf_code) === String(r.etf_code); })[0];
    if (hit) {
      if (!hit.etf_name) hit.etf_name = r.etf_name;
      if (_atNum(hit.amount) == null) hit.amount = r.amount;
      if (!hit.trigger_note) hit.trigger_note = r.trigger_note;
    } else {
      buys.push(r);
    }
  });
...
function _atRemindGroupHtml(g, today, nowAct, byDate) {
  ...
  let rows = '<tr class="auto-trade-steps-daysep"><td colspan="7" style="' + daySepStyle + '">' +
...
function _atRender(slot, planDoc, stepsDoc) {
  ...
  // 「查看全部计划」: 全量历史列表弹窗(含未到期计划, 卖出独立成行)
  const moreBtn = slot.querySelector(".auto-trade-steps-more-btn");
  if (moreBtn) moreBtn.onclick = function () { _atOpenAllModal(); };
...
// ---- 「查看全部计划」弹窗(提醒视图历史收口, 2026-09-06 用户拍板): 全部历史计划完整列表 ----
// 含未到期计划, 卖出按 sell_date 独立成行; 点行打开所属买入组/所属日全时间线(复用 _atOpenModal/_atModalRender 能力)
function _atAllModalRender(overlay) {
  const days = _atBuildDays(_atPlanDoc, _atStepsDoc);
  const today = _atToday();
  const byDate = _atStepsByDate(_atStepsDoc);
  const nowAct = _atNowAction(byDate);
  ...
  const head = '<div class="lab-sigkelly-modal-head">' +
    ...
    '<span class="auto-trade-steps-sub">全部历史计划(含未到期)· 卖出按卖出日独立成行 · 点行看所属日全时间线 · 点 ETF 代码看走势</span>' +
    '<span style="flex:1"></span>' +
    '<button class="lab-sigkelly-modal-close">✕</button>' +
    '</div>';
  const body = rowsHtml
    ? '<div class="lab-sigkelly-modal-tablewrap"><table class="lab-sigkelly-trades-table auto-trade-steps-modal-table">' +
      '<thead><tr><th>日期</th><th>计划动作</th><th>挂单价</th><th>金额/份数</th><th>状态</th><th>现价/成交</th><th>触发条件摘要</th></tr></thead>' +
      '<tbody>' + rowsHtml + '</tbody></table></div>'
    : '<div class="auto-trade-steps-empty">暂无实操计划。</div>';
  overlay.innerHTML = '<div class="lab-sigkelly-modal auto-trade-steps-modal">' + head + body + '</div>';
  const closeBtn = overlay.querySelector(".lab-sigkelly-modal-close");
  if (closeBtn) closeBtn.onclick = function () { _atModalClose(); };
  overlay.querySelectorAll(".auto-trade-steps-row").forEach(function (row) {
    row.onclick = function () { _atOpenModal(row.getAttribute("data-open-date") || row.getAttribute("data-date")); };
  });
  overlay.querySelectorAll(".auto-trade-steps-etf-link").forEach(function (a) {
    a.onclick = function (e) {
