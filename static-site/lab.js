...
function _kellyGihStratExplain(modeKey) {
  ...
  if (key === "hold5w") return "满仓不买@5万: 到 5 万就停买、不强制平仓, 等有自然卖出腾出资金再买新信号(手段A, 无强平→完全确定 224.92%/+112,461, 全场主推)";
...
function _renderSigKellyBar(bar, data, period) {
  ...
  const _gihRefRows = [
    ...
    { m: "H", mName: "H · 卖出+追止损中长线", strat: "满仓不买@5万",
      b0: ["224.92%", "+112,461", "5万"], b1: ["224.92%", "+112,461", "5万"] },
  ...
  const _gihTip =
    ...
    _gihTipG + "\n" +
    "【H·全场主推】满仓不买@5万(手段A): 到5万就停买、不强制平仓等自然卖出腾位再买——实际卖出=自然卖出(卖出信号/追止损触发), 无强平=完全确定, 收益率全场最高 224.92%/净+112,461, 峰5笔=5倍本金(超低本金充分可操作), 年化7.90%, 负年2023。主推两条硬道理(2026-08-30 用户拍板): ①收益率最高 224.92% 且无强平=结果完全确定; ②操作最简单——满仓不买、无强平规则, 实盘执行不混乱。\n" +
  ...
  const aihlineCompareHTML =
    `<div id="lab-kelly-gih-compare-body" class="lab-sigkelly-ai-macro-body" style="${_gihCompareOpen ? "" : "display:none"}">` +
      `<div class="lab-sigkelly-toggle-group lab-sigkelly-toggle-group-poscap">` +
        `<div class="lab-sigkelly-advice-li lab-sigkelly-advice-note">G/H/I 仓位管理 · 三档定案对比(推荐 K=1 口径, 2026-08-30 用户三连拍板, b0/b1 估算废弃)。G=P≤3d先卖年轻仓@10万(真实权威数 146.72%/+146,718/年化6.00%/强平375笔/负年3个2011/2021/2023), H=满仓不买@5万(真实权威数 224.92%/+112,461/年化7.90%/无强平完全确定, ✩全场主推), I=P≤3d@9万(真实权威数 159.63%/+143,670/年化6.35%/强平358笔/负年2个2021/2023)。收益率=净利÷峰值占用资金; 真实权威数=强平日按当日日级真实净值价重算(G/I 原"16/16全正"已作废)。✩H 主推两条硬道理(2026-08-30 用户拍板): 收 224.92% 全场最高 + 无强平=完全确定; 操作最简单=满仓不买、无强平规则, 实盘执行不混乱。<span class="lab-sigkelly-advice-warn-red">⚠G/I(P3d)红色警示——结果稳定性存疑+实操性差(2026-08-30 用户拍板): 强平盈亏随资金量剧烈波动(同池扫描 3万档强平+20,285 → 10万档 -31,547, 同方法不同资金量结果不一致), 且需严格执行「先卖≤3天年轻仓」规则操作复杂易执行偏差致结果不稳定——方法价值保留为可选档, 明确不主推, 主推详见 H。</span>【核实源:2026-09-03 GHI 真实权威数重算 + docs/kelly/position/kelly-ghi-avsp-method-sweep.md】。</div>` +
      ...
      `<details class="lab-sigkelly-advice-details lab-sigkelly-advice-outer"${state.labSigKellyAdviceOpen ? " open" : ""}>` +
        ...
        `<div class="lab-sigkelly-advice-body">` +
          ...
          `<table class="lab-sigkelly-table lab-sigkelly-advice-table"><thead><tr><th>投资习惯</th><th>建议</th><th>真实回测数据</th></tr></thead><tbody>` +
            ...
            `<tr class="lab-sigkelly-advice-hl"><td><b>总建议</b></td><td><b>全信号都看 + 完全遵守交易页面展示的交易方法（卖出信号 H 模式）</b></td><td>AI仓位建议 K=1（主推，每日池+费率重算口径）下，长线三模式 ai长线<u>主推 H（满仓不买@5万）</u>：真实权威数 收 <b>224.92%</b>、净 <b>+112,461</b>、峰 5笔=5倍本金可操作、年化 7.90%，无强平=完全确定，理由=两条硬道理（2026-08-30 用户拍板）：①收益率全场最高 224.92% 且无强平=结果完全确定；②操作最简单=满仓不买、无强平规则，实盘执行不混乱；G=P≤3d@10万 146.72%/+146,718、I=P≤3d@9万 159.63%/+143,670（见下方 ai长线 对比表，原 b0/b1 估算与"16/16全正"已作废）——<span class="lab-sigkelly-advice-warn-red">⚠G/I(P3d)红色警示——结果稳定性存疑+实操性差（2026-08-30 用户拍板）：强平盈亏随资金量剧烈波动（同池扫描 3万档强平+20,285 → 10万档 -31,547，同方法不同资金量结果不一致）+操作复杂需严格执行先卖年轻仓规则，方法价值保留为可选档、明确不主推</span>；按年（真实权威数口径）：G 3 负年（2011/2021/2023），I 2 负年（2021/2023），H 1 负年（2023）</td></tr>` +
          `</tbody></table>` +
          ...
          `<div class="lab-sigkelly-advice-li lab-sigkelly-gmethod"><b>为什么可信（对比证明）</b>：P≤3d 在 若干档位收益率都高于旧 FIFO（卖最老＝卖掉了利润引擎本体）；15 个不同起始年 <b>全部</b>胜 FIFO（收益率均值 98.9% vs FIFO 62.0%）、随机 30 个起始点 <b>0/30 负</b>。且 P≤3d 强平的正好是 0-3 天新仓（还没累积利润）→ 强平日按真实净值价重算（2026-08-30 拍板真实权威数）＝<b>数字即真实、不再依赖估算模型</b>；反观旧 FIFO 强平的是最老仓（平均已持 73 天、自然利润合计 +45 万）→ 原估算区间宽 105pp 不可信。<span class="lab-sigkelly-advice-warn-red">⚠P3d 红色警示——结果稳定性存疑+实操性差（2026-08-30 用户拍板）：方法价值保留，但强平盈亏随资金量剧烈波动（同池扫描 3万档强平+20,285 → 10万档-31,547，同方法不同资金量结果不一致）+操作复杂需严格执行先卖年轻仓规则→明确不主推。</span><b>结论：G 用户若上仓位管理，用 P≤3d「先卖年轻仓」代替旧 FIFO（真实权威数 146.72%）；三模式中最稳最优推 H（满仓不买无强平 224.92%）——主推两条硬道理（2026-08-30 用户拍板）：①收益率最高且无强平=完全确定；②操作最简单=满仓不买、无强平规则，实盘执行不混乱。</b></div>` +
          `<div class="lab-sigkelly-advice-li lab-sigkelly-advice-note lab-sigkelly-gmethod">📌 G 方法三层流程（白话说一遍）：① 选组合=AI宏默认组合（S06 动态基座，以页面实时为准）；② G 玩法教学已定案（见上方「G 玩法完整交易方法」），2026-08-30 用户三连拍板 P≤3d@10万；③ 实盘仓位=每日池均分 + P≤3d「先卖年轻仓」@10万定案单档(2026-08-30 定案，15/20 档早删)。<span class="lab-sigkelly-advice-warn-red">⚠G/I(P3d)红色警示——结果稳定性存疑+实操性差（2026-08-30 用户拍板）：同方法不同资金量结果不一致（3万档强平反盈+20,285 → 10万档巨额-31,547）+操作复杂需严格执行先卖年轻仓规则，方法价值保留为可选档、明确不主推</span>；✿主推建议=H（满仓不买@5万，无强平 224.92% 全场最高+操作最简单，2026-08-30 用户拍板）；G 3 负年、I 2 负年。本段为研究结论（详见上方「G模式复核」报告），实际交易以页面「ai长线模式(G/H/I)仓位管理」开关勾选联动为准，仍需你盯盘确认信号。</div>` +
          ...
          `<div class="lab-sigkelly-advice-li lab-sigkelly-advice-note">⚠ 口径说明：本节「投资习惯」行为表格/总建议数字为<b>每日资金池等分+top-K（2026-08-13 恢复, 2026-08-14 #BC 对齐重算）口径</b>；页面实时 K 档评级/全信号表同为<b>每日资金池等分+top-K</b>，可对照。核心决策以每日池为准：A/F 短持→维持页面默认降亏组合；长线三模式 ai长线→主推 H（满仓不买@5万，无强平完全确定 224.92%+操作最简单，2026-08-30 用户拍板），G=P≤3d@10万 可操作档、I=P≤3d@9万 为备选（真实权威数；<span class="lab-sigkelly-advice-warn-red">⚠G/I(P3d)红色警示——结果稳定性存疑+实操性差：强平随资金量剧烈波动（3万档+20,285→10万档-31,547）+操作复杂，非主推</span>）。上方 A/F/G 三玩法表为全周期 all 口径(每日池,实时联动)，下方「最后结果」全信号表随周期切换（切到「全部」时两表同值）。</div>` +
        `</div>` +
      `</div>` +
    `</details>` +
    `</div>`
  ...
  return (
    `<div class="lab-sigkelly-wm-pop">` +
      ...
      `<div class="lab-sigkelly-wm-sec">` +
        ...
        `<div class="lab-sigkelly-wm-li lab-sigkelly-wm-li-x">推荐规则: ①先看可操作性(峰值同时持仓≤20万=≤20倍单次本金, 不可操作模式不推荐) ②再看收益率(峰值资金收益率 return_pct_max_holding) ③净盈亏/最大持仓只是佐证, 不比排序。X = 可操作层中收益率最高的方案字母。例(G/H/I 模式开关对比, 决策链走①②③)：关(旧FIFO)峰值持仓 136 万=不可操作, 即便净利 +64.2万 全模式最高也先被①淘汰; 开(满仓不买@5万)峰值 5 万=可操作, 再按②收益率排, 224.92%(真实权威数) 胜出→推荐 H@5万(2026-08-30 用户三连拍板, 全场主推), 净利/持仓只当佐证不当排序【核实源:2026-09-03 GHI 三档真实权威数重算】</div>` +
...
function _renderSigKellyCard(qk, q, period, cardCmp) {
  ...
  for (const m of modes) {
    ...
    const _gihBadge = _kellyIsGih(m) && _gihOnThis && _gihRow ? `<span class="lab-sigkelly-gih-badge" title="ai长线模式仓位管理已开: 本行套「${_kellyGihStratShort(m) || ""}」仓位法(${_kellyGihStratExplain(m)})后的真实权威数口径(2026-08-30 拍板, 2026-09-03 重算, 废弃 b0/b1 估算; H=全程主推 224.92% 无强平完全确定+操作最简单)">${_kellyGihStratShort(m) || ""}</span>` : "";
