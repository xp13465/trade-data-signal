...
**最后更新**:2026-09-07 14:4x(**收尾落档完成,可重启代理换新会话**).**✅ sim-price-toggle 已 main-merge 并推 main(d1bc6af84,统一 bump a554)+changelog 补登 a554 完成(5685e68f9);17:50 update_all 自动 deploy 上线,18:05 兜底 cron ffa63f46 验证+打 v1.1.17 tag**。reviewer **PASS** 结论:核心全对(§22 切档/独立键/上下换行/回归/公示/§5.4⑦ 对账逐位一致),2 非阻塞建议已交用户拍板=**带上线后修,已落 TaskList #72**:F1(置信75)切档竞态 `_simSetBuyBasis` 未重置 `_simKellyLoading`(app.js L3547);F2(置信80)前端交易日历近似(trades 并集)vs 后端真实价格日期,稀疏年展示错位 1 天,**用户拍板 F2=后端补真实买入交易日字段**(改 signal_kelly_backtest.py 数据生成)。**⚠️ 代理重启:时点由用户拍板,主控不得自行动手**(#48/#59 merge 后重启代理=断当前 8899 通道,重启前确认用户在场)。**在跑(用户拍板已派 09-07 15:0x)**:#48 P0-3 代理高可用+#59 商汤 7key **打包 implementer a4a42cc7**(feat/proxy-rework,巡检 cron 16b53b4e 盯 /tmp/agent-progress-proxy-rework.md;范围=OUT_OF_RANGE 重放定位(§6.2 旧归因已证伪)+enabled 型归一+healthz+KEY1-7 轮询(KEY6/7 已入库)+冷却持久化+jitter+停 kimi;**merge 后重启代理=等用户时点,断 8899 会话前确认在场**)。**FAPI 观察期 cron 1e0ef6fb 09-08 到期(周一收尾一次)**。TaskList:#71 打 tag in_progress(ffa63f46 兜底 18:05)、#72 F1/F2 待修、#48/#59 pending。✅ #101 北交所宽度=已拍板C+已完成上线(残留指针已清 264dfad9d)。待用户确认:本地审计清理清单 docs/local-state-audit-20260903.md。
