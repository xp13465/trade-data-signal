...
**最后更新**(2026-09-12 周六 15:2x):✅ **8 项 reviewer 非阻断小项打包收尾上线**(feat/reviewer-pack8-cleanup,reviewer 7 PASS+1 FAIL(报告"50倍余量"数字不自洽)→修复 9b10e9e77 复核,main-merge **20260912-a585** 上线,§0 三查 PASS):**演进弹窗 3 小项** ①Playwright 脚本 probe-intraday-fade0.mjs 迁 docs/kelly/analysis/scripts/(docstring+报告引用同步)②嵌套空目录 static-site/data/static-site/ 清理 ③sigkelly-snapshot-pollution-fix 报告措辞修正(50倍→7倍余量,数字自洽可核);**实操步骤 P2 5 项** ④**PRD 卖出日 D+10 修正(核心定案)**:真实产物 159023 卖出日=20260918 一锤定音,权威口径=信号日后第10交易日(回测 A 模式 hold_days=10),PRD L427/§1.2 L64/生成器 docstring/示例 json 全链路无 9/21 残留 ⑤_atTimeHint 与 bar 一致性(卖出行报「卖出日 X」+全闭环收口)⑥check_data_integrity 补 seq 链完整性校验(1/2/3/5,纯卖出豁免,8 执行日不误卡)⑦R2 空计划日缺口 verify 已修不复发(steps_changed 判定,销号)⑧迁移补行补 backfilled 标记+tooltip「阶段一干跑不回写执行状态」。**✅ README L72 回填角标描述同步 2.5 语义**(164cdd3b7,push main)。前序:**✅ #91 次日开盘口径核销完成**(7676baad2,非待拍板:v1.1.4 默认+前端双档按钮已上线)+**✅ TASKS 交接段瘦身**(e80dc015f,09-06~09-11 详述折叠归档 docs/archive/TASKS-handoff-20260912.md)。**✅ 实操步骤提醒视图两轮上线**(feat/steps-reminder-view,a583/a584,周六 T0=上周五+下周一修复)。**待办指针**:②#98 CodeGraph ui 待官方发包复查;③#65 四档升级判定源 hs300→cyb 待用户拍板;⑥待观察:FAPI 概念观察期 cron 1e0ef6fb 到期收尾、22:35 check_data_gap 告警自动恢复确认、20:55 生成器连续生产跑稳定性观察。
