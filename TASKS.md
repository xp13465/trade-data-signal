...
**最后更新**:2026-08-12 中午(**4 agent 并行:own-feishu review/SVG4fix实施/17:50时点调研/凯利reviewer;飞书抄送根因定位+修复已推feat;§18归属+claude-work-mode+凯利实施全DONE;等3 PASS 后统一 merge main**)。**本轮**:
- **飞书抄送异常根因定位+修复已推 feat**(2817d8bd0,用户报"轮询被抄送+我发的svg/申万两消息和你的回复没同步"): 根因①主控 cron 轮询 prompt 触发 UserPromptSubmit 被当"用户消息"误抄→SKIP_PROMPT_PREFIXES("轮询"/[system等)过滤 ②长 turn(盘中打断/cron注入)期间用户真实消息+我的回复无独立 hook 事件→漏抄→_sweep_unforwarded 每次任意 hook 触发后补扫 transcript 尾部 120 条未抄送的 user/assistant 文本,指纹与主 handler 同公式互斥去重;dry-run 3 测试过(识别✓/二次去重✓/轮询跳过✓)+lint 全过;待 own-feishu reviewer 合入
...
- **合并链**:§18 归属+feishu 修复共审 reviewer ad780254e76fd74e7(4a8d60216+2817d8bd0,cron f8d15545),PASS 后统一 merge main(带 a8c91b5fd claude-work-mode+凯利/SVG 各自 reviewer PASS 后并入)
- 后台 4 agent 跑中:own-feishu review/SVG 实施/17:50 调研/凯利 review
