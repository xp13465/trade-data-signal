...
**状态**：纯调研落档，待用户排期。NOTES §48 AZ90 有完整调研摘要（参考性结论/数据源 fundf10+akshare 9 接口/设计方案 5 指标+新 tab+首页角标+4 维共振/优先级风险/实证案例 005827 减仓 17.38pct）。完整调研报告 404 行存 `/tmp/public-fund-research.md`（含学术背景/披露规则/数据源对比/采集字段/指标计算/展示 ASCII 示意图/风险缓解/附录调研过程）。实施时派 agent：①季度全量采集脚本 ②日更轻量采集 ③5 核心+3 衍生指标计算 ④前端新 tab ⑤首页角标 ⑥信号灯规则 ⑦4 维资金面共振联动。预计工时 ~3.5 天。
