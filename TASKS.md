...
- **测试象限**:高/中/低 3种评级各自统计(3组对比可靠度)。
- **卖出2模式**(分别回测):
  - 模式A:固定持有10天后卖出。
  - 模式B:持有10天内,收益若达到止盈线(3%/5%/7%)就自动卖出;若没达到,持有满10天也自动卖出,不管盈亏。
- **测试周期**:近1年/近3年/全部(同模拟回测 trade_sim 周期)。
- **展示位置**:策略实验室 -> 自定义分析 -> 新建「信号凯利回测」tab。

### 待用户确认点
- **3/5/7% 止盈阈值**:主控理解为3个独立止盈阈值分别各跑一组回测(参数扫描,对比哪个止盈线最优),非阶梯递进。若如此,模式B=3个子策略(B3%/B5%/B7%),共4组回测[A+B3+B5+B7] × 3评级 × 3周期。请用户确认。

### 依赖(已有可复用)
- 信号评级(高/中/低):AZ85 commit 27b365be 首页技术参考点按 type 分类评级已有
- ETF匹配逻辑:现有 _etfMatchTags/board_etf_map 匹配最匹配ETF
...
- **工时**:待需求确认后调研估算
- **优先级**:第二优先级(用户派序:第一=复权->T1->D1;第二=T9+信号凯利回测)
- **排期**:待复权->T1->D1 第一优先级完成后,与T9并列第二优先级。3/5/7%确认点定后调研方案+估工时。
...
### 会话状态(2026-08-08,用户回来派序,连轴转开发启动中)
- feat 有本地 commit 未 push:7ebe1652d(数据语境审计+清理尾巴)+77f573e22(复权方案定稿)+信号凯利回测补落档(本条),等复权实施一起 push(非 force)
- **复权阶段1 在跑**:agent 采集 accum_nav 回填 etf_daily(~2h 凌晨跑中),进度文件 /tmp/agent-progress-etf-adjust-stage1.md
- **用户派序(2026-08-08)**:
  - 第一优先级:复权阶段1(在跑)-> T1 复权2/3 -> D1 ETF评分+信号灯
  - 第二优先级:T9 全站性能 + 信号凯利回测(新需求补落档✓,待确认3/5/7%细节)
  - T5 日图hover:已确认实施(commit 726eca7be 2026-08-06),从待办移除
- 待用户确认:D1 的 TE/Dev 权重(推荐TE30/Dev25)+ 信号凯利回测的 3/5/7% 阈值语义(独立阈值vs阶梯)
- 等复权阶段1 报告 -> 接 T1
