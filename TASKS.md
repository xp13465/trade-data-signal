...
- **港股行业分类历史趋势** 🟡 P2（2026-07-16 翻盘）：前次调研以为需自建每日落库累积，本次深挖实测 `stock_hk_index_daily_sina(symbol)` 支持**全38指数历史daily**（CESG10博彩业2523行2016~至今），**可一次性回填无需累积**！但38只是"港股相关指数大杂烩"非恒生11行业完整体系，真正板块属性约8-10只（博彩/科技/地产/油气/银行/消费/中资企业/红利），建议命名"港股板块指数"。落库方案：复用 `index_daily` 表（无需新表）+ config 加 `market: hk_industry`（新market避免混入三大指数大图）+ index_id 加 `hk_` 前缀 + 一次性回填脚本（每只sleep 3s防封IP）+ 每日增量并入17:50 update_all（不需新launchd）+ 前端复用 `renderIndustryGrid` 接入 renderHK 末尾（web:3926/static:3995）+ CSS复用现有 `.industry-grid`。详见 NOTES §24。待"每日全球数据复盘日报"需要或用户指定时实施。
