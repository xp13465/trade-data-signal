...
- **宽基全量 进行中**(agent a32f1c84b,cron d8d60a8c):用户改口径"宽基也全量(完全一视同仁)"--所有指数(含宽基上证/深成指/沪深300)统一a+b+c+d不分层。改board_ids收宽基(market=a/hk)+宽基不走_build_index_etf_map_auto单独路径+KW误匹配过滤('上证'->'上证50'等)+重跑全量。进度文件/tmp/agent-progress-broad-full.md
