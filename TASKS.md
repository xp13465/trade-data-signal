...
**最后更新**:2026-08-12 凌晨(**用户目测发现 4 问题,fix4 修复 agent 已派**:①跨市场综合评分线断成点(用户点名最严重,查 _lwLineCard connectNulls 根因,排查同类恐贪/情绪分/AD)②市场宽度 1:1 复查(用户质疑)③申万1级热力图 x 轴行业名竖排(首页+板块分化2处,小屏横排重叠)④申万1级热力图行序对齐 echarts(原版第一行近5日/第二行近1日,SVG 反了);agent aedd2503aede88cee,cron b60023df,§9 sw a150,不 merge main)。**SVG 修正主链已上线 main**(a149,§8 三查过,待用户目测整体)。**次日开盘回测 DONE 待办已落档**(docs/kelly-nextday-open-backtest.md,核心:次日开盘滑点成本极低 每日池净利差-0.01%,-1%限价单 K=1 +844,931/52.81%)。**positionCap 修复已 merge main**。**在跑**:fix4。待 fix4 →reviewer→merge main→用户复查目测。待派:默认最优解+弃用标志+结论展示+辩论详情入口。
