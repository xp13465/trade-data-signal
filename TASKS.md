...
- **R2迁移阶段3 实施中**(agent a4b4fa3ac44680062,16:30派):定时任务去git push改R2(intraday_snapshot.sh L190-313/deploy.sh L281-352/push_stats/update_lab去数据git push,改只R2上传+purge_cache+notify告警)。代码push保留(开发者手动)。static-site/data/仍tracked(阶段4前)。旧版git push安全过渡不急于17:50前。reviewer后push main。进度文件/tmp/agent-progress-r2-stage3.md
- **阶段5 staticdata方案**(用户2026-08-08定):staticdata=**DB原件+差异化增量日志作用**。存DB原件(3个:sentiment.db/etf_national_team.db/public_fund.db,**不压缩**,git存delta追踪每日变化=差异化增量日志)+配置(.env.example/wrangler.jsonc/launchd plist模板,不含密钥)+小JSON日频快照(overview/intraday/boot等几MB文本,git diff)。**大文件(index/industry/lab/trade_sim 268MB+)不存**(从DB产出)。灾备分工:①R2 signal-backup私有桶=DB**压缩快照**(upload_r2.py upload-db推gz,分层backup/30+weekly/28+monthly/365,全量恢复用)②staticdata git=DB**原件不压缩**(git delta差异化增量日志,看每日变化历史)。两者互补不重复:signal-backup解压恢复全量,staticdata git diff看变化。DB原件进git首次80MB,后续delta增量几MB/天(git对binary存delta非全量,非40MB/天误估)
