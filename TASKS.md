...
- **阶段5 主体✅完成**(staticdata GitHub 6894d9e + deploy.sh f015dd8e0 feat):配置(.env.example脱敏+wrangler+23 plist脱敏)+小JSON(35文件排除大文件)+.gitignore排除DB+大文件+.gz。deploy.sh L523-584 staticdata备份块(best-effort rsync DB到本地db/不进git+配置+小JSON+commit+push)。§0验收通过。**DB暂排除(GitHub 100MB限制),后续迁GitLab**
- **⚠️待办:DB迁GitLab**(用户2026-08-08定,后续注册后):DB原件(sentiment 125MB/etf_nt 179MB/public_fund 2.2GB)超GitHub 100MB限制,换GitLab(无限制+免费+国内快)原样进git。用户注册GitLab提供URL后,派agent改staticdata remote GitHub->GitLab+移除.gitignore的`db/*.db`排除+git add db/*.db+push GitLab。保留DB原件差异化日志(git diff)
