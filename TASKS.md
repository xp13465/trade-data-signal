...
- **港股行业分类历史趋势** 🟡 P2：akshare 无 daily_hist 接口（`stock_hk_index_spot_sina` 仅给当日 spot 快照，含恒生银行/油气/地产/博彩/VHSI 等 38 指数）。做历史趋势需**自建每日落库累积**（每日跑 spot 存盘）；做当日涨跌分布可直接用 spot。2026-07-16 全球数据补全调研确认（`index_global_hist_sina` 只覆盖日经/KOSPI/欧洲三大等股价指数，不含港股行业）。当前未启动，待"每日全球数据复盘日报"需要港股行业分布时再做。
