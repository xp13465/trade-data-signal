...
4. **申万一级口径调研(验收完成)+实施(进行中 agent ab7b0002af598d5c7)**:用户认知"反查很麻烦但非公开信息更有价值"(公开数据+重构口径/反向推算=非公开洞察,信息差是alpha来源)。调研验收数据:fund_portfolio_hold 1期(20260630)/937基金/9337行/每基金~10条重仓股/平均集中度42.37%(范围0.19-93.64%);sw_components.json 31个申万一级行业/5210成分股/重仓股反查命中率94.6%(未命中69只港股代码)。3个硬限制:①时序不可用[fund_portfolio_hold只1期做不了历史对比]②采集偏制造业[collect_portfolio_hold L1220硬编码WHERE industry_name='制造业',937只里935只制造业基金非制造业主题基金重仓股没采]③仓位覆盖率42%[top10重仓股平均占净值42.37%只反映42%仓位行业暴露]。实施(进行中):去制造业硬编码采全市场+_compute_sw_industry_alloc()全市场重仓股反查31行业+第四档sw切换(fetch新JSON换数据源)+诚实标注3硬限制(副标题+❓弹窗强调反查价值:揭示真实风格暴露vs官方口径)+制造业breakdown在sw档禁用+bump sw ui83。详见NOTES §48 AZ109。

**3待办(写在此处防compact丢)**:
- 申万一级实施验收(agent ab7b0002af598d5c7 完成后逐字验收:去制造业硬编码+全市场反查31行业+第四档sw切换+3硬限制标注+制造业breakdown禁用+sw ui83)
- 预估点位前端:88魔咒图加"今日预估95.01%"点(读public_fund_position_estimate.json,等申万一级完成避免撞app.js)
- 预估仓位每日自动更新闭环:pipeline_daily加estimate重算+cp+deploy(等申万一级完成避免撞public_fund.py,否则JSON永远手动跑旧值)
...
**活跃 agent**:
- 申万一级实施 ab7b0002af598d5c7 进行中(去collect_portfolio_hold L1220制造业硬编码采全市场+_compute_sw_industry_alloc()全市场重仓股反查31行业+第四档sw切换+3硬限制标注+制造业breakdown禁用+bump sw ui83;详见NOTES §48 AZ109;完成后待办:预估点位前端接入88魔咒图+预估仓位每日自动更新闭环pipeline_daily,两件等申万完成避免撞app.js/public_fund.py)
...
**正在等**:①申万一级实施 agent ab7b0002af598d5c7 完成(去制造业硬编码采全市场+全市场重仓股反查31行业+第四档sw切换+3硬限制标注+制造业breakdown禁用+sw ui83,完成后逐字验收)②申万完成后串行2件[避撞app.js/public_fund.py]:预估点位前端接入88魔咒图(读public_fund_position_estimate.json加"今日预估95.01%"点)+预估仓位每日自动更新闭环(pipeline_daily加estimate重算+cp static-site/data/同步+deploy,否则JSON永远手动跑旧值);前次历史待办:a9bfb02d 8任务采集时点调研报告(关键发现rzhb 23:00滞后4-5h应提前到19:00-19:30违反第一时间发布)+下一轮优化候选(a37 P1-1 compute/runner.py 14步串行->ThreadPool并行B并发省30-50%[7独立步骤+6指数循环]/P1-2 export.py 30次重复查DB->内存切片A脚本合理性/rzhb 23:00->19:00-19:30第一时间发布);远期ss.fx8.store CF deploy优化(GH Actions wrangler)
