...
**最后更新**:2026-09-07 14:2x(**会话收尾落档:重启代理后换新会话,新会话先读本节恢复**).**在跑**:⚠️ reviewer 已回=**sim-price-toggle PASS**(核心全对 §22/独立开关/换行/回归/公示/对账全 PASS,**2 非阻塞建议待用户拍板**):F1(置信75)切档竞态——加载中点切档 `_simSetBuyBasis` 未重置 `_simKellyLoading` → 旧口径写回+卡 loading(全量兜底 64MB 窗口明显);F2(置信80)次日档「实际买入日期」用 trades 记录并集近似交易日历,后端定价用真实价格日期——稀疏年(2015 年片 23 中 5 个 gap>3 天)前端日期与 buy_price 可能错位 1 天,不影响盈亏计算,违背"次日=下一交易日"语义。**merge 决策=待用户拍板(带上线/先修 F1/先不 merge);F1/F2 已落档待办**。若拍板上线:main-merge.sh 统一 bump(机制C)→changelog 补登→§0 三查。**v1.1.17 发布链**:内审+外审全 PASS(59 commits,rev-20260907-001 无 P0/P1);fix555+intraday 已 merge(a551/a552);sim-price-col 已上线(a553);**deploy+打 tag 兜底 cron ffa63f46(09-07 18:05 一次性,update_all 17:50 自动部署后自动验证+打 tag)**;盘中不跑全量 deploy。**模拟回测弹窗第二件(用户 09-06 拍板)**:双档切换(次日开盘默认/当日收盘,独立键 tds_sim_buy_basis 与 lab 零联动)+「计划买入时间」列上下换行+首列「信号日期」=commit b791759e5,implementer 自验 21/21+§5.4⑦ 逐位一致,reviewer PASS。**⚠️ 代理重启:时点由用户拍板,主控不得自行动手**(#48/#59 merge 后重启代理=断当前 8899 通道,重启前确认用户在场)。**待派(用户确认后)**:#48 P0-3 代理高可用(OUT_OF_RANGE 定位/thinking 归一/healthz 告警,worktree 隔离+隔离测试)+#59 商汤 7key(SENSENOVA_KEY1-7 已入库,派 implementer:KEY6/7 轮询+P0-1 清诊断残留+P0-2 冷却持久化+P0-3 jitter+P2-1 停 kimi)。**FAPI 观察期 cron 1e0ef6fb 09-08 到期(周一收尾一次)**。TaskList:#71 打 tag in_progress(ffa63f46 兜底)、#48/#59 pending。✅ #101 北交所宽度=已拍板C+已完成上线(残留指针已清 264dfad9d)。待用户确认:本地审计清理清单 docs/local-state-audit-20260903.md。
