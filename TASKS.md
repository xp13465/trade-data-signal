...
11. ✅ **sh 专属 C1 降尖尖上线**（2026-07-22，替代小节AU sh 豁免）：sh 豁免致 sh 尖尖率 10.38%（10 指数最高）。agent 调研方案 B 对 sh 误滤根因（dist_from_low60>30% 对 sh 趋势中继误滤）+ C1 洞察（dist_from_high>=15% 精准滤低位假突破，尖尖组 11.13% vs 非尖尖 5.59% ratio 1.99，>=15% 档尖尖率 23.91% baseline 2.3 倍）。用户确认采纳 **C1 = `(atr_pct>=2.5%) OR (dist_from_high>=0.15)` 替代 sh 豁免**。效果（vs sh 豁免基线，全维度改善无反害）：742->612(保留 82.5%) / mdd -3.72%->-3.01%(-0.71pp) / 尖尖率 10.38%->7.35%(-3.02pp/降 29%) / bot_acc 66.04%->69.12%(+3.08pp) / ret20 +5.27%->+6.29%(+1.02pp 不损反升)。其他 9 指数继续方案 B 不变。signals.py 改 3 处（L805-807 新增 dist_from_high 计算 + L809-812 sh C1 分支 + L786-803/L828-831 注释）。compute()+store() 验证：sh 742->612，total 12369->12239（减 130=sh 单独）。详见 NOTES §48 小节AV
