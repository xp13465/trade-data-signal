...
**⚠️ codex 缓存命中差(待办)**:`~/.codex/config.toml` `disable_response_storage=true`(关服务器 response storage)+`wire_api=responses` 直连 bailian(不经 thinking_proxy),且每次 codex exec 用 `--ephemeral` 新 session,无前缀缓存复用——本次 2-commit 外审烧 143K tokens。候选:①`disable_response_storage=false` 试开 response storage ②codex base_url 改走 thinking_proxy(127.0.0.1:8899;Claude 侧靠它做到 ~0.97 命中)③须在 bailian/峰谷定价下实测,勿贸然改全局 config。
