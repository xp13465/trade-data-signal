...
- **测试象限(分期,2026-08-08 用户扩展)**:
  - 一期:信号评级 高/中/低 3种各自统计(3组对比可靠度)
  - 二期扩展(未来):增加 ETF 跟踪归类维度 强关联/相关/近似(track_tier: strong/related/approx),与信号评级交叉对比(如高评级+强关联 vs 高评级+近似,看ETF跟踪质量对信号收益的影响)
...
- **二期扩展依赖(2026-08-08 用户新增)**:D1 新权重(TE30方案)做好后,完善 track_tier 3档位划分(strong≥80/related 70-79/approx 50-69 阈值,基于新权重实测分布调优)。一期只需 track_score 第一名,二期需 track_tier 归类完善
