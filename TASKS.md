...
2. **BB_upper_revert 验证有效后融入生产**：作为 sell_aux 写入 signal_daily，届时才动 signals.py（08候选C，与D1时间互补）。触发条件：08建议「D1实盘连续2季度10日胜率<45%则启用BB_upper_revert互补」。融合前需用最新数据重跑验证 + 用户确认。
