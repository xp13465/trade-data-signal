...
- **✅ openrouter 成本优化调研完成并落档(docs/openrouter-cost-opt-research-20260823.md)**:三问三答=①联网搜索费当前配置恒为零(opt-in 才触发,:online 弃用,零动作)②缓存直连透传正常+sticky 自动,读0.1x,官方量化省70%,靠前缀稳定+会话连续保命中,禁 :floor/:nitro/order ③exclude:true 不省钱(照常计费),正道=CLAUDE_EFFORT high→medium+实施类子agent low。**待用户拍板**:effort 降档/settings.json 补 `ANTHROPIC_API_KEY:""` 防坑/AUTH_TOKEN 挪 keychain 并轮换;用户侧先去 Activity 页看 cached_tokens 建基线。
