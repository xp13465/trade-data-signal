...
### P1 🔴 回测验证（当前优先）
- 实现 `app/compute/alert_score.py`（~150 行，复用 normalize 框架算 HIGH_ALERT/LOW_ALERT 两分，8+8 维度加权求和，120 日滚动百分位归一化，缺项重归一化）
- 实现 `scripts/backtest_alert.py`（复用 signal_stats forward 收益逻辑，跑 2016-01 至今 2559 天历史）
- 验证目标：高位预警后 5/10/20 日下跌占比>55%；低位预警后上涨占比>60%（底比顶更好抓）；盈亏比>1.2
- 触发频率目标：高位 3-8 次/年，低位 2-5 次/年（过高=阈值偏松收紧，过低=偏紧放宽）
- 防过拟合：2016-2022 训练定参，2023-2026 样本外验证胜率不显著下降；维度间低相关（相关矩阵检验）
- 不达标则调权重/维度（8 维度权重 0.20/0.18/0.15/0.15/0.10/0.10/0.07/0.05 整数化不微调）
