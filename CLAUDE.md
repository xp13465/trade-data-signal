...
- **子agent 耗时优化规范(2026-08-11 用户定,深挖 v4-flash 慢根因后)**:①**禁读大文件规则已撤销**——曾为提速私自加"禁全文读 app.js 定点 grep"(用户定:私自加的规则先撤销,恢复自由读,验证是否恢复原速;后续需再启用须用户确认)。app.js 1.3MB 巨型耦合,强制定点 grep 只看到片段致 agent 多轮往返/误判反而更慢;恢复自由读。**Prompt too long 事故按需处理(2026-08-11 用户定,不预先降级)**:不预先禁读/精简,碰到了才为那次请求做处理(分段询问/分批继续让任务继续),不碰超长就不精简——"Prompt too long 事故应该有别的优化方法,或者说碰到了才为那次请求做某些操作(比如分段询问或干嘛让任务可以继续),不碰到超长就不应该此次精简"。②**合并 Bash 规范(用户确认实施)**:合并**小输出同类轻量查询**(2-3 个轻量 grep 一条跑)为规范;重输出/易错/需中间调试的保持单条,不一刀切。③**拆任务原则(用户确认:不为小而小)**:同任务一个 agent 做完(复用上下文+经验),拆任务=每 agent fresh context 重读代码=token 浪费无性价比;真正的"减步数"=同一 agent 内少调工具次数(每步 API 响应 60-400s 是瓶颈,非工具本身)。④动态并发实验(2026-08-11 完成):2/3/4 并发同构任务测批次总耗时+丢通知率。**结论:并发非瓶颈,维持现状不调上限**;慢尾部是 agent 个体异常非并发排队(两种形态:b2c 4工具/2235s=每步API响应极慢,b3c 1381工具/3083s=agent陷入工具循环);文件漂移干扰并发agent(b1a因app.js被并发修改重查);丢通知9/9小样本。**实验暴露3个独立问题+对策**:A**agent工具循环**(b3c 1381次=token浪费)→cron轮询加工具调用数监控(派agent必设cron,cron prompt含 `grep -c '"tool_use"' jsonl`,次数>40且未DONE=疑似循环→SendMessage介入,现cron只看mtime 900s对"活着但循环"无感);B**API偶发慢**(b2c每步9min)→prompt加单步超时自检(单步等待>5min自查/降级,模型侧慢主控控制有限,可选);C**文件漂移**(b1a)→调度原则:实施agent改某文件期间不对该文件派只读调研agent(§3延伸)+调研prompt加"读到结果即用,不因mtime变化反复重查"
- **规范落档铁律(2026-08-11 用户定)**:任何规范操作/决策必须落 CLAUDE.md——哪怕后来发现不合适也要落(便于反查+让用户知道怎么去掉),memory 会忘。落了 CLAUDE.md 才可被 review/反查;不落=不可取。用户原话:"这种规范性的操作一定要进 claude.md。那怕是错误的。也能让别人知道有对应的规则可以反查。毕竟你的memory 也会忘。进了 claude 发现不合适也才可以知道让你去掉。像这次问题是我问你你才回答说你干了规范的活又没落档规范。这事不可取的行为"
- **性能对比必须外观对等(2026-08-11 用户定,ETF hover 轻量版验收触发)**:任何"提速/轻量/精简版"切换,必须**功能+外观完全一致**,只允许渲染引擎/性能差异。用户原话:"所谓性能对比,一定是功能样子完全一样的情况下,提高性能了才是一个正确的对比。否则用不对等的东西在那比较,是不合理的。"——轻量版不是"另一套简化设计",须逐像素复刻原版样式(背景色/网格线/透明度/坐标轴/节点/字号全对齐),唯一差异=渲染引擎(如轻量零 echarts 依赖,但 SVG 输出要和 echarts 完全同款外观)。教训:ETF 评分弹窗近30日走势轻量 SVG 版补了 hover 功能但背景色/背景线/面积透明度/xy轴尺寸/节点和 echarts 完全不同,用户不接受。防重犯:做轻量/替代版前先拉原版完整视觉参数(含是否随主题变化),逐项复刻,验收口径含"轻量版与原版外观逐项对照一致"
...
3. **memory周review**(每周日23:00+):派agent review memory全部文件:删过时(文件/字段/规范变化,验证后删)+合并重复+更新MEMORY.md索引。**+防重犯条款有效性审查(2026-08-12 用户定)**:审查 §18 所有防重犯条款,被违反≥2次的条款=条款无效,必须重写(更具体可执行/换执行机制/改到§20 quickstart 让子agent真读),治"条款堆积但反复违反"——不是再堆一条,是改条款本身
...
- **历史对照优化(2026-08-12 用户定,核心:总结须结合历史,非只追加今天的错)**:
  ...
  - **每日/会话级总结 agent prompt 必含"历史对照步骤"**(总结前必做):先读 §18 全部历史过错(含归档到 docs/archive 的)+ 相关 memory,当天每个新过错**先分类**:
  ...
  - **总结输出必含"复发强化清单"**(本次强化/改进了哪些历史条款,附改动),不只"新增过错清单";复核 agent 检查时也验"复发是否走强化而非新开条"
  - **防重犯条款有效性审查**:每周 memory review(机制第3条)审查被违反≥2次的条款重写;§18 中心思想="不重犯同类+经验积累成长",同类复发须让条款更有效,不是记录得更勤
