...
### 5.4 测试基准锚点铁律:一切回测/测试以 1.0.0 推荐最优组合为前提(2026-08-15 用户定)
**触发词**:派任何回测/测试/挖掘/穷举任务前;researcher 设计口径时;判断回测结论可操作性时。
**核心一句话:已打 v1.0.0 正式版标签(2026-08-15),v1.0.0 定稿的「推荐最优组合」就是后续一切测试的唯一前提,任何测试口径偏离它=违规。**
- **① 基准定义(v1.0.0 推荐最优组合)**:默认 AI 推荐 = AI宏4+3+1(7键+1类回测剔除)+ 每日资金池等分 + AI仓位建议 K=1 + **G 用 13万 P≤3d「先卖年轻仓」可操作口径**(峰持仓≤20倍本金);A/F 短持 = 每日池+top-K。**裸 G(无 P≤3d,峰持仓86万/136万不可操作)不是基准**。
- **② 测试前提 = 基准**:一切回测/测试/挖掘/穷举,**默认前提必须是 v1.0.0 推荐最优组合**。要测非基准口径(如裸 G、其他 K 档、其他 P 档、历史 fixed 口径),必须**显式声明"非 v1.0.0 基准口径"** + 说明为什么测它 + 结论标注口径差异,不作为主推结论。
- **③ 派单钉基准**:主控/所有角色派回测/测试/挖掘任务,prompt 必须写「测试基准 = v1.0.0 推荐最优组合(具体定义见 §5.4①)」,防认知偏离。researcher 设计口径第一件事 = 确认基线落在基准上(能复现基准基线才往下,如 G 基线必须 = 13万 P≤3d 的 +202,508/155.78%,不是裸 G +654,551)。
