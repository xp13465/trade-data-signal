...
- ✅ **稳定方案=主控主动查,不靠通知(2026-08-08 定,机制转换非修补)**:通知丢失根因两层:①SendMessage 丢失率近100% ②task-notification 只 agent 终止时发,came to rest(完成等指令不终止)不发--这层之前没认清。**主控不靠 SendMessage/task-notification,主动查进度文件 ## DONE 为唯一状态来源**:①cron 缩短到5分钟(自15分钟,省cron调用让位准确性)查所有在跑 agent DONE+mtime ②主控报状态/派新 agent/用户问时主动 Bash 查所有在跑 agent(不只靠通知) ③查到 DONE 收结论+清 cron,mtime>900s 卡死 resume/重派。进度文件磁盘持久,主控查确定性,不依赖通知。通知丢了自己查补救。这是机制转换(被动等通知->主动查),非修补通知。代价=轮询token,但比漏收结论/状态不准成本低(8/8 5agent漏收,报6实际1,用户发现才查)
