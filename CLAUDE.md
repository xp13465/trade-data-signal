...
- **① 基准定义(v1.1.0 推荐最优组合)**:默认 AI 推荐 = **AI宏 4+3+1+K2C5 = 基础4(n2NovSpecialIndustry/excludeSpecialBear/janMidRating/janMidSpecial)+核心3(r7MayReinforced/excludeAuxCross/greedy15)+K2C5(港股追涨剔除,第8键)+1类回测剔除(债类/波段不入宇宙 _bt_in_universe)** = **8键+1类,总数从8变9**。**K2C5 不算进基础4或核心3**(那 7 键是穷举验证的稳定核心),是**新增第 8 个降亏键**。组合 = 每日资金池等分 + AI仓位建议 K=1 + **G 用 13万 P≤3d「先卖年轻仓」可操作口径**(峰持仓≤20倍本金);A/F 短持 = 每日池+top-K。**裸 G(无 P≤3d,峰持仓86万/136万不可操作)不是基准**。
  - v1.1.0 增量依据:`docs/kelly/analysis/kelly-k2c5-return-quadrant-check.md`(K2C5 加入默认,全信号除 G 外双升、港股卡剔除后 0 负全转正)。**基准计数:v1.0.0 = AI宏 4+3+1(7键+1类),v1.1.0 = AI宏 4+3+1+K2C5(8键+1类)**。
