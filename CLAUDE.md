...
- ⚠️ **方向纠正(2026-08-08 用户定):稳定方案从主动通知入手,非兜底查**(原"主控主动查"方案用户否决,原话"兜底查既成本高还不及时,我需要的稳定方案是你从主动通知入手,不是从兜底查入手")。**主控主动查进度文件=cron 兜底,非主方案**(cron 保持15分钟省token,仅兜底防傻等)。**标准方案=找主动通知稳定机制**(调研中):①让agent完成即terminate触发task-notification(came to rest不触发/terminate触发,待验证)②SendMessage丢失根因(假设①agent没调sm_use=0 vs ②harness没送达,grep jsonl验证)③StructuredOutput返回作完成信号④fswatch监听trigger文件。通知丢失根因两层:①SendMessage丢失率近100% ②task-notification只agent终止时发came to rest不发--这层之前没认清。调研产出推荐方案后§11据此更新
...
- **2026-08-08 追加(方案A board_etf_map数据产物遗漏+reviewer误报,印证回归检查不完整)**:
  - 过错:①方案A agent 改 build_board_etf_map.py 代码(TRACK_WEIGHTS_INDIRECT)但没重跑生成新 board_etf_map.json(只重跑simulate_trade/index detail/overview,漏board_etf_map),local+R2都旧版516630 ②reviewer验"local board_etf_map=159586"误报,实际local=516630(reviewer没真读文件,信agent自验结论) ③主控§0 curl board_etf_map才发现(overview 159586但board_etf_map 516630不一致),非等用户发现
  - 防重犯:①算法改动重跑数据产物时,列所有依赖该算法的数据产物清单(board_etf_map/overview/index detail/trade_sim)逐个确认重跑不遗漏,不只重跑显眼的 ②reviewer验数据产物必须真读文件/curl验内容(非信agent自验),reviewer prompt明确"curl local+R2+CF三处验具体字段值" ③主控§0不只验主路径(overview),验所有相关数据产物三版本一致(overview vs board_etf_map vs index detail) ④agent自验+reviewer+主控§0三层都需真验文件内容,任一层信结论不验文件=漏洞
