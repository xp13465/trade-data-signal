...
- **核心 5 条**:
  ...
  ⑥**防前视铁律=调研回测核心宗旨**(2026-08-23 用户定,原话"牛熊分界等这种变化信号一定不能是后视镜模式。就是在某个时间点只能用时间点前的数据信号判断,不能用未来的数据判断过去";也是防过拟合手段):任何择时/状态/切换类信号(牛熊分界/QVIX 分位/宽度阈值等)在时间点 t **只能用 t 之前的数据判定**,t 日收盘出信号次日生效;三条硬机检:①分位数阈值禁用全期分位(=暗用未来数据),必须 expanding/滚动窗口并写明参数 ②复用既有特征库先核查固化口径,全期固化的重算纯历史口径再入 ③时点穿越测试(截断到历史 t 重算信号序列,与全量数据算出的逐位一致才 PASS)。全文见 researcher skill §3.1;实施/reviewer 要点见各 role skill(对应 skill:role-researcher §3.1 / role-implementer / role-reviewer;参照 kelly-etf-freeze-lookahead 同精神)
...
- **⑥ 版本升级原则(2026-08-15 用户定)**:用户原话"因为动了最核心的东西(ai过滤算法),需要发中间版本定义 1.1.0,以后每次动到这个最核心的实用功能就必须发版本"。**动到「AI 推荐 / 降亏过滤」这个唯一有实战价值的核心实用功能的默认组合/算法,必须发一次中间版本(如 v1.1.0→v1.1.1→v1.2.0...),同步更新基准定义(memory `test-baseline-v112-anchor`) + 前端默认值 + §21 公示 + README + 全部键集登记点(前后端+邮件链路 check_signals.py AI_MACRO_KEYS 类;18 处登记点全量表见 docs/kelly/analysis/v115-new14-baseline-alignment-audit.md)+ 键集一致性机检 PASS**;纯新增不影响默认行为的可不动基准版本。**v1.1.1(2026-08-16)先例:动到「AI推荐/降亏过滤」相关数据层人口口径(非默认组合本身)也须发版本标记**,但不回测基准定义;凡动了默认组合/算法本身才升级测试基准定义。基准升级时,老版本组合不再作测试前提,但保留在 memory/文档供对照。
...
### 过错索引(43 条:锚点 id|日期|主题|一句话防重犯|归档原文位置,按归属 5 类分节:通用 6 / 主控 16 / 实施 13 / 调研 7 / 测试 1)
...
- **③ 与 §5.4/§15 关系**:§5.4 是回测/测试必须落在当前测试基准(memory `test-baseline-v112-anchor`),本条是更上位的功能冻结契约——已发布的展示/交互/布局同样冻结;§15 reviewer 回归防改坏老功能,本条从源头拦"不该动老功能"。
