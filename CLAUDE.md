...
### 5.4 测试基准锚点铁律:一切回测/测试以 1.1.0 推荐最优组合为前提(v1.0.0 2026-08-15 / v1.1.0 2026-08-15 用户定)
**触发词**:派任何回测/测试/挖掘/穷举任务前;researcher 设计口径时;判断回测结论可操作性时;动到"AI 推荐/过滤"核心实用功能时(版本升级原则见 ⑥)。
**核心一句话:已打 v1.0.0 正式版标签(2026-08-15),v1.1.0(2026-08-15 用户拍板)定稿的「推荐最优组合」就是后续一切测试的唯一前提,任何测试口径偏离它=违规。**
...
- **⑥ 版本升级原则(2026-08-15 用户定)**:用户原话"因为动了最核心的东西(ai过滤算法),需要发中间版本定义 1.1.0,以后每次动到这个最核心的实用功能就必须发版本"。**动到「AI 推荐 / 降亏过滤」这个唯一有实战价值的核心实用功能的默认组合/算法,必须发一次中间版本(如 v1.1.0→v1.2.0...),同步更新本基准定义 + memory 基准锚点文件 + 前端默认值 + §21 公示 + README**;纯新增不影响默认行为的可不动基准版本。基准升级时,老 v1.0.0 组合不再作测试前提,但保留在 memory/文档供对照。
