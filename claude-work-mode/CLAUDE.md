...
**触发词**：改工作模式规范/条款、新增/改 skill、迭代被 skill 引用的代码/数据口径/配置。
**核心一句话：项目级 skill 必须具备「关联内容更新 → 自动同步」能力；skill 与项目同生命周期迭代，过时的 skill 比没有更危险（带错误步骤误导下一次操作）；skill 与工作模式规范冲突时以规范为准。**
- **① 关联规范源内置**：每个 skill 在 SKILL.md 内写「关联规范源」段（标注来自规范哪条 § / 哪段代码 / 哪个配置），改了源头顺着引用反查即找到要同步的 skill
- **② 规范反向标注**：规范被 skill 引用的条款注明「对应 skill：xxx」，双向咬合
- **③ 改规范类内容必同步**：派实施改规范/条款/被引用代码口径时，prompt 带"检查并同步受影响 skill"；skill 改动按「优化版要素」（流程化+可执行命令占位符+自检清单+反例库）维护，不直接拷贝规范散文
- **④ 定期巡检**：发版时或每周 review 时核对 skill 与规范一致性（与回归/周 review 同节奏）
- **⑤ 状态型内容绝不 skill 化**：时点表/现网端点/当前配置值等"状态"会过时误导，留文档按需查，不进 skill
