#!/bin/bash
# backfill_indices.sh - 凌晨+晚间轻量补采兜底（launchd 16:35/21:00/02:00 定时）
# 已被 backfill_metrics.sh 替代（新增 direct 指标补采），此文件保留作历史参考。
# 只做：校验补采缺失指数 + (有补则)重算情绪分 + 推送。不全量采集（几十秒）。
# 兜底场景：15:33 早跑时三源都没今日数据 -> 21:00 三源已更新，补上；02:00 凌晨再兜一次确保次日清晨数据齐全。
