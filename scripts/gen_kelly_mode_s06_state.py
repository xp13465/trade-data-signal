...
size_spread + s06_grid_selection_freeze.py 冻结值逐位对齐, 报告=docs/kelly/analysis/codex-auto-external-factor-final-20260825.md)
  ...
  - 键集映射: effective_mode='a9' ↔ common.js _KELLY_FADE_MODE_PRESETS id=a9(A 进攻王);
              effective_mode='new15' ↔ id=new15(NEW14+1·15键)。回测对照锚点=S06_A_vs_14plus1
              (/tmp/codex-auto/s06_vs_14plus1_results.json: 验段净利 +93,813.21 / mdd -3,811.27 / 强平 +81,434.72)
...
def build_daily(dates: list[str], spread: dict[str, float]) -> list[dict]:
    """sticky_array 状态机(逐位对齐 /tmp/codex-auto/external_factor_v6.py sticky_array):
    T 日收盘判定、T+1 生效; 进入 on 立即(次日), 退出需连续破坏 confirm 天且过最短持有。"""
    out: list[dict] = []
...
def main() -> int:
    ...
    snap = {
        ...
        "_provenance": {
            ...
            "backtest_anchor": "/tmp/codex-auto/s06_vs_14plus1_results.json S06_A_vs_14plus1 val=+93813.21 mdd=-3811.27 forced=+81434.72",
