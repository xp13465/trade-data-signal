...
#
# 用途:从 ../trade-data/.env(仓外)读 SENSENOVA_KEY1/2/3/4/5/6/7,注入 env 后 exec 纯 key 轮换代理(kimi-k3 版)。
...
if [ -f "$ENV_FILE" ]; then
  ...
  for k in SENSENOVA_KEY1 SENSENOVA_KEY2 SENSENOVA_KEY3 SENSENOVA_KEY4 SENSENOVA_KEY5 SENSENOVA_KEY6 SENSENOVA_KEY7; do
