...
"$PY" "$REPO/scripts/with_lock.py" /tmp/trade_deploy.lock bash -c '
  ...
  rsync -a --checksum "$REPO/static-site/data/." static-site/data/
  ...
  if ! "$PY" "$REPO/scripts/upload_r2.py" upload-index 2>&1 | tail -1 | tee -a "$LOG"; then
    echo "✗ upload-index R2 失败，发告警邮件" | tee -a "$LOG"
    "$PY" "$REPO/scripts/notify.py" "[intraday告警] upload-index R2 失败" "走势图数据源(kc50-all.json等)未推 R2，卡片(overview)将上线，三处数据不一致，需手动补刷 R2: bash scripts/upload_r2.py upload-index" --severe 2>&1 | tail -1 | tee -a "$LOG" || true
  fi
  if ! "$PY" "$REPO/scripts/upload_r2.py" upload-industry 2>&1 | tail -1 | tee -a "$LOG"; then
    echo "✗ upload-industry R2 失败，发告警邮件" | tee -a "$LOG"
    "$PY" "$REPO/scripts/notify.py" "[intraday告警] upload-industry R2 失败" "行业指数走势图数据源未推 R2，卡片(overview)将上线，三处数据不一致，需手动补刷 R2: bash scripts/upload_r2.py upload-industry" --severe 2>&1 | tail -1 | tee -a "$LOG" || true
  fi
