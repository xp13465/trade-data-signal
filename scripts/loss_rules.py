...
# 【已知设计取舍(2026-08-24 codex 外审指出)】QTH 为全史快照阈值 = 对滚动分位数再取全史分位
#   = 样本内阈值优化(特征层 mine10_features rolling_pctile(win=756) 本身无前视, 前视只在阈值层);
#   仅限挖掘产出筛选用途 —— 若未来用于实盘择时必须改 expanding 窗口重算阈值
#   (§5.1⑥ 防前视铁律: 择时类分位数阈值禁用全期分位)。注释即公示, 防未来误用。
# ---------------------------------------------------------------------------
QTH = {
