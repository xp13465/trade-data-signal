...
用途:商汤 token.sensenova.cn 对 kimi-k3(商汤新支持模型,支持 thinking/non-thinking + 1M context,
      与 v4-flash 同形态)按 RPM(每分钟请求数)限流,单 token 撞 429(inference tpm exhausted)
      会反复导致 implementer 死限流;客户端退避间隔写死 4 秒封顶调不了。
      ...
      与 v4-flash 版(sensenova-rotate-proxy.py)的差异:①不作 adaptive thinking 参数剔除(body 原样转发)
      ②独立端口 8898/日志/dump 目录,两代理可同时常驻互不干扰。

用法(常驻,launchd 守护):
  bash scripts/sensenova-rotate-proxy-kimi.sh   # 包装脚本(负责从 .env 导出 key 再 exec 本代理)
  或直接(已注入 key 的 env):
  SENSENOVA_KEY1=.. SENSENOVA_KEY2=.. SENSENOVA_KEY3=.. SENSENOVA_KEY4=.. SENSENOVA_KEY5=..
  SENSENOVA_KEY6=.. SENSENOVA_KEY7=.. \
  python3 scripts/sensenova-rotate-proxy-kimi.py
  监听 127.0.0.1:8898(可 TTP_PORT 改),转发 https://token.sensenova.cn/
  真实 key 禁进 git/日志;key 从 env 或 ../trade-data/.env(仓外)读 SENSENOVA_KEY1/2/3/4/5/6/7。

激活步骤:
  1. 确认 ../trade-data/.env 含 SENSENOVA_KEY1/2/3/4/5/6/7(仓外不提交)
  2. launchctl load scripts/com.trade.thinking-proxy-kimi.plist(TTP_PROVIDER=sensenova-rotate 守护本脚本)
  3. settings.json env "ANTHROPIC_BASE_URL": "http://127.0.0.1:8898"(切换由 scripts/kimi-proxy-switch.sh 双向切换)

风险(P0):代理挂 = 商汤轮换不可用。launchd KeepAlive 守护 + claude 重试兜底。
回退:bash scripts/kimi-proxy-switch.sh v4flash(还原 settings 到 8899 v4-flash 代理);停 kimi 代理:
  launchctl unload scripts/com.trade.thinking-proxy-kimi.plist
...
env:
  ...
  TTP_REQDUMP_DIR=.../sensenova-req-dump-kimi  # dump 目录(默认 trade-data/data/logs/sensenova-req-dump-kimi,仓外,与 v4-flash 版隔离)
...
LOG = os.environ.get("TTP_LOG", "/Users/linhuichen/code/trade-data/data/logs/sensenova-rotate-kimi-req.log")

# ═══ 日志级别控制(2026-09-01 用户定「日志可以关闭了」)═══
# 背景:代理每请求写 RESP 行 + REQDUMP 全量落盘(req.log 13MB+、sensenova-req-dump-kimi 16MB),
...
#   REQDUMP 写盘另受 TTP_REQDUMP 开关(默认 0 关);关掉后历史 sensenova-req-dump-kimi 目录不再增长。
...
# 2026-09-06 扩展 KEY6/KEY7(与 v4-flash 版同步):序列动态生成,没配 KEY6/7 时向后兼容。
...
def _rotate_backoff():
    ...
    2026-09-06 P0-3 加全抖动(AWS exp backoff + jitter,与 v4-flash 版同步):返回 random(0, base),
    多请求同时恢复时退避错开不共振,避免高峰并发把上游同时打满。"""
    if _is_peak_hour():
...
# 2026-09-06 P0-2 冷却持久化(与 v4-flash 版同步;本版用独立 cooldown 文件,防两进程并发覆盖)。
# 存储格式:{"keys": {"keyN": {"cooled_until_ms": int, "level": int}, ...}, "updated_at": int(ms)}。
# 真实 key 绝不落盘,只存 keyN 序号。路径可用 TTP_COOLDOWN_FILE 覆盖。
COOLDOWN_FILE = os.environ.get("TTP_COOLDOWN_FILE", "/Users/linhuichen/code/trade-data/data/sensenova-cooldown-kimi.json")
...
# ═══ 冷却状态持久化(P0-2,2026-09-06,与 v4-flash 版同实现)═══
...
# 用 logmsg 写(复用 sensenova-rotate-kimi-req.log,不自建文件)。
_DETECT_COUNT = 0
...
#   TTP_REQDUMP_DIR        # dump 目录(默认 trade-data/data/logs/sensenova-req-dump-kimi,仓外 gitignore)
...
REQDUMP_DIR = os.environ.get("TTP_REQDUMP_DIR", "/Users/linhuichen/code/trade-data/data/logs/sensenova-req-dump-kimi")
...
# ═══ kimi-k3 版说明:不做 adaptive thinking 剔除(2026-09-01)═══
# 本版(kimi-k3 5key 轮询)不做 v4-flash 版的 _strip_thinking_adaptive 参数剔除:
# v4-flash 端点对大上下文 adaptive thinking 会按上下文自推导 thinking_budget,推导值超上限触发
# 400("thinking_budget parameter must be a positive integer...")→ 当时治本是删请求里的 thinking 字段。
# kimi-k3 支持 thinking/non-thinking + 1M context,与该 400 参数剔除逻辑无关 → body 原样转发,
# thinking 相关处理交给上游(kimi-k3 端点)默认行为,不做本地改写。
...
class Handler(http.server.BaseHTTPRequestHandler):
    def _forward(self):
        ...
        # kimi-k3 版:body 原样转发(不做 v4-flash 版的 adaptive thinking 剔除,交给上游 kimi-k3 端点默认处理)
...
if __name__ == "__main__":
    ...
    _port = int(os.environ.get("TTP_PORT", "8898"))
    server = http.server.ThreadingHTTPServer(("127.0.0.1", _port), Handler)
    logmsg(f"sensenova-rotate-proxy-kimi listening on 127.0.0.1:{_port} -> https://{UPSTREAM_HOST}{UPSTREAM_BASE} "
