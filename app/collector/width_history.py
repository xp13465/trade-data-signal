...
def limit_rule(code: str) -> float:
    ...
    300/301(创业板) 688/689(科创板) → 0.20；920xxx(北交所) → 0.30；
    其余（主板）→ 0.10。
    ST 5% 无法判定（mootdx 无 ST 标记），不单独处理。

    920 分支为北交所宽度（app/collector/beijiao_width.py，独立指标组 a_bj_*）
    复用本规则函数（单一事实源）。mootdx 源不含 920 代码，故此分支不影响现有
    主宽度（a_width_*）任何数字。
    """
    if code[:3] in ("300", "301", "688", "689"):
        return 0.20
    if code[:3] == "920":  # 北交所 30% 涨跌幅（2026-09-02 方案C 新增）
        return 0.30
    return 0.10
