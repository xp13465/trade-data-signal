...
### AZ86 2026-07-30 续：favicon替换+白边修复/期货净加多空配色/全站A股配色统一/verify_backup R2重试加固

**【今日续5项闭环（AZ85 8项+本节5项=今日共14项）】**

1. favicon 替换（commit `6a1127b3`）：用户放 `股票网站ICO设计.png`（2048x2048 color-type 2 无alpha）到根目录。magick 转 ico：`magick "PNG" -define icon:auto-resize=16,32,48,64 favicon.ico`。index.html L18 favicon 引用 svg->ico：`<link rel="icon" type="image/x-icon" href="./favicon.ico?v=beddb3f1">`。sw.js PRECACHE_URLS 加 favicon.ico
2. favicon 白边修复（commit `b89805d0`）：问题=PNG color-type 2 无alpha通道，白边是 PNG 自带白底（非透明被填白），`-background none`/`-trim` 都无效。修复=`magick "PNG" -fuzz 5% -transparent white -background none -define icon:auto-resize=16,32,48,64 favicon.ico`（-fuzz 5% -transparent white 把白底变透明，PaletteAlpha 8-bit alpha，32038字节，md5 `1dbb579c`）。index.html L18 版本号 bump `?v=1dbb579c`。**教训**：PNG 无 alpha（color-type 2）白边是自带白底，用 `-transparent white` 非 `-background none`（memory `magick-png-no-alpha-whiteedge` 已落档）
...
### AZ90 2026-07-31 公募基金持仓佐证大盘调研（季度披露+88魔咒/抱团度/净申赎反向，纯调研落档待排期，不改码）

**【背景】** 用户提出公募基金持仓能否作为大盘走向的佐证维度，补充现有"北向/两融/产业资本"3维资金面。本节纯调研落档，不改任何代码。完整调研报告 404 行存 `/tmp/public-fund-research.md`（22KB），本节为摘要落档。

#### 一、参考性结论：中等（非强信号，作辅助维度有价值）

| 维度 | 结论 |
|---|---|
| 参考性等级 | **中**（季度披露为"滞后反向指标"；日更估算仓位为"实时同步指标"） |
| 核心限制 | 季报披露滞后 15 个工作日，披露时点持仓 ≠ 当前持仓 |
| 最有价值信号 | ①基金平均股票仓位（"88 魔咒"：>88% 见顶概率高，反向指标）②重仓股抱团度（集中度急升=风险积累，瓦解=见顶信号）③净申赎（净申购=散户乐观反向看空，净赎回=散户悲观反向看多） |
| 最佳用法 | 作辅助维度与北向/两融/产业资本联动，4 维共振时信号最强；单独使用价值有限 |

**学术背景**：①中国公募基金存在显著羊群效应（许年行等 2013《经济研究》），加剧"抱团-瓦解"循环 ②基金平均股票仓位与未来大盘收益负相关（统计显著），即"88 魔咒"（历史应验 2009/7、2015/5、2021/1 等）③2021/1 白酒新能源抱团瓦解领先沪深 300 见顶（2021/2/18 5930 点）约 1 个月 ④基金净申赎与未来 1-3 个月大盘收益负相关（散户情绪反向）。

**披露规则（决定参考性边界）**：季报季末+15 工作日披露（Q1≈4/22、Q2≈7/22、Q3≈10/22、年报≈3/31 前），季报只披露前十大重仓股+资产配置+行业配置，全部持仓要等中报（60 日内）/年报（90 日内）。滞后意味着披露的持仓已是过去时，基金可能已调仓，不能直接当现在持仓用。

#### 二、数据源：东财 fundf10 子页 + akshare 9 接口（全部免费可得）

**东方财富 fundf10 子页**（URL 模式 `https://fundf10.eastmoney.com/{页面代号}_{基金代码}.html`，已验证可访问）：
- `ccmx` 十大重仓股（季度）/ `jjcc` 持仓明细（半年）/ `hytz` 行业配置（季度）/ `zcpz` 资产配置含股票仓位+净资产规模（季度）/ `jbl` 持仓变动走势（季度）/ `cyem` 重仓股变化（季度）/ `jjjz` 历史净值（日更）/ `fhsp` 申赎状态（日更）

**akshare 接口**（已验证，免费无需 key，项目已用 akshare 实施成本极低）：
- `fund_portfolio_hold_em(symbol, date)` 十大重仓股（季度）
- `fund_individual_detail_hold_xq(symbol)` 大类资产比例（雪球，季度+估算）
- `fund_open_fund_daily_em()` 全部基金日净值+涨幅（每交易日 16:00-23:00）
- `fund_open_fund_info_em(symbol, indicator)` 单基金历史净值/累计收益率/分红/拆分（日更）
- `fund_value_estimation_em()` **基金净值估算**（盘中实时估算，日更）
- `fund_etf_spot_em()` ETF 实时行情（实时）
- `fund_purchase_em()` 申赎状态（日更）
- `fund_name_em()` 全部基金基本信息（单次）
- `fund_portfolio_industry_allocation_em` 行业配置 / `fund_portfolio_change_em` 持仓变动 / `fund_scale_change_em` 规模变动（需源码确认，通常 akshare 有）

**数据频率分类**：
- 季度硬数据（季报）：十大重仓 / 行业配置 / 资产配置（股票仓位比例）/ 重仓变动
- 日更软数据（估算/衍生）：基金净值 / 净值估算（盘中实时）/ 申赎状态 / ETF 实时行情
- 无法日更的硬限制：真实股票仓位仅季报披露（季末时点），日更只能靠净值估算反推（误差 ±5%）；真实净申赎金额仅季报披露份额变动，日更只有"申赎状态"（是否暂停申购）

#### 三、设计方案：5 核心指标 + 新独立 tab + 首页角标 + 4 维资金面共振

##### 3.1 采集设计

**季度全量采集**（季报披露后触发，时点 1/22、4/22、7/22、10/22、3/31 后次日 03:00）：
- 范围：全市场偏股混合+灵活配置+股票型约 4000-5000 只，按规模排序取前 500 只（覆盖 80%+ 规模）
...
- 方式：`ak.fund_portfolio_hold_em(symbol, date)` 十大重仓 + fundf10 子页爬虫兜底（行业/资产/变动）
- 耗时：500 只 × ~0.5s/只 ≈ 4-5 分钟（限流 sleep 后）

**日更轻量采集**（盘中估算仓位，每交易日 16:30）：
- 字段：fund_code/fund_name/date + estimated_nav/estimated_change_pct（`fund_value_estimation_em`）+ actual_nav（T+1 确认）
- 用途：估算净值涨跌 vs 实际涨跌反推仓位变化（粗略），或直接追踪基金表现

##### 3.2 指标设计（5 核心 + 3 衍生）

| 类型 | 指标 | 计算 | 信号 |
|---|---|---|---|
| 核心 | 基金平均股票仓位 | 加权平均（按净资产）`Σ(stock_ratio×net_asset)/Σ(net_asset)` | >88%=见顶；<80%=见底 |
| 核心 | 重仓股抱团度（Herfindahl） | `Σ(weight²)` 加总全市场基金前十大集中度 | 急升=风险积累；急降=瓦解 |
| 核心 | 重仓股重叠度 | 头部 100 只基金前十大重仓股去重数/1000 | <300=高度抱团；>500=分散 |
| 核心 | 行业配置集中度 | Top3 行业占比之和 | >60%=高度集中 |
| 核心 | 基金净申赎率 | `Σ(份额变化×净值)/Σ(总规模)` | 净申购激增=见顶；净赎回激增=见底 |
| 衍生 | 基金加仓/减仓比例 | 加仓基金数/减仓基金数 | 情绪面方向 |
| 衍生 | 头部基金调仓方向 | 规模 Top20 基金仓位变化均值 | 顶流整体方向 |
| 衍生 | 重仓股 Top30 集中度 | 全市场基金前十大中前 30 只股票总占比 | "核心资产"集中度 |

##### 3.3 展示方案

**新独立 tab「基金持仓」**（遵循 memory `new-feature-isolated-tab-first`：新功能先单开 tab 验证后融合）：
...
**首页角标**（轻量接入）：首页"资金面"卡片组增加「基金仓位」角标，显示当前平均仓位（如 `89.2%⚠️`）+ 较上季变化（如 `↑1.2pct`），颜色 >88% 红/80-88% 黄/<80% 绿，点击跳转 tab 详情。

##### 3.4 4 维资金面共振

加入现有"资金面"模块（北向/两融/产业资本），构成 4 维：
- 北向资金（外资，日更）：聪明钱
- 两融余额（杠杆资金，日更）：杠杆情绪
- 产业资本（重要股东，月更）：内部人视角
- 基金持仓（机构资金，季更）：机构仓位（新增）

**4 维共振信号最强**：例如"北向流出 + 两融下降 + 产业资本减持 + 基金减仓"=4 维共振看空。

**信号灯规则**：仓位 >88% → "基金仓位高位 ⚠️ 88 魔咒见顶"；<80% → "基金仓位低位 ✅ 80 抄底见底"；抱团度 >0.20 且重叠度 <320 → "重仓股高度抱团 ⚠️ 抱团瓦解风险"；净申赎 >500 亿 → "基金净申购激增 ⚠️ 散户乐观反向看空"；<-500 亿 → "基金净赎回激增 ✅ 散户悲观反向看多"。

#### 四、优先级 + 复杂度 + 风险

**优先级：P1（中等，排期做）**
- ✅ 参考性中等作辅助维度有价值（与现有 3 维资金面构成完整拼图）
- ✅ 数据可得性高（免费 + akshare 封装，无需付费终端）
- ✅ 实现复杂度可控（~3.5 天）
- ⚠️ 滞后性限制大，不能作主信号，优先级低于现有日更资金面
- ⚠️ 当前已有两融/北向/外资日更资金面，基金持仓为季度补充，边际价值递减
- 不作 P0（现有资金面不缺）也不作 P2（本方案含信号联动，非仅展示）

**实现复杂度（合计 ~3.5 天）**：
| 模块 | 工时 |
|---|---|
| 季度全量采集脚本（akshare + 爬虫兜底） | 1 天 |
| 日更轻量采集（净值估算） | 0.5 天 |
| 指标计算（5 核心 + 3 衍生，需历史回填） | 0.5 天 |
| 前端新 tab 页面（图表+热力图） | 1 天 |
| 首页角标接入 | 0.2 天 |
| 信号灯规则接入 | 0.3 天 |
...
| 风险 | 影响 | 缓解 |
|---|---|---|
| 滞后性误导 | 把过去持仓当现在用，信号失真 | 页面醒目提示"数据截止日期+已滞后天数"；不作主信号 |
| 抱团误导 | 抱团股未必瓦解（如茅台抱团 5 年才瓦解） | 仅作辅助信号，需结合估值（PE 历史分位）确认 |
| 披露规则限制 | 季报只披露前十大，全持仓要等中报/年报 | 用十大重仓+行业配置+资产配置三维度交叉验证 |
| 样本偏差 | 只看头部 500 只基金，忽略小基金 | 加权方式用净资产规模加权，大基金权重大 |
| 数据质量 | akshare 接口偶尔失败/数据缺失 | 采集脚本加 retry + 缺失告警；关键指标多源校验 |
| 88 魔咒失效 | 历史规律未必未来应验（2020 仓位持续 90%+ 大盘仍涨） | 仅作"风险提示"非"卖出信号"；结合其他维度共振 |
| 估算仓位误差 | 日更估算仓位（净值反推）误差大（±5%） | 估算仓位仅作趋势参考，不作精确信号；季报披露后校正 |

#### 五、实证案例（2026-07-20 调研抓取）

**易方达蓝筹精选混合（005827，张坤）资产配置**：

| 报告期 | 股票占净比 | 净资产（亿） | 较上期 |
|---|---|---|---|
| 2026-06-30 | 76.43% | 204.16 | 仓位 -17.38pct，规模 -23.80% |
| 2026-03-31 | 93.81% | 268.07 | - |
| 历史长期 | 92%-95% | - | 高仓位运行 |

**解读**：头部顶流基金 2026Q2 大幅减仓 17.38 个百分点 + 规模缩水 23.80%（净值下跌+赎回），是 2026 年最显著的"头部基金减仓+规模缩水"信号，**若随后大盘走弱则印证参考性**。

#### 六、待办动作清单

1. [ ] 季度全量采集脚本（akshare `fund_portfolio_hold_em` + fundf10 子页爬虫兜底，500 只头部基金，时点 1/22、4/22、7/22、10/22、3/31 后次日 03:00）
2. [ ] 日更轻量采集（akshare `fund_value_estimation_em`，每交易日 16:30）
3. [ ] 5 核心指标计算（平均仓位/抱团度/重叠度/行业集中度/净申赎率）+ 3 衍生指标（加仓减仓比/头部调仓/Top30 集中度）
4. [ ] 前端新 tab「基金持仓」页面（4 信号灯卡片 + 仓位 vs 上证主图 + Top30 排行 + 行业热力图 + 头部调仓 Top20 + 滞后性提示）
5. [ ] 首页角标接入（平均仓位+颜色 >88% 红/80-88% 黄/<80% 绿）
6. [ ] 信号灯规则接入（88 魔咒/80 抄底/抱团瓦解/净申赎反向）
7. [ ] 4 维资金面共振联动（北向/两融/产业资本/基金持仓）

**状态**：纯调研落档，待用户排期。详见 TASKS.md 「公募基金持仓佐证大盘」待办条目。完整调研报告 404 行存 `/tmp/public-fund-research.md`（含学术背景/数据源对比/采集字段/指标计算/展示 ASCII 示意图/风险缓解/附录调研过程）。

#### 七、方案修正（2026-07-31 用户定）

原方案「前500只+日更估算放 P1.5 后续」修正为「**全量采集 + 日更估算一步到位**」：

**①采集量：前500只 -> 全量（4000-5000只）**
- 理由1：季度才跑一次（每年 5 个时点 1/22、4/22、7/22、10/22、3/31 后次日 03:00），即使 30-50 分钟耗时也完全可接受，非日常瓶颈
- 理由2：抱团度/重叠度是**计数集中度指标**，前500只漏小基金重仓股会算偏（小基金虽规模小但持仓集中度高，对集中度分布的尾部贡献不可忽略），全量更准
- 理由3：净资产规模加权下小基金不污染平均仓位（权重低），但完整反映全市场抱团结构（计数维度不被规模稀释），所以全量采既不损失平均仓位精度又能保住集中度完整性

**②日更估算：P1.5 后续 -> 一步到位**
- 理由：日更估算与季度硬数据**互补不互斥**--季度给绝对值（88 魔咒/抱团度/净申赎的精准阈值触发），日更填补 15 天披露滞后窗口的仓位趋势（>88/<80 阈值仅趋势参考非精确触发，季报披露后校正）
- 一步到位完整，不留「后续再补」尾巴（遵循 memory `proposal-default-criteria-no-compromise` 终极完整合集方案不妥协）

**③排期：2026-08-01 白天实施**
- 周末不开盘无冲突，盘中禁全量 export+deploy 约束不触发（本任务不跑 export+deploy）

**④风险与缓解**：
- 全量反爬风险：东财 fundf10 4000-5000 只 × 7 子页可能触发频控/封 IP。缓解：延时 + retry + 断点续采（记录已采 fund_code 到 `/tmp/fund-collect-progress.json`，重跑跳过已采），最坏降级头部 1000 只（按净资产排序覆盖 95%+ 规模）
- 日更 ±5% 误差：88-80 阈值仅趋势参考非精确触发，季报披露后用硬数据校正估算漂移
...
## §48 小节AO：2026-08-05 晚 A股自动买卖可行路径调研（3 agent 综合落档）

### 1. 调研背景

用户做本网站的初衷是：**分析出信号 -> 最终信号可信度对应的自动买卖**。情绪看板目前已完成信号生成、AI 评分、买卖清单、ETF 评分弹窗、回测实验室等环节，自动买卖是信号链路的终点。本次调研中国 A 股散户可落地的自动买卖可行路径，并明确合规边界与风险，为远期分阶段实施提供依据。调研由 3 个 agent 并行完成（券商 API / 第三方平台+合规 / RPA 桌面客户端），进度文件见 `/tmp/agent-progress-auto-trade-{1,2,3}.md`。

### 2. 三条可行路径

#### 路径一：券商官方交易 API（QMT / Ptrade）

- **QMT（迅投迅极量化终端）**：券商申请开通，散户常用 miniQMT 精简版（完整版 QMT 门槛 50 万起，部分券商 100 万）。
  - **2026 门槛**：miniQMT 约 10 万资产（20 日日均）起，个别券商无门槛无费用；国金证券明确支持。
  - **SDK**：xtquant（Python3.6-3.12），含 XtData 行情 + XtTrader 交易两模块；下单/撤单/查询资产/委托/成交/持仓完整 API；需先启动 MiniQMT 客户端。
  - **执行方式**：**本地执行**（QMT 客户端须运行，Windows 平台无 Mac 原生版，Mac 需 Parallels 虚拟机）。
- **Ptrade（恒生电子）**：券商申请开通，门槛多数 50 万，部分券商几乎无门槛；支持国金/华泰/安信等。
  - **执行方式**：**服务端托管**（无需本地挂机，比 QMT 灵活稳定），适合日内回转/篮子/网格/追涨停/盘口扫单。
- **其他券商自研 API**：华泰 matic（打电话给营业部购买，散户门槛高）、国泰君安君弘、矩阵等，信息少且散户可达性低。

#### 路径二：第三方量化平台（多需对接券商 API）

- **聚宽 JoinQuant**：不直接实盘，**需 QMT 中介对接**券商才能实盘交易。
- **米筐 RQAlpha**：开源框架，支持实盘但需 Mod 对接券商（文档无细节）。
- **掘金量化**：有实盘交易文档，支持券商对接（文档抓取 403，详情待补）。
- **BigQuant BigTrader**：终端支持股票实盘，对接 5 家券商（湘财/万和/信达/东亚前海/中天国富），Windows 文件单模式，旗舰版付费；一创实盘已关。
- **优矿 / 万矿**：偏向纯研究平台，实盘支持弱。

#### 路径三：RPA 控制桌面客户端（模拟键鼠，不需券商授权）

- **easytrader**（GitHub 10k stars，MIT，2026-02 仍活跃维护）：模拟 GUI 键盘鼠标剪切板操作同花顺/雪球/国金/海通/华泰客户端；支持 miniQMT 官方接口模式。
  - **底层依赖**：pywinauto 0.6.6（Windows GUI 自动化专用），核心交易功能**仅 Windows 可运行**，Mac 原生不支持。
  - **痛点**：同花顺反自动化，频繁操作触发验证码（issue #448/#452/#461 长期未解决）；新版客户端界面变更致适配失效（#524 2026-03）；交易过程中不可使用鼠标（抢占焦点）。
- **同类项目**：skyformat99/ths_trade（pywinauto 控制同花顺 xiadan.exe + tornado WEB API，449 stars，低更新）、nladuo/THSTrader（手机版 uiautomator2 图色，804 stars 2023）、limitget/THS（逆向 Android TCP 协议，仅可转债申购，法律+稳定性风险高，不适合生产）。
- **Mac 方案**：Hammerspoon（Lua 脚本，hs.application/hs.eventtap 控制窗口+模拟键鼠，依赖无障碍权限，国产客户端无障碍支持大概率弱）+ PyAutoGUI（坐标+图像识别，脆弱）。Mac 版同花顺无障碍支持程度未知，RPA 在 Mac 可行性远低于 Windows。

### 3. 合规边界（2026 现行新规，核心红线）

2026 年程序化交易新规已落地，**核心要求**：

- **报备前置**：所有程序化交易者须**事前报备**，无频率豁免，**先备案后交易**。
- **高频认定收紧**（较 2024 旧规 300 笔/秒收紧 20 倍）：**每秒 >=15 笔** 或 **单日 >=20000 笔**即认定为高频交易，适用差异化收费（流量费/撤单费）。
- **撤单限制**：每秒撤单 <=15 笔，单日撤单率 <=15%，最小停留时间 >=50 微秒。
- **日志留存**：5 年。
- **落地时间**：2026-04-07 第一阶段，2026-07-07 全面落地；上交所细则 2025-07-07 生效，期货市场规定 2025-10-09 生效。
- **违规后果**：警示 / 监管谈话 / 暂停限制交易 / 公开谴责 / **限制交易 1 个月** / 诚信档案 3-5 年。
- **穿透式监管**：不得分拆产品规避高频认定，高频从严从重。
- **监控标准**（君合律所梳理）：四类异常--瞬时申报速率/频繁瞬时撤单/频繁拉抬打压/短时间大额成交，具体阈值未公开，试运行中。

### 4. 风险评估

- **合规风险**：RPA 方案（easytrader）不需券商授权即操作客户端，属合规风险点；所有程序化交易（含低频）均须备案，**未备案即交易 = 违规**。
- **个人低频**：<15 笔/秒不受高频严格限制，**但仍需备案**；频繁虚假挂撤单仍受监控。
- **技术风险**：RPA 滑点/故障/重复下单/漏单；同花顺反自动化验证码；新版客户端适配失效；逆向协议方案法律+稳定性风险。
- **平台风险**：第三方平台实盘多需对接券商 API（聚宽需 QMT 中介），非独立路径；部分平台实盘已关（一创）。
- **Mac 劣势**：QMT/Ptrade/easytrader 均为 Windows 平台，Mac 需虚拟机或转 Hammerspoon（无障碍支持弱），自动买卖在 Mac 可达性显著低于 Windows。

### 5. 落地建议（分阶段，合规前置）

- **阶段 1：人工 + 信号辅助**（当前已具备）
  - 信号看板 + AI 评分 + 买卖清单 + ETF 评分弹窗 + 回测实验室已上线，人工决策+手动下单。
  - 不涉及程序化交易报备，零合规风险。
- **阶段 2：半自动 RPA 低频备案后**（中期，需先备案）
  - 选 easytrader + miniQMT 官方接口模式（非纯 GUI 模拟，稳定性更高），Windows 环境运行。
  - **合规前置**：先向券商/交易所完成程序化交易报备，再做低频自动下单（远低于 15 笔/秒）。
  - 适用场景：信号触发后自动执行备买/追买等低频操作，人工复核确认。
- **阶段 3：QMT 全自动**（远期，门槛+合规均需满足）
  - 开通 miniQMT（10 万门槛）或 Ptrade（服务端托管更稳），用 xtquant SDK 全链路自动买卖。
  - **合规前置**：完成报备 + 日志留存 5 年机制 + 撤单率/频率监控熔断（确保 <15 笔/秒、撤单率 <15%）。
  - 适用场景：信号可信度达阈值后全自动执行，含风控熔断+持仓上限+异常告警。

### 6. 推荐排序与风控熔断（补充：原落档缺推荐排序+风控熔断清单+落地步骤+合规优势+时效提醒）

补充自原调研 agent 最终报告（比 commit e478c2afc 落档版更详细，含方案对比表/推荐排序/风控熔断/落地步骤）。本节针对用户场景（Mac + 低频信号驱动）给出可立即决策的排序与配套。

#### 6.1 推荐排序（用户 Mac + 低频信号场景）

- **首选 Ptrade**：服务端托管（关机也跑），Mac 免虚拟机浏览器管理，合规官方 API + 报备，封号风险低；痛点多数券商 50 万门槛（部分无门槛，需咨询客户经理）。
- **次选 miniQMT + Windows 虚拟机/云服务器**：10 万低门槛，xtquant 接口文档详尽可立即开发；代价 Mac 需 Parallels 常驻 + 客户端常开（断线即无法下单），或租 Windows 云服务器解决常驻。
- **不推荐 easytrader + 同花顺**：验证码长期未解 + 灰色合规 + 高维护。
- **同花顺/通达信无合规可用官方交易 API**：iFinD 是数据终端非交易/委托 API，无官方开放，逆向违法风险。

#### 6.2 风控熔断清单（必须配套）

- 单笔金额/数量上限 + 每日止损线（达 X% 自动停）。
- 撤单率远低于 15% + 重复下单幂等校验。
- 仓位红线（单票上限/总仓位上限）。
- 限价单为主（避免市价单滑点）。
- 异常告警（API 断开/数据错误/价格异常自动停 + 通知）。
- 日志留存 >=5 年（合规要求）。
- 灾备：Ptrade 服务端托管（首选）或 QMT + UPS + 网络双链路。

#### 6.3 落地步骤

选券商开户（确认 Ptrade/miniQMT 门槛）-> 完成程序化交易报备（先报告后交易）-> 架构打通（信号看板 -> 信号文件/API -> Ptrade/miniQMT 策略读取执行）-> 模拟盘验证（hxquant.com QMT 模拟账号）-> 小资金实盘（保留人工确认）-> 逐步放量。

#### 6.4 关键合规优势

个人低频信号驱动（一天几笔到几十笔）远低于高频线（15 笔/秒 / 2 万笔日），用官方 API + 报备 = 完全合规；T+1 制度下低频天然合规，是监管表态"基本不受影响"群体。

#### 6.5 信息时效提醒

关键数字（门槛 50 万/10 万、高频 15 笔/秒）来自 2025-2026 报道 + 搜索片段，落地前向券商客户经理二次确认最新门槛与政策；xtquant 函数签名来自迅投官方文档直抓可靠可立即开发。

### 7. 桌面端控制其他方法调研（easytrader 以外，2026-08-05 补充）

补充调研 easytrader 以外的桌面端控制方法（开源 RPA 库 / 商业 RPA / 硬件宏），以及跳出"桌面端控制"范畴的更优解。完整对比表与逐项分析见 `/tmp/desktop-trade-methods.md`（22KB），进度文件 `/tmp/agent-progress-desktop-methods.md`。

#### 7.1 核心结论：桌面端控制范畴内无比 easytrader 更优开源方案

- **easytrader 本身基于 pywinauto 封装**同花顺操作，直接用 pywinauto 等于用其底层且更难（easytrader 已封装同花顺控件定位逻辑）。
- **uiautomation（yinkaisheng，3.5k star）明确不支持 DirectUI 自绘控件**（GitHub 原文说明），而同花顺/通达信正是 DirectUI，内部控件无法识别，基本不可用。
- **pywinauto（6.1k star）业余项目维护**（README 自述 "hobby project"），504 open issues；对 DirectUI 同样力不从心，实际退化坐标点击。
- **SikuliX 原作者 RaiMan 停更转 Oculix**（官网确认），分辨率/主题/DPI 变化即失配，已过时。
- **Hammerspoon（Mac）** 对第三方应用无障碍支持差，Mac 上 A 股客户端功能弱，不现实。
- **pyautogui** 纯坐标最脆弱；**AutoHotkey** ControlSend 可发快捷键但需手工摸索按键序列、维护成本高；**Windows UIAutomation 直调** 同 uiautomation 限制。
- 综上：开源桌面端控制库均不比 easytrader 更优（要么是其底层、要么 DirectUI 不可用、要么停更/脆弱）。

#### 7.2 跳出桌面端控制更优解（重点）

- **① miniQMT + xtquant（券商官方 API）**：10 万门槛（国金等可更低），合规稳定，无验证码/客户端更新失效问题，是"自动下单"终极方案。xtquant 可完全脱离 easytrader 独立使用（`from xtquant import xttrader`）。详见本小节路径一。
- **② 券商/行情软件自带条件单（新发现的重要路径）**：
  - **同花顺 iFinD 条件单**（WebFetch 确认）：价格触发 / 时间触发 / **云条件单** / 预警 / 自动下单。
  - **云条件单存云端，关机/断网也触发执行**——相比 RPA 桌面端控制（需电脑常开）的巨大优势。
  - **完全合规**：券商/行情软件经监管批准的官方功能，券商系统内执行，用户使用**不需单独报备程序化交易**（券商已就其系统合规化）。
  - **免费 + Mac/手机可用**。
  - **限制**：只能简单条件触发（到价/涨跌幅/时间），不能跑复杂策略（多因子/均线交叉）。
  - **重点判断**：对"低频信号已算好，只需到价执行"的场景，**券商条件单可能已满足需求，无需 RPA**。
- 通达信条件单原理同同花顺（Mac 版功能弱）；各券商 APP（华泰涨乐财富通/国金佣金宝/平安等）普遍支持条件单/网格/定投，均官方合规免费云端执行。

#### 7.3 商业 RPA 影刀

- **影刀 RPA**（官网确认）：可视化拖拽 0 编程，支持 Windows + Mac 桌面软件自动化，AI 辅助搭建。
- **图像识别 fallback 绕过 DirectUI**：影刀有"图像识别"指令，可绕过开源库的 DirectUI 控件识别硬伤，用图像匹配 + 坐标点击操作同花顺/通达信——这是商业 RPA 相比开源库的主要优势（开源库遇 DirectUI 基本卡死）。
- **金融场景**：官网有金融板块（银行/保险/证券/基金/风控），证券场景列了"业务清算/估值处理/夜间清算"，**未点名同花顺/通达信**，交易下单案例未见。
- **收费**：个人免费，企业版联系报价。**个人免费可下载实测同花顺操作可行性**。
- UiBot 同类（企业级更强但收费更高）；实在智能/八爪鱼/来也能力相近需逐一实测。
- 商业 RPA 共性劣势：金融交易下单案例少（多为清算/数据搬运）、图像识别仍坐标依赖、企业版收费。

#### 7.4 vnpy（VeighNa，44.2k star）

- 开源量化框架（GitHub 确认），支持 CTA 策略/算法交易（TWAP/Sniper/Iceberg）/价差/期权等。
- **支持的 A 股券商机构接口**：中泰 XTP（A股、ETF期权）/ 华鑫奇点（A股、ETF期权）/ 东证 OST（A股）/ 东方财富 EMT（A股）。
- **属券商 API 路径，非桌面 RPA**：机构级接口，个人门槛比 miniQMT 更高（通常需机构客户或高净值），适合专业量化用户。功能最全的开源量化方案。

#### 7.5 硬件宏反检测在交易场景无用

- 罗技 G HUB / 雷蛇 Synapse / 硬件宏键鼠：宏存固件由硬件驱动执行，操作系统层面与真实人工输入无法区分，**反检测最强**（游戏反作弊都难检测）。
- **但股票交易软件不做反作弊检测**（不像游戏会封号），所以硬件宏的反检测优势在交易场景完全不显著——用 pyautogui 和用罗技宏对同花顺没区别。
- 仍模拟人工操作，合规性质不变（灰色）；唯一价值"不依赖 Python 环境"，但仍需 Windows + 同花顺常开 + 坐标/图像脆弱问题不变。**不推荐为此选硬件宏**。

#### 7.6 对原推荐排序的补充（重要）

- **券商条件单是新发现的重要路径**：官方、免费、云端、合规、低门槛，低频信号驱动场景可能优于 RPA。
- **建议阶段 2 优先评估券商条件单而非 easytrader**：用户的低频信号若能转化为"到价下单"规则（如价格到 X 买入），直接用云条件单，免一切 RPA 折腾（验证码/灰色合规/高维护全规避）。
- **如券商条件单满足需求，可跳过 RPA 直接到 QMT 全自动**（阶段 3）：阶段 2 评估条件单 -> 够用则 RPA 阶段跳过 -> 不够用且资金达标走 miniQMT -> 仍不够再考虑影刀 RPA 实测。
- 修订后优先级：**① 券商条件单（先评估）> ② miniQMT + xtquant（资金达标）> ③ 影刀 RPA（一定要桌面端控制）> ④ easytrader（不推荐）**。
- 桌面端 RPA 共同硬伤（换工具解决不了）：客户端更新即失效 / 需 Windows 常开 / 验证码 / 合规灰色 / 维护成本高。**根本解法是不走桌面端控制，走官方 API 或官方功能（条件单）**。

### 8. 关键结论

1. **合规是第一前置条件**：任何程序化交易（含个人低频 RPA）均须先备案后交易，未备案即违规。
2. **券商 API（QMT/Ptrade）是正规路径**：2026 门槛降至 10 万，散户可达，合规清晰；Ptrade 服务端托管更稳。
3. **RPA 是灰色地带**：不需券商授权 = 合规风险点，且技术不稳定（验证码/适配失效），仅作过渡。
4. **Mac 是劣势**：所有主流方案均为 Windows，Mac 需虚拟机或弱化方案。
5. **当前阶段 1 已完成**：信号链路已通到"人工决策+手动下单"，自动买卖为远期增量，不阻塞当前迭代。

### 9. 关联

- memory `trade-sentiment-dashboard`（项目入口：信号->自动买卖初衷）
- TASKS.md "自动买卖（长期方向）"远期待办（分阶段+合规前置）
- §14 生产稳定性 P0（自动买卖涉及实盘交易，稳定性要求最高，风控熔断为阶段 3 必备）
- §5 调研后给方案（本次为完整正确调研落档，不偷懒不妥协，三路径+合规+风险+分阶段全闭环）
