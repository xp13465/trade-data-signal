...
### 小节AV：sh 专属 C1 降尖尖上线（2026-07-22，替代小节AU sh 豁免）

> 接小节AU。方案B + sh 豁免上线后，sh 豁免致 sh 尖尖率仍 10.38%（10 指数最高，其他 6-9%）。用户诉求 sh 也降尖尖。派 agent 调研 sh 专属降尖尖方案，对比方案 B 对 sh 误滤根因，提出 C1 替代豁免（`atr_pct>=2.5% OR dist_from_high>=15%`）。用户确认接受 C1。本小节实施上线。
...
**C1 sh 实测效果**（vs sh 豁免基线，全维度改善无反害）：
| 指标 | sh 豁免基线 | C1 保留 | 变化 |
|---|---|---|---|
...
- compute()+store() 实跑验证：sh buy_special 742->612，total buy_special 12369->12239（减 130=sh 单独减少，其他 9 指数不变：sz 303/hs300 303/csi500 329/csi_div 241 与小节AU 一致）
...
- sh 指数：用 C1 `(atr_pct>=2.5%) OR (dist_from_high>=0.15)` 替代原豁免（pd.Series(False)）
- sh C1 不影响其他指数，其他指数方案 B 不影响 sh，互不干扰
- 三层过滤叠加关系不变：B4 close 站稳（第一层）+ h5 R2 真过滤（第二层）+ peak_dd_filter（第三层，sh 用 C1 / 其他用方案 B），任一层命中即 drop

**改动 3 处**（signals.py，约 15 行）：
1. **L805-807 新增 dist_from_high 计算**：`high_250 = high.rolling(250, min_periods=1).max()` / `dist_from_high = (high_250 - close) / high_250`（在 dist_from_low60 计算后，2 行）
2. **L809-812 改 sh 分支**：原 `if iid == "sh": peak_dd_filter_mask = pd.Series(False, ...)` 改为 `peak_dd_filter_mask = ((atr_pct >= 0.025) | (dist_from_high >= 0.15)).fillna(False)`
3. **L786-803 + L828-831 注释更新**：说明 sh 用 C1 替代豁免（atr OR dist_from_high>=15%），其他 9 指数继续方案 B，含 C1 sh 实测全维度改善数据 + 核心洞察（dist_from_low60 误滤，dist_from_high 精准滤低位假突破）

**风险**：
1. **sh 专属偏置**：C1 仅对 sh 应用 dist_from_high>=15%，其他 9 指数继续用 dist_from_low60>30%。若未来 sh 市场特征变化（趋势性减弱转震荡市），dist_from_high 阈值可能需重调。监测：sh mdd_20d + 尖尖率 + ret20 vs 其他指数
...
4. **sh 尖尖率仍 7.35%**：虽降 3.02pp 但仍高于其他指数均值（6-9%），因 sh 趋势性强本质高波动。进一步降需更激进过滤（损 ret20），暂不再加严

**git**：commit 待定。push feat/sh-c1-peak-filter + push feat:main（fast-forward）。根 `data/`（signal_stats.json/sw_components.json）未 add 保持本地 M。本小节改 `app/compute/signals.py`（约 15 行）+ NOTES.md + TASKS.md 落档。
