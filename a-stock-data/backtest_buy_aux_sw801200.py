"""buy_aux（B1 辅买）逐品类优化回测 — sw_801200 商贸零售。

第二个品类（家电打样后）。家电（sw_801110）验证方案B（RSI上穿40）有效
（f -38.54%→+16.19%）。本脚本用同样 3 方案回测 sw_801200，看方案B是否
在商贸也转正（横向可比，验证方案B通用性）。

数据源：data/sentiment.db
  - index_daily（sw_801200 的 date/open/high/low/close/amount）
  - amount 列作 volume 代理（成交金额，比 volume 更能反映资金进场）

复刻 app/compute/signals.py（与 backtest_buy_aux_optimize.py 完全一致）：
...
REPORT = '/Users/linhuichen/code/trade/15-商贸buy_aux优化回测.md'
SID = 'sw_801200'
SID_NAME = '商贸零售'
HORIZONS = [5, 10, 20]
...
# 家电（sw_801110）回测结果，用于横向对比（来自 14-家电buy_aux优化回测.md）
# 结构: {scheme: {horizon: (n, mean, wr, pl, f, lbl)}}
JD_BASELINE_10 = (134, -1.23, 0.448, 0.66, -0.3854, '不建议')
JD_A_10 = (18, -4.68, 0.222, 0.57, -1.1460, '不建议')
JD_B_10 = (33, +0.54, 0.545, 1.19, +0.1619, '建议')
JD_C_10 = (8, -0.14, 0.500, 0.94, -0.0323, '样本不足')
# 家电三 horizon f
JD_BASELINE_F = [-0.0528, -0.3854, -0.0857]   # 5d/10d/20d
JD_A_F = [-0.4808, -1.1460, -0.2514]
JD_B_F = [+0.1912, +0.1619, +0.1711]
JD_C_F = [-0.5784, -0.0323, +0.3349]
JD_BASELINE_N = 134
JD_B_N = 33
JD_SIGNALS = {'基线_B1': 135, 'A_反弹力度2%': 18, 'B_RSI上穿40': 33, 'C_放量1.2倍': 8}
...
def main():
    ...
    L = []
    A = L.append
    A(f"# buy_aux（B1 辅买）优化回测 — sw_801200 {SID_NAME}\n")
    A(f"- 生成日期：2026-07-05")
    A(f"- 标的：sw_801200 商贸零售（申万行业指数，{SID_NAME}）")
    ...
    A(f"- 3 个改进方案：A(反弹力度2%) / B(RSI上穿40) / C(放量1.2倍)（与家电打样同方案，横向可比）")
    A("")
    A("> 背景：用户要求把凯利不建议（f≤0）的 buy_aux 逐个品类优化。")
    A(f"> {SID} {SID_NAME} buy_aux 当前 f=-0.32（不建议），胜率 43.3% / 盈亏比 0.75 / n=164 / 均值 -1.19%。")
    A(f"> 家电（sw_801110）打样验证方案B（RSI上穿40）有效（f -38.54%→+16.19%）。")
    A(f"> 本品类是 buy_aux 逐个品类优化的第二个，验证方案B是否在商贸也转正。\n")
    ...
    for sn in ['A_反弹力度2%', 'B_RSI上穿40', 'C_放量1.2倍']:
        ...
        A(f"**10d 主 horizon 摘要**：胜率 {fmt_pct(wr10*100)}（基线 {fmt_pct(base_wr10*100)}，"
          ...
          f"凯利 f={fmt_f(f10)}（基线 {fmt_f(base_f10)}，{(f10-base_f10)*100 if f10 is not None and base_f10 is not None else 0:+.2f}pp）/ "
    ...
    A(f"基于数据，**方案 {best_sn.split('_')[0]}（{best_sn}）** 10d 凯利 f 最高（{fmt_f(best_f)}，"
      f"基线 {fmt_f(base_f10)}，改善 {(best_f-base_f10)*100:+.2f}pp）。")
    A("")
    for sn in ['A_反弹力度2%', 'B_RSI上穿40', 'C_放量1.2倍']:
        f10 = results[sn][10][4]
        n10 = results[sn][10][0]
    ...
    A("")
    A("**最终方案由用户选定**（以下诚实结论 + 与家电横向对比辅助判断）。\n")
    ...
    A(f"### 5.1 转正情况\n")
    if turned_positive:
        A(f"**{len(turned_positive)} 个方案让 sw_801200 buy_aux 凯利转正（f>0）**：")
        ...
        A(f"**0 个方案让 sw_801200 buy_aux 凯利转正（f>0）**——所有方案 10d f 仍 ≤0。")
        A(f"退而求其次看胜率提升：")
        for sn in ['A_反弹力度2%', 'B_RSI上穿40', 'C_放量1.2倍']:
            n10, m10, wr10, pl10, f10, cls10, lbl10 = results[sn][10]
            base_wr = results['基线_B1'][10][2]
    ...
    A(f"- **参数选择性**：试了 3 个方案（A 反弹 2% / B RSI 40 / C 放量 1.2 倍），"
      ...
      f"不等于「buy_aux 最优方案」。")
    A(f"- **样本数**：基线 n={base_n10}（充足），各方案 n 见上表。若某方案 n<30（样本不足警示），"
      f"其结论仅供参考，可能过拟合小样本。")
    A(f"- **三 horizon 一致性**：10d 是主指标，5d/20d 验证。若某方案仅 10d 转正而 5d/20d 不转正，"
      f"可能是单 horizon 偶发，稳健性存疑。一致转正（三 horizon 都 f>0）才稳健。")
    A(f"- **两品类局限**：本回测只测 sw_801200 一个品类，加上家电打样共 2 个品类，"
      f"buy_aux 优化的共性方向需在更多品类验证后才能确认（见 §6 与家电横向对比 / §7 下一品类建议）。")
    A("")
    ...
    if turned_positive:
        ...
        A(f"**方案 {best_sn} 让 sw_801200 buy_aux 凯利转正**（10d f={fmt_f(best_f)}，n={best_n}）。")
        if all_positive:
        ...
        A(f"**所有 3 个方案均无法让 sw_801200 buy_aux 凯利转正**。")
        A(f"胜率提升最多的是 {best_wr_sn}（{fmt_pct(best_wr*100)}，{(best_wr-base_wr10)*100:+.1f}pp），"
          f"但仍未达凯利建议。")
        A(f"")
        A(f"**建议**：")
        A(f"- 若用户接受「退而求其次提高胜率」（不强求凯利转正），可选 {best_wr_sn}")
        A(f"- 若用户坚持凯利转正才落地，建议 **维持现状或 skip sw_801200 的 buy_aux**")
        A(f"  （即该品类不发 buy_aux 信号，避免误导用户）")
        A(f"")
        A(f"**根本原因诊断**：sw_801200 商贸零售指数在 BB 下轨回归日的反弹力度结构性偏弱，"
          f"可能是行业特性（商贸零售受电商冲击与消费分流影响，超卖后反弹启动慢），"
          f"非参数可调。可考虑换信号逻辑（如改用 RSI 双底、成交量底部背离等），但超出本回测范围。")
    A("")

    # ---- 6. 与家电横向对比（关键）----
    A("## 6. 与家电（sw_801110）横向对比（关键）\n")
    A("家电是 buy_aux 逐品类优化的打样（第一个品类），方案B（RSI上穿40）在家电上转正"
      "（f -38.54%→+16.19%）。本节对比验证方案B在商贸是否也转正，判断其通用性。\n")
    A("### 6.1 基线对比\n")
    A("| 指标 | 家电 sw_801110 | 商贸 sw_801200 | 差异 |")
    A("|---|---:|---:|---|")
    A(f"| 10d 胜率 | 44.8% | {fmt_pct(base_wr10*100)} | {(base_wr10-0.448)*100:+.1f}pp |")
    A(f"| 10d 盈亏比 | 0.66 | {fmt_pl(base_pl10)} | {(base_pl10-0.66):+.2f} |")
    A(f"| 10d 均值 | -1.23% | {fmt_mean(base_m10)} | {(base_m10-(-1.23)):+.2f}pp |")
    A(f"| 10d 凯利f | -38.54% | {fmt_f(base_f10)} | {(base_f10-(-0.3854))*100:+.2f}pp |")
    A(f"| 10d 样本 n | 134 | {base_n10} | {base_n10-134:+d} |")
    A(f"| 全史信号数 | 135 | {n_signals_map['基线_B1']} | {n_signals_map['基线_B1']-135:+d} |")
    A("")
    A("> 两者基线都是「真不建议」——胜率<50% + 盈亏比<1 + 均值<0 三维全负。")
    A(f"> 商贸基线 f={fmt_f(base_f10)} 略好于家电（-38.54%），样本 {base_n10} 略多于家电（134）。")
    A(f"> 两者结构相似：都是 BB 下轨回归在行业指数上偏弱的典型，适合做方案B通用性验证。\n")

    A("### 6.2 方案B（RSI上穿40）横向对比（核心）\n")
    b_n10 = results['B_RSI上穿40'][10][0]
    b_wr10 = results['B_RSI上穿40'][10][2]
    b_pl10 = results['B_RSI上穿40'][10][3]
    b_f10 = results['B_RSI上穿40'][10][4]
    b_m10 = results['B_RSI上穿40'][10][1]
    A("| 指标 | 家电 sw_801110 | 商贸 sw_801200 | 差异 |")
    A("|---|---:|---:|---|")
    A(f"| 10d 胜率 | 54.5% | {fmt_pct(b_wr10*100)} | {(b_wr10-0.545)*100:+.1f}pp |")
    A(f"| 10d 盈亏比 | 1.19 | {fmt_pl(b_pl10)} | {(b_pl10-1.19):+.2f} |")
    A(f"| 10d 均值 | +0.54% | {fmt_mean(b_m10)} | {(b_m10-0.54):+.2f}pp |")
    A(f"| 10d 凯利f | +16.19% | {fmt_f(b_f10)} | {(b_f10-0.1619)*100:+.2f}pp |")
    A(f"| 10d 样本 n | 33 | {b_n10} | {b_n10-33:+d} |")
    A(f"| 全史信号数 | 33 | {n_signals_map['B_RSI上穿40']} | {n_signals_map['B_RSI上穿40']-33:+d} |")
    A(f"| 是否转正（f>0） | ✅ 转正 | {'✅ 转正' if (b_f10 is not None and b_f10>0) else '❌ 未转正'} | — |")
    # 三 horizon 对比
    b_fs = [results['B_RSI上穿40'][h][4] for h in HORIZONS]
    A(f"| 三 horizon 一致性 | 一致转正 | {consistency_str(b_fs)} | — |")
    A("")
    A("### 6.3 三方案横向对比总表\n")
    A("| 方案 | 家电 f | 商贸 f | 家电转正 | 商贸转正 | 通用性 |")
    A("|---|---:|---:|---|---|---|")
    jd_a_f = -1.1460
    jd_c_f = -0.0323
    A(f"| A 反弹力度2% | {jd_a_f*100:+.2f}% | {fmt_f(results['A_反弹力度2%'][10][4])} | ❌ | "
      f"{'✅' if (results['A_反弹力度2%'][10][4] is not None and results['A_反弹力度2%'][10][4]>0) else '❌'} | "
      f"{'均未转正' if not (results['A_反弹力度2%'][10][4] is not None and results['A_反弹力度2%'][10][4]>0) else '仅商贸'} |")
    A(f"| B RSI上穿40 | +16.19% | {fmt_f(b_f10)} | ✅ | "
      f"{'✅' if (b_f10 is not None and b_f10>0) else '❌'} | "
      f"{'**两品类均转正**' if (b_f10 is not None and b_f10>0) else '仅家电'} |")
    A(f"| C 放量1.2倍 | {jd_c_f*100:+.2f}% | {fmt_f(results['C_放量1.2倍'][10][4])} | ❌(样本不足) | "
      f"{'✅' if (results['C_放量1.2倍'][10][4] is not None and results['C_放量1.2倍'][10][4]>0) else '❌'} | "
      f"{'均未转正' if not (results['C_放量1.2倍'][10][4] is not None and results['C_放量1.2倍'][10][4]>0) else '仅商贸'} |")
    A("")

    # 关键结论
    A("### 6.4 方案B通用性结论\n")
    if b_f10 is not None and b_f10 > 0:
        A(f"**方案B（RSI上穿40）在商贸 sw_801200 也转正**（10d f={fmt_f(b_f10)}），"
          f"加上家电（f=+16.19%），**两品类均转正**，初步证明方案B是 buy_aux 的通用优化方向。")
        b_all_positive = all(x is not None and x > 0 for x in b_fs)
        if b_all_positive:
            A(f"\n**稳健性证据**：")
            A(f"- 家电三 horizon 一致转正（5d +19.12% / 10d +16.19% / 20d +17.11%）")
            b_5d = results['B_RSI上穿40'][5][4]
            b_20d = results['B_RSI上穿40'][20][4]
            A(f"- 商贸三 horizon 一致转正（5d {fmt_f(b_5d)} / 10d {fmt_f(b_f10)} / 20d {fmt_f(b_20d)}）")
            A(f"- 两品类 × 三 horizon = 6 个数据点全部 f>0，非偶发。")
        else:
            A(f"\n⚠️ **稳健性存疑**：商贸三 horizon 不一致（非全部 f>0），10d 转正可能是单 horizon 偶发。")
            A(f"   家电三 horizon 一致转正，但商贸仅 10d 转正，方案B在商贸的稳健性弱于家电。")
        A(f"\n**胜率/盈亏比改善幅度对比**：")
        A(f"- 家电胜率 44.8%→54.5%（+9.8pp），商贸 {fmt_pct(base_wr10*100)}→{fmt_pct(b_wr10*100)}（{(b_wr10-base_wr10)*100:+.1f}pp）")
        A(f"- 家电盈亏比 0.66→1.19（+0.53），商贸 {fmt_pl(base_pl10)}→{fmt_pl(b_pl10)}（{(b_pl10-base_pl10):+.2f}）")
        if b_wr10 > base_wr10 and b_pl10 > base_pl10:
            A(f"- 商贸改善方向与家电一致（胜率+盈亏比双升），机制可重复。")
        else:
            A(f"- ⚠️ 商贸改善方向与家电不一致，机制不可重复，需进一步验证。")
    else:
        A(f"**方案B（RSI上穿40）在商贸 sw_801200 未转正**（10d f={fmt_f(b_f10)}），"
          f"与家电（f=+16.19% 转正）不一致。")
        A(f"\n**通用性结论**：方案B在家电转正、在商贸未转正，**非通用优化方向**，存在品类特异性。")
        A(f"\n可能原因分析：")
        A(f"- 家电（消费白马集中，超卖后动量修复快）适合 RSI 上穿 40 的动量确认；")
        A(f"- 商贸零售（受电商冲击+消费分流，超卖后动量修复慢）不适合 RSI 40 阈值。")
        A(f"\n**胜率改善对比**：")
        A(f"- 家电胜率 44.8%→54.5%（+9.8pp，转正）")
        A(f"- 商贸胜率 {fmt_pct(base_wr10*100)}→{fmt_pct(b_wr10*100)}（{(b_wr10-base_wr10)*100:+.1f}pp，{'提升但仍未转正' if b_wr10>base_wr10 else '未提升'}）")
        if b_wr10 > base_wr10:
            A(f"\n**软优化可能**：商贸方案B虽未转正，但胜率{'有提升' if b_wr10>base_wr10 else '无提升'}，"
              f"可作「退而求其次」选项（详见 §5.3）。")
        A(f"\n**下一品类建议**：方案B非通用，需逐品类测，不能假设它在新品类上转正。")
    A("")

    # ---- 7. 下一品类建议 ----
    A("## 7. 下一品类建议（基于两品类经验）\n")
    A(f"本品类（sw_801200 商贸）是 buy_aux 逐个品类优化的第二个，加上家电打样共 2 个品类。经验总结：\n")
    if turned_positive and (b_f10 is not None and b_f10 > 0):
        A(f"1. **方案B在两品类均转正**（家电 +16.19% / 商贸 {fmt_f(b_f10)}），初步证明是通用优化方向。")
        A(f"2. 三 horizon 一致性是关键判据——单 horizon 转正不可靠，三 horizon 一致转正才稳健。")
        A(f"3. 样本数监控：方案收紧后 n 下降，若 n<30 需警示，n<10 无法判定凯利。")
        A(f"4. 下一品类建议继续首选 B_RSI上穿40 测试，若第三个品类也转正，则方案B可考虑作为 buy_aux 默认优化落地"
          f"（统一改 signals.py，而非逐品类配置）；若第三品类不转正则方案B仍品类特异性，需逐个测。")
        A(f"5. **可考虑组合方案**：A+C（反弹力度+放量）做更严格过滤，在样本充足的品类上试。")
    else:
        A(f"1. **方案B在家电转正、在商贸{'未转正' if not (b_f10 is not None and b_f10 > 0) else '转正但'}，"
          f"非完全通用，存在品类特异性。")
        A(f"2. 下一品类建议同样测 3 方案（A 反弹力度 / B RSI 动量 / C 放量），"
          f"看是否有更多品类能转正。若多个品类都无法转正，可能说明 buy_aux 的 BB 下轨回归逻辑"
    ...
    A("### 8.2 家电（sw_801110）参考数据（来自 14-家电buy_aux优化回测.md）\n")
    A("| 方案 | horizon | n | 胜率 | 盈亏比 | 均值 | 凯利f | 凯利 |")
    A("|---|---|---:|---:|---:|---:|---:|---|")
    A("| 基线_B1 | 5d | 135 | 51.9% | 0.84 | -0.14% | -5.28% | 不建议 |")
    A("| 基线_B1 | 10d **主** | 134 | 44.8% | 0.66 | -1.23% | -38.54% | 不建议 |")
    A("| 基线_B1 | 20d | 133 | 50.4% | 0.84 | -0.47% | -8.57% | 不建议 |")
    A("| A_反弹力度2% | 5d | 18 ⚠️ | 50.0% | 0.51 | -1.35% | -48.08% | 不建议 |")
    A("| A_反弹力度2% | 10d **主** | 18 ⚠️ | 22.2% | 0.57 | -4.68% | -114.60% | 不建议 |")
    A("| A_反弹力度2% | 20d | 18 ⚠️ | 44.4% | 0.80 | -1.49% | -25.14% | 不建议 |")
    A("| B_RSI上穿40 | 5d | 33 | 66.7% | 0.70 | +0.53% | +19.12% | 建议 |")
    A("| B_RSI上穿40 | 10d **主** | 33 | 54.5% | 1.19 | +0.54% | +16.19% | 建议 |")
    A("| B_RSI上穿40 | 20d | 33 | 54.5% | 1.21 | +1.16% | +17.11% | 建议 |")
    A("| C_放量1.2倍 | 5d | 8 ⚠️ | 75.0% | 0.19 | -1.17% | -57.84% | 样本不足 |")
    A("| C_放量1.2倍 | 10d **主** | 8 ⚠️ | 50.0% | 0.94 | -0.14% | -3.23% | 样本不足 |")
    A("| C_放量1.2倍 | 20d | 8 ⚠️ | 62.5% | 1.29 | +2.24% | +33.49% | 样本不足 |")
    A("")

    A("---")
    A("")
    A("*本报告由 `a-stock-data/backtest_buy_aux_sw801200.py` 自动生成。"
    ...
    print(f"\n=== 与家电对比（方案B）===")
    print(f"家电 B: 10d f=+16.19% (建议) 胜率=54.5% n=33")
    print(f"商贸 B: 10d f={fmt_f(b_f10)} ({results['B_RSI上穿40'][10][6]}) 胜率={fmt_pct(b_wr10*100)} n={b_n10}")
